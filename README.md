<!--lint disable awesome-contributing awesome-license awesome-list-item match-punctuation no-repeat-punctuation no-undefined-references awesome-spell-check-->
# Awesome Stars [![Awesome](https://awesome.re/badge.svg)](https://github.com/sindresorhus/awesome)

> A curated list of my GitHub stars! Generated using [starred](https://github.com/NiNiyas/starred).

## Contents

  - [Assembly](#assembly)
  - [Astro](#astro)
  - [AutoHotkey](#autohotkey)
  - [Batchfile](#batchfile)
  - [C](#c)
  - [C#](#c-1)
  - [C++](#c-2)
  - [CSS](#css)
  - [Clojure](#clojure)
  - [Crystal](#crystal)
  - [Cython](#cython)
  - [Dart](#dart)
  - [Dockerfile](#dockerfile)
  - [Elixir](#elixir)
  - [Elm](#elm)
  - [Fluent](#fluent)
  - [Go](#go)
  - [HTML](#html)
  - [Haskell](#haskell)
  - [Java](#java)
  - [JavaScript](#javascript)
  - [Jinja](#jinja)
  - [Jupyter Notebook](#jupyter-notebook)
  - [Kotlin](#kotlin)
  - [Lua](#lua)
  - [MDX](#mdx)
  - [Makefile](#makefile)
  - [Markdown](#markdown)
  - [Nim](#nim)
  - [Objective-C](#objective-c)
  - [Odin](#odin)
  - [Others](#others)
  - [PHP](#php)
  - [Perl](#perl)
  - [PowerShell](#powershell)
  - [Python](#python)
  - [Ruby](#ruby)
  - [Rust](#rust)
  - [SCSS](#scss)
  - [Shell](#shell)
  - [Svelte](#svelte)
  - [Swift](#swift)
  - [Text](#text)
  - [TypeScript](#typescript)
  - [VBScript](#vbscript)
  - [Visual Basic .NET](#visual-basic-net)
  - [Vue](#vue)
  - [YAML](#yaml)
  - [Zig](#zig)

## Assembly 

- [microsoft/MS-DOS](https://github.com/microsoft/MS-DOS) - The original sources of MS-DOS 1.25, 2.0, and 4.0 for reference purposes.
   - Updated on `Thu 25 April 2024 at 10:32 PM` | 32224⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Astro 

- [lissy93/awesome-privacy](https://github.com/lissy93/awesome-privacy) - 🦄  A curated list of privacy & security-focused software and services.
   - Updated on `Tue 09 June 2026 at 01:42 PM` | 9471⭐ | `Creative Commons Zero v1.0 Universal`
 
**[`^        back to top        ^`](#)**

## AutoHotkey 

- [ltguillaume/librewolf-winupdater](https://github.com/ltguillaume/librewolf-winupdater) - Mirror of https://codeberg.org/librewolf/winupdater. An attempt to make (automatic) updating of LibreWolf for Windows much easier. Can be used for installed and portable instances (https://github.com/.
   - Updated on `Thu 04 June 2026 at 10:26 PM` | 183⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Batchfile 

- [adavak/Win_ISO_Patching_Scripts](https://github.com/adavak/Win_ISO_Patching_Scripts) - Win_ISO_Patching_Scripts.
   - Updated on `Tue 09 June 2026 at 10:42 PM` | 517⭐ | `MIT License`
- [AveYo/MediaCreationTool.bat](https://github.com/AveYo/MediaCreationTool.bat) - Universal MCT wrapper script for all Windows 10/11 versions from 1507 to 21H2!.
   - Updated on `Thu 15 August 2024 at 12:41 PM` | 10227⭐ | `MIT License`
- [ionuttbara/windows-defender-remover](https://github.com/ionuttbara/windows-defender-remover) - A tool which is uses to remove Windows Defender in Windows 8.x, Windows 10 (every version) and Windows 11.
   - Updated on `Mon 09 February 2026 at 04:29 PM` | 7860⭐ | `Other`
- [massgravel/Microsoft-Activation-Scripts](https://github.com/massgravel/Microsoft-Activation-Scripts) - Open-source Windows and Office activator featuring HWID, Ohook, TSforge, and Online KMS activation methods, along with advanced troubleshooting.
   - Updated on `Thu 07 May 2026 at 04:42 PM` | 177891⭐ | `GNU General Public License v3.0`
- [Atlas-OS/Atlas](https://github.com/Atlas-OS/Atlas) - 🚀 An open and lightweight modification to Windows, designed to optimize performance, privacy and usability.
   - Updated on `Thu 21 May 2026 at 03:40 PM` | 20801⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## C 

- [mpc-hc/mpc-hc](https://github.com/mpc-hc/mpc-hc) - MPC-HC's main repository.  For support use our Trac: https://trac.mpc-hc.org/.
   - Updated on `Fri 24 April 2020 at 11:04 AM` | 5253⭐ | `GNU General Public License v3.0`
- [systemd/systemd](https://github.com/systemd/systemd) - The systemd System and Service Manager.
   - Updated on `Tue 09 June 2026 at 09:26 PM` | 16362⭐ | `GNU General Public License v2.0`
- [FreeRDP/FreeRDP](https://github.com/FreeRDP/FreeRDP) - FreeRDP is a free remote desktop protocol library and clients.
   - Updated on `Tue 09 June 2026 at 08:03 PM` | 13280⭐ | `Apache License 2.0`
- [hengyoush/kyanos](https://github.com/hengyoush/kyanos) - Kyanos is a networking analysis tool using eBPF. It can visualize the time packets spend in the kernel, capture requests/responses, makes troubleshooting more efficient.
   - Updated on `Tue 09 June 2026 at 11:32 PM` | 5026⭐ | `Apache License 2.0`
- [pawelsalawa/letos](https://github.com/pawelsalawa/letos) - A free, open source, multi-platform SQLite database manager.
   - Updated on `Tue 09 June 2026 at 09:48 PM` | 6571⭐ | `Other`
- [ZG089/Re-Malwack](https://github.com/ZG089/Re-Malwack) - A fully-fledged ad-block module. Contains all your needs ✨.
   - Updated on `Tue 09 June 2026 at 03:33 AM` | 382⭐ | `GNU General Public License v3.0`
- [brunodev85/winlator](https://github.com/brunodev85/winlator) - Android application for running Windows applications with Wine and Box86/Box64.
   - Updated on `Mon 01 June 2026 at 10:46 AM` | 17816⭐ | `GNU Lesser General Public License v2.1`
- [openssl/openssl](https://github.com/openssl/openssl) - General purpose TLS and crypto library.
   - Updated on `Tue 09 June 2026 at 06:20 PM` | 30281⭐ | `Apache License 2.0`
- [obsproject/obs-studio](https://github.com/obsproject/obs-studio) - OBS Studio - Free and open source software for live streaming and screen recording.
   - Updated on `Tue 09 June 2026 at 06:57 PM` | 73046⭐ | `GNU General Public License v2.0`
- [henrypp/simplewall](https://github.com/henrypp/simplewall) - Simple tool to configure Windows Filtering Platform (WFP) which can configure network activity on your computer.
   - Updated on `Mon 20 April 2026 at 09:50 AM` | 8513⭐ | `GNU General Public License v3.0`
- [flightlessmango/MangoHud](https://github.com/flightlessmango/MangoHud) - A Vulkan and OpenGL overlay for monitoring FPS, temperatures, CPU/GPU load and more.
   - Updated on `Mon 08 June 2026 at 10:29 PM` | 8697⭐ | `MIT License`
- [rainmeter/rainmeter](https://github.com/rainmeter/rainmeter) - Desktop customization tool for Windows.
   - Updated on `Tue 09 June 2026 at 03:23 PM` | 5820⭐ | `GNU General Public License v2.0`
- [winfsp/winfsp](https://github.com/winfsp/winfsp) - Windows File System Proxy - FUSE for Windows.
   - Updated on `Sun 31 May 2026 at 06:34 PM` | 8649⭐ | `Other`
- [valkey-io/valkey](https://github.com/valkey-io/valkey) - A flexible distributed key-value database that is optimized for caching and other realtime workloads.
   - Updated on `Tue 09 June 2026 at 11:45 PM` | 26078⭐ | `Other`
- [Next-Flip/Momentum-Firmware](https://github.com/Next-Flip/Momentum-Firmware) - 🐬 Feature-rich, stable and customizable Flipper Firmware.
   - Updated on `Tue 02 June 2026 at 11:16 PM` | 8634⭐ | `GNU General Public License v3.0`
- [flipperdevices/flipperzero-firmware](https://github.com/flipperdevices/flipperzero-firmware) - Flipper Zero firmware source code.
   - Updated on `Fri 05 December 2025 at 07:35 PM` | 16131⭐ | `GNU General Public License v3.0`
- [Klipper3d/klipper](https://github.com/Klipper3d/klipper) - Klipper is a 3d-printer firmware.
   - Updated on `Tue 09 June 2026 at 04:53 PM` | 11621⭐ | `GNU General Public License v3.0`
- [darktable-org/darktable](https://github.com/darktable-org/darktable) - darktable is an open source photography workflow application and raw developer.
   - Updated on `Tue 09 June 2026 at 07:12 PM` | 12549⭐ | `GNU General Public License v3.0`
- [libvips/libvips](https://github.com/libvips/libvips) - A fast image processing library with low memory needs.
   - Updated on `Tue 09 June 2026 at 11:35 AM` | 11382⭐ | `GNU Lesser General Public License v2.1`
- [tsl0922/ttyd](https://github.com/tsl0922/ttyd) - Share your terminal over the web.
   - Updated on `Tue 09 June 2026 at 02:30 PM` | 11818⭐ | `MIT License`
- [videolan/vlc](https://github.com/videolan/vlc) - VLC media player - plays everything, runs anywhere. Code here: https://code.videolan.org/videolan/vlc.
   - Updated on `Tue 09 June 2026 at 07:53 PM` | 18646⭐ | `GNU General Public License v2.0`
- [git/git](https://github.com/git/git) - Git Source Code Mirror - This is a publish-only repository but pull requests can be turned into patches to the mailing list via GitGitGadget (https://gitgitgadget.github.io/). Please follow Documentat.
   - Updated on `Tue 09 June 2026 at 10:33 PM` | 61409⭐ | `Other`
- [cyd01/KiTTY](https://github.com/cyd01/KiTTY) - :computer: KiTTY, a free telnet/ssh client for Windows.
   - Updated on `Sun 17 September 2023 at 05:21 PM` | 1797⭐ | `Other`
- [sandboxie-plus/Sandboxie](https://github.com/sandboxie-plus/Sandboxie) - Sandboxie Plus & Classic.
   - Updated on `Mon 08 June 2026 at 04:32 AM` | 18572⭐ | `GNU General Public License v3.0`
- [curl/curl](https://github.com/curl/curl) - A command line tool and library for transferring data with URL syntax, supporting DICT, FILE, FTP, FTPS, GOPHER, GOPHERS, HTTP, HTTPS, IMAP, IMAPS, LDAP, LDAPS, MQTT, MQTTS, POP3, POP3S, RTSP, SCP, SF.
   - Updated on `Tue 09 June 2026 at 07:17 PM` | 42090⭐ | `Other`
- [coturn/coturn](https://github.com/coturn/coturn) - coturn TURN server project.
   - Updated on `Sat 06 June 2026 at 05:12 AM` | 14083⭐ | `Other`
- [ventoy/Ventoy](https://github.com/ventoy/Ventoy) - A new bootable USB solution.
   - Updated on `Thu 07 May 2026 at 03:18 AM` | 77123⭐ | `GNU General Public License v3.0`
- [jszczerbinsky/lwp](https://github.com/jszczerbinsky/lwp) - Multi-platform parallax wallpaper engine.
   - Updated on `Sat 10 May 2025 at 02:28 PM` | 949⭐ | `MIT License`
- [redis/redis](https://github.com/redis/redis) - For developers, who are building real-time data-driven applications, Redis is the preferred, fastest, and most feature-rich cache, data structure server, and document and vector query engine.
   - Updated on `Tue 09 June 2026 at 11:13 AM` | 74769⭐ | `Other`
- [pbatard/rufus](https://github.com/pbatard/rufus) - The Reliable USB Formatting Utility.
   - Updated on `Wed 03 June 2026 at 03:45 PM` | 36412⭐ | `GNU General Public License v3.0`
- [owntracks/recorder](https://github.com/owntracks/recorder) - Store and access data published by OwnTracks apps.
   - Updated on `Wed 03 June 2026 at 11:32 AM` | 1178⭐ | `Other`
- [allinurl/goaccess](https://github.com/allinurl/goaccess) - GoAccess is a real-time web log analyzer and interactive viewer that runs in a terminal in *nix systems or through your browser.
   - Updated on `Tue 09 June 2026 at 03:22 AM` | 20626⭐ | `MIT License`
- [netdata/netdata](https://github.com/netdata/netdata) - The fastest path to AI-powered full stack observability, even for lean teams.
   - Updated on `Tue 09 June 2026 at 04:57 PM` | 79097⭐ | `GNU General Public License v3.0`
- [pi-hole/FTL](https://github.com/pi-hole/FTL) - The Pi-hole FTL engine.
   - Updated on `Mon 08 June 2026 at 06:08 PM` | 1658⭐ | `Other`
- [Legrandin/pycryptodome](https://github.com/Legrandin/pycryptodome) - A self-contained cryptographic library for Python.
   - Updated on `Sun 29 March 2026 at 07:51 AM` | 3232⭐ | `Other`
 
**[`^        back to top        ^`](#)**

## C# 

- [Santodan/PlayniteAchievements](https://github.com/Santodan/PlayniteAchievements) - .
   - Updated on `Wed 03 June 2026 at 04:18 PM` | 47⭐ | `MIT License`
- [itsfatduck/optimizerDuck](https://github.com/itsfatduck/optimizerDuck) - Free, open-source Windows optimization tool for performance, privacy, and simplicity.
   - Updated on `Mon 08 June 2026 at 01:15 PM` | 2754⭐ | `Other`
- [justin-delano/PlayniteAchievements](https://github.com/justin-delano/PlayniteAchievements) - .
   - Updated on `Wed 10 June 2026 at 12:21 AM` | 61⭐ | `MIT License`
- [Greedeks/GTweak](https://github.com/Greedeks/GTweak) - Portable Tool for an Ideal Windows Setup.
   - Updated on `Tue 09 June 2026 at 03:28 PM` | 1183⭐ | `BSD 3-Clause "New" or "Revised" License`
- [HotCakeX/Harden-Windows-Security](https://github.com/HotCakeX/Harden-Windows-Security) - Harden Windows Safely, Securely using Official Supported Microsoft methods and proper explanation | Always up-to-date and works with the latest build of Windows | Provides tools and Guides for Persona.
   - Updated on `Tue 09 June 2026 at 07:59 PM` | 4309⭐ | `MIT License`
- [BornToBeRoot/NETworkManager](https://github.com/BornToBeRoot/NETworkManager) - A powerful open-source tool for managing networks and troubleshooting network problems!.
   - Updated on `Mon 08 June 2026 at 05:07 PM` | 8354⭐ | `GNU General Public License v3.0`
- [rayenghanmi/RyTuneX](https://github.com/rayenghanmi/RyTuneX) - RyTuneX is a cutting-edge optimizer built with the WinUI 3 framework, designed to amplify the performance of Windows devices. Crafted for both Windows 10 and 11.
   - Updated on `Sun 07 June 2026 at 03:09 PM` | 5095⭐ | `GNU Affero General Public License v3.0`
- [shrimqy/Sefirah](https://github.com/shrimqy/Sefirah) - Phone Link / KDE Connect alternative.
   - Updated on `Tue 09 June 2026 at 06:49 PM` | 2473⭐ | `GNU General Public License v3.0`
- [vpnhood/VpnHood](https://github.com/vpnhood/VpnHood) - Undetectable, Fast,  Cross-Platform, Free VPN.
   - Updated on `Mon 08 June 2026 at 01:26 AM` | 2204⭐ | `GNU Lesser General Public License v2.1`
- [insomniachi/FluentFin](https://github.com/insomniachi/FluentFin) - .
   - Updated on `Thu 27 November 2025 at 04:10 PM` | 91⭐ | `GNU General Public License v3.0`
- [IAmParadox27/jellyfin-plugin-home-sections](https://github.com/IAmParadox27/jellyfin-plugin-home-sections) - Add server provided home sections to your Jellyfin instance.
   - Updated on `Mon 25 May 2026 at 08:05 PM` | 438⭐ | `GNU General Public License v3.0`
- [intro-skipper/intro-skipper](https://github.com/intro-skipper/intro-skipper) - Automatically detect and skip intro/credit sequences in Jellyfin.
   - Updated on `Mon 08 June 2026 at 06:57 PM` | 2443⭐ | `GNU General Public License v3.0`
- [beeradmoore/dlss-swapper](https://github.com/beeradmoore/dlss-swapper) - .
   - Updated on `Mon 20 April 2026 at 09:09 PM` | 6703⭐ | `GNU General Public License v3.0`
- [rogerfar/rdt-client](https://github.com/rogerfar/rdt-client) - Real-Debrid Client Proxy.
   - Updated on `Sat 06 June 2026 at 02:32 AM` | 1514⭐ | `MIT License`
- [microsoft/PowerToys](https://github.com/microsoft/PowerToys) - Microsoft PowerToys is a collection of utilities that supercharge productivity and customization on Windows.
   - Updated on `Tue 09 June 2026 at 08:13 PM` | 133969⭐ | `MIT License`
- [gibbed/SteamAchievementManager](https://github.com/gibbed/SteamAchievementManager) - A manager for game achievements in Steam.
   - Updated on `Thu 19 March 2026 at 09:35 AM` | 8458⭐ | `zlib License`
- [JustArchiNET/ArchiSteamFarm](https://github.com/JustArchiNET/ArchiSteamFarm) - C# application with primary purpose of farming Steam cards from multiple accounts simultaneously.
   - Updated on `Mon 08 June 2026 at 04:34 PM` | 13373⭐ | `Apache License 2.0`
- [BigheadSMZ/PGS](https://github.com/BigheadSMZ/PGS) - Multi-monitor application. Swap primary display when playing games.
   - Updated on `Fri 27 September 2024 at 09:21 PM` | 105⭐ | `Creative Commons Zero v1.0 Universal`
- [seerge/g-helper](https://github.com/seerge/g-helper) - Lightweight Armoury Crate alternative for Asus laptops with nearly the same functionality. Works with ROG Zephyrus, Flow, TUF, Strix, Scar, ProArt, Vivobook, Zenbook, Expertbook, ROG Ally, and many mo.
   - Updated on `Tue 09 June 2026 at 04:41 PM` | 13609⭐ | `GNU General Public License v3.0`
- [TheJoeFin/Text-Grab](https://github.com/TheJoeFin/Text-Grab) - Use OCR in Windows quickly and easily with Text Grab. With optional background process and notifications.
   - Updated on `Sat 06 June 2026 at 03:29 AM` | 4748⭐ | `MIT License`
- [Sophia-Community/SophiApp](https://github.com/Sophia-Community/SophiApp) - :zap: The most powerful open source tweaker on GitHub for fine-tuning Windows 10 & Windows 11.
   - Updated on `Fri 05 June 2026 at 05:22 PM` | 4994⭐ | `MIT License`
- [AutoDarkMode/Windows-Auto-Night-Mode](https://github.com/AutoDarkMode/Windows-Auto-Night-Mode) - Automatically switches between the dark and light theme of Windows 10 and Windows 11.
   - Updated on `Sun 10 May 2026 at 07:57 PM` | 9483⭐ | `GNU General Public License v3.0`
- [0x7c13/Notepads](https://github.com/0x7c13/Notepads) - A modern, lightweight text editor with a minimalist design.
   - Updated on `Mon 20 April 2026 at 11:45 AM` | 10121⭐ | `MIT License`
- [Devolutions/UniGetUI](https://github.com/Devolutions/UniGetUI) - UniGetUI: The Graphical Interface for your package managers. Could be terribly described as a package manager manager to manage your package managers.
   - Updated on `Tue 09 June 2026 at 08:47 PM` | 24342⭐ | `MIT License`
- [LibreHardwareMonitor/LibreHardwareMonitor](https://github.com/LibreHardwareMonitor/LibreHardwareMonitor) - Libre Hardware Monitor is free software that can monitor the temperature sensors, fan speeds, voltages, load and clock speeds of your computer.
   - Updated on `Fri 29 May 2026 at 10:14 AM` | 8486⭐ | `Mozilla Public License 2.0`
- [builtbybel/Bloatynosy](https://github.com/builtbybel/Bloatynosy) - The Bloaty and the Nosy: No Bloat, No Problem!.
   - Updated on `Tue 14 January 2025 at 05:57 PM` | 5593⭐ | `MIT License`
- [gerardog/gsudo](https://github.com/gerardog/gsudo) - Sudo for Windows.
   - Updated on `Thu 04 June 2026 at 12:02 AM` | 5971⭐ | `MIT License`
- [UnigramDev/Unigram](https://github.com/UnigramDev/Unigram) - Telegram for Windows.
   - Updated on `Fri 29 May 2026 at 04:14 PM` | 5218⭐ | `GNU General Public License v3.0`
- [chocolatey/choco](https://github.com/chocolatey/choco) - Chocolatey - the package manager for Windows.
   - Updated on `Wed 27 May 2026 at 08:30 AM` | 11409⭐ | `Other`
- [files-community/Files](https://github.com/files-community/Files) - A modern file manager that helps users organize their files and folders.
   - Updated on `Tue 09 June 2026 at 11:11 PM` | 43836⭐ | `MIT License`
- [Leo-Corporation/Gavilya](https://github.com/Leo-Corporation/Gavilya) - Gavilya is a simple game launcher written in C# .NET 8.
   - Updated on `Fri 17 April 2026 at 11:36 AM` | 72⭐ | `MIT License`
- [Orbmu2k/nvidiaProfileInspector](https://github.com/Orbmu2k/nvidiaProfileInspector) - .
   - Updated on `Sun 31 May 2026 at 06:21 AM` | 6579⭐ | `MIT License`
- [DevToys-app/DevToys](https://github.com/DevToys-app/DevToys) - A Swiss Army knife for developers.
   - Updated on `Wed 25 February 2026 at 11:53 AM` | 31617⭐ | `MIT License`
- [hellzerg/optimizer](https://github.com/hellzerg/optimizer) - The finest Windows Optimizer.
   - Updated on `Tue 20 January 2026 at 07:04 PM` | 18196⭐ | `GNU General Public License v3.0`
- [immense/Remotely](https://github.com/immense/Remotely) - A remote control and remote scripting solution, built with .NET 8, Blazor, and SignalR.
   - Updated on `Tue 17 December 2024 at 03:12 PM` | 5052⭐ | `GNU General Public License v3.0`
- [nicknsy/jellyscrub](https://github.com/nicknsy/jellyscrub) - Smooth mouse-over video scrubbing previews for Jellyfin.
   - Updated on `Sun 10 November 2024 at 09:56 PM` | 776⭐ | `MIT License`
- [recyclarr/recyclarr](https://github.com/recyclarr/recyclarr) - Automatically sync TRaSH Guides to your Sonarr and Radarr instances.
   - Updated on `Sun 07 June 2026 at 04:54 PM` | 2001⭐ | `MIT License`
- [ErsatzTV/legacy](https://github.com/ErsatzTV/legacy) - Open-source platform that transforms your personal media library into live, custom TV channels.
   - Updated on `Tue 09 June 2026 at 02:02 PM` | 2820⭐ | `zlib License`
- [philosowaffle/ambientweather-local-server](https://github.com/philosowaffle/ambientweather-local-server) - A server for receiving metrics from your local Ambient Weather Console and exposing them as Prometheus Metrics.
   - Updated on `Mon 28 October 2024 at 06:46 PM` | 47⭐ | `GNU General Public License v3.0`
- [shemanaev/jellyfin-plugin-webhooks](https://github.com/shemanaev/jellyfin-plugin-webhooks) - Webhooks for Jellyfin.
   - Updated on `Sun 19 October 2025 at 10:56 AM` | 35⭐ | `MIT License`
- [cjmanca/plex-credits-detect](https://github.com/cjmanca/plex-credits-detect) - Augments plex's built in intro detection, additionally detecting credits.
   - Updated on `Sun 12 February 2023 at 10:44 PM` | 112⭐ | `MIT License`
- [BCUninstaller/Bulk-Crap-Uninstaller](https://github.com/BCUninstaller/Bulk-Crap-Uninstaller) - Remove large amounts of unwanted applications quickly.
   - Updated on `Tue 09 June 2026 at 09:57 PM` | 19566⭐ | `Apache License 2.0`
- [ConfusedPolarBear/intro-skipper](https://github.com/ConfusedPolarBear/intro-skipper) - Fingerprint audio to automatically detect and skip intro sequences in Jellyfin.
   - Updated on `Mon 04 September 2023 at 09:47 AM` | 972⭐ | `GNU General Public License v3.0`
- [Whisparr/Whisparr](https://github.com/Whisparr/Whisparr) - .
   - Updated on `Mon 08 June 2026 at 06:27 PM` | 1001⭐ | `GNU General Public License v3.0`
- [Jackett/Jackett](https://github.com/Jackett/Jackett) - API Support for your favorite torrent trackers.
   - Updated on `Tue 09 June 2026 at 03:08 PM` | 15542⭐ | `GNU General Public License v2.0`
- [ShareX/ShareX](https://github.com/ShareX/ShareX) - ShareX is a free and open-source application that enables users to capture or record any area of their screen with a single keystroke. It also supports uploading images, text, and various file types t.
   - Updated on `Tue 09 June 2026 at 12:27 AM` | 38025⭐ | `GNU General Public License v3.0`
- [Prowlarr/Prowlarr](https://github.com/Prowlarr/Prowlarr) - Prowlarr is an indexer manager/proxy built on the popular *arr .net/reactjs base stack to integrate with your various PVR apps, supporting management of both Torrent Trackers and Usenet Indexers.
   - Updated on `Tue 09 June 2026 at 07:31 PM` | 6655⭐ | `GNU General Public License v3.0`
- [darkalfx/requestrr](https://github.com/darkalfx/requestrr) - Requestrr is a chatbot used to simplify using services like Sonarr/Radarr/Ombi via the use of chat. Current platform is Discord only, but the bot was built around the ideology of quick adaptation for .
   - Updated on `Sat 06 January 2024 at 04:58 AM` | 901⭐ | `MIT License`
- [JosefNemec/Playnite](https://github.com/JosefNemec/Playnite) - Video game library manager with support for wide range of 3rd party libraries and game emulation support, providing one unified interface for your games.
   - Updated on `Tue 26 May 2026 at 01:02 PM` | 13207⭐ | `MIT License`
- [jonas-med-ett-s/idle_master_extended](https://github.com/jonas-med-ett-s/idle_master_extended) - 🃏 Get your Steam Trading Cards the Fast Way (Fast Mode Extension 🚀).
   - Updated on `Tue 28 April 2026 at 09:11 AM` | 3228⭐ | `GNU General Public License v2.0`
- [oonqt/Emby.Notifications.Discord](https://github.com/oonqt/Emby.Notifications.Discord) - Emby plugin for Discord notifications.
   - Updated on `Mon 08 December 2025 at 04:27 PM` | 25⭐ | `MIT License`
- [duplicati/duplicati](https://github.com/duplicati/duplicati) - Store securely encrypted backups in the cloud!.
   - Updated on `Tue 09 June 2026 at 07:00 PM` | 14636⭐ | `Other`
- [JavScraper/Emby.Plugins.JavScraper](https://github.com/JavScraper/Emby.Plugins.JavScraper) - Emby/Jellyfin 的一个日本电影刮削器插件，可以从某些网站抓取影片信息。.
   - Updated on `Mon 08 July 2024 at 09:05 AM` | 3770⭐ | `❌`
- [mregni/EmbyStat](https://github.com/mregni/EmbyStat) - Multiplatform statistics server for Emby server.
   - Updated on `Wed 15 January 2025 at 12:11 PM` | 292⭐ | `MIT License`
- [Lidarr/Lidarr](https://github.com/Lidarr/Lidarr) - Looks and smells like Sonarr but made for music.
   - Updated on `Tue 09 June 2026 at 07:58 AM` | 5375⭐ | `GNU General Public License v3.0`
- [MediaBrowser/Emby](https://github.com/MediaBrowser/Emby) - Emby Server is a personal media server with apps on just about every device.
   - Updated on `Wed 27 March 2024 at 06:20 PM` | 4893⭐ | `GNU General Public License v2.0`
- [jellyfin/jellyfin](https://github.com/jellyfin/jellyfin) - The Free Software Media System - Server Backend & API.
   - Updated on `Tue 09 June 2026 at 07:23 PM` | 53082⭐ | `GNU General Public License v2.0`
- [Radarr/Radarr](https://github.com/Radarr/Radarr) - Movie organizer/manager for usenet and torrent users.
   - Updated on `Tue 09 June 2026 at 07:31 PM` | 13760⭐ | `GNU General Public License v3.0`
- [Sonarr/Sonarr](https://github.com/Sonarr/Sonarr) - Smart PVR for newsgroup and bittorrent users.
   - Updated on `Sun 07 June 2026 at 09:02 PM` | 13946⭐ | `GNU General Public License v3.0`
- [Xeroday/Spotify-Ad-Blocker](https://github.com/Xeroday/Spotify-Ad-Blocker) - EZBlocker - A Spotify Ad Blocker for Windows.
   - Updated on `Sun 17 April 2022 at 09:55 PM` | 1850⭐ | `GNU General Public License v3.0`
- [microsoft/WhatTheHack](https://github.com/microsoft/WhatTheHack) - A collection of challenge based hack-a-thons including student guide, coach guide, lecture presentations, sample/instructional code and templates.  Please visit the What The Hack website at: https://a.
   - Updated on `Tue 09 June 2026 at 05:35 PM` | 1891⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## C++ 

- [OpenSteam001/OpenSteamTool](https://github.com/OpenSteam001/OpenSteamTool) - Open Source Steam Unlocker.
   - Updated on `Wed 03 June 2026 at 04:20 PM` | 887⭐ | `GNU General Public License v3.0`
- [Korthos-Software/low_latency_layer](https://github.com/Korthos-Software/low_latency_layer) - Vulkan layer for hardware agnostic input latency reduction.
   - Updated on `Sun 24 May 2026 at 06:44 AM` | 686⭐ | `MIT License`
- [windirstat/windirstat](https://github.com/windirstat/windirstat) - WinDirStat is a disk usage statistics viewer and cleanup tool for Microsoft Windows.
   - Updated on `Mon 08 June 2026 at 12:14 PM` | 3408⭐ | `GNU General Public License v2.0`
- [chrisant996/clink](https://github.com/chrisant996/clink) - Bash's powerful command line editing in cmd.exe.
   - Updated on `Tue 09 June 2026 at 05:10 PM` | 5282⭐ | `GNU General Public License v3.0`
- [hyprwm/Hyprland](https://github.com/hyprwm/Hyprland) - Hyprland is an independent, highly customizable, dynamic tiling Wayland compositor that doesn't sacrifice on its looks.
   - Updated on `Tue 09 June 2026 at 06:01 PM` | 36288⭐ | `BSD 3-Clause "New" or "Revised" License`
- [Alex313031/thorium](https://github.com/Alex313031/thorium) - Chromium fork named after radioactive element No. 90. Source code and Linux releases. Windows/MacOS/ARM builds served in different repos, links are towards the top of the README.md.
   - Updated on `Mon 08 June 2026 at 08:32 AM` | 7202⭐ | `BSD 3-Clause "New" or "Revised" License`
- [LadybirdBrowser/ladybird](https://github.com/LadybirdBrowser/ladybird) - Truly independent web browser.
   - Updated on `Tue 09 June 2026 at 11:34 PM` | 63985⭐ | `BSD 2-Clause "Simplified" License`
- [shadps4-emu/shadPS4](https://github.com/shadps4-emu/shadPS4) - PlayStation 4 emulator for Windows, Linux, macOS and FreeBSD written in C++.
   - Updated on `Tue 09 June 2026 at 03:33 PM` | 31356⭐ | `GNU General Public License v2.0`
- [ramensoftware/windhawk](https://github.com/ramensoftware/windhawk) - The customization marketplace for Windows programs: https://windhawk.net/.
   - Updated on `Mon 08 December 2025 at 08:30 PM` | 8085⭐ | `GNU General Public License v3.0`
- [omeryusufyagci/fast-music-remover](https://github.com/omeryusufyagci/fast-music-remover) - A C++ based, lightweight music and noise remover for YouTube and other internet media, using DeepFilterNet for audio enhancement.
   - Updated on `Wed 15 January 2025 at 07:11 PM` | 717⭐ | `MIT License`
- [FreeCAD/FreeCAD](https://github.com/FreeCAD/FreeCAD) - Official source code of FreeCAD, a free and opensource multiplatform 3D parametric modeler.
   - Updated on `Tue 09 June 2026 at 02:33 PM` | 31432⭐ | `GNU Lesser General Public License v2.1`
- [WerWolv/ImHex](https://github.com/WerWolv/ImHex) - 🔍 A Hex Editor for Reverse Engineers, Programmers and people who value their retinas when working at 3 AM.
   - Updated on `Sun 07 June 2026 at 01:17 AM` | 53813⭐ | `GNU General Public License v2.0`
- [xHybred/DLSSEnhancer](https://github.com/xHybred/DLSSEnhancer) - Force DLAA on DLSS-supported titles, custom scaling ratios, change presets, disable anti-aliasing.
   - Updated on `Tue 29 April 2025 at 07:35 PM` | 58⭐ | `MIT License`
- [amnezia-vpn/amnezia-client](https://github.com/amnezia-vpn/amnezia-client) - Amnezia VPN Client (Desktop+Mobile).
   - Updated on `Tue 09 June 2026 at 04:35 PM` | 12376⭐ | `GNU General Public License v3.0`
- [WinMerge/winmerge](https://github.com/WinMerge/winmerge) - WinMerge is an Open Source differencing and merging tool for Windows. WinMerge can compare both folders and files, presenting differences in a visual text format that is easy to understand and handle.
   - Updated on `Tue 09 June 2026 at 10:31 PM` | 8876⭐ | `GNU General Public License v2.0`
- [snake-4/Zygisk-Assistant](https://github.com/snake-4/Zygisk-Assistant) - A Zygisk module to hide root for KernelSU, Magisk and APatch, designed to work on Android 5.0 and above.
   - Updated on `Mon 04 May 2026 at 02:08 PM` | 2495⭐ | `MIT License`
- [CalcProgrammer1/OpenRGB](https://github.com/CalcProgrammer1/OpenRGB) - Open source RGB lighting control that doesn't depend on manufacturer software. Supports Windows, Linux, MacOS.  Mirror of https://gitlab.com/CalcProgrammer1/OpenRGB.  Releases can be found on GitLab.
   - Updated on `Tue 09 June 2026 at 01:17 PM` | 4109⭐ | `GNU General Public License v2.0`
- [notepad-plus-plus/notepad-plus-plus](https://github.com/notepad-plus-plus/notepad-plus-plus) - Notepad++ official repository.
   - Updated on `Tue 09 June 2026 at 06:42 PM` | 28331⭐ | `Other`
- [tnodir/fort](https://github.com/tnodir/fort) - Fort Firewall for Windows.
   - Updated on `Thu 04 June 2026 at 06:18 AM` | 3118⭐ | `GNU General Public License v3.0`
- [GloriousEggroll/proton-ge-custom](https://github.com/GloriousEggroll/proton-ge-custom) - Compatibility tool for Steam Play based on Wine and additional components.
   - Updated on `Tue 09 June 2026 at 11:41 PM` | 14363⭐ | `Other`
- [ValveSoftware/Proton](https://github.com/ValveSoftware/Proton) - Compatibility tool for Steam Play based on Wine and additional components.
   - Updated on `Tue 09 June 2026 at 11:44 PM` | 31801⭐ | `Other`
- [RPCS3/rpcs3](https://github.com/RPCS3/rpcs3) - PlayStation 3 emulator and debugger.
   - Updated on `Tue 09 June 2026 at 08:53 PM` | 18929⭐ | `GNU General Public License v2.0`
- [azerothcore/azerothcore-wotlk](https://github.com/azerothcore/azerothcore-wotlk) - Complete Open Source and Modular solution for MMO.
   - Updated on `Wed 10 June 2026 at 12:19 AM` | 8453⭐ | `GNU General Public License v2.0`
- [jrouwe/JoltPhysics](https://github.com/jrouwe/JoltPhysics) - A multi core friendly rigid body physics and collision detection library. Written in C++. Suitable for games and VR applications. Used by Horizon Forbidden West and Death Stranding 2.
   - Updated on `Tue 09 June 2026 at 12:05 PM` | 10470⭐ | `MIT License`
- [SerenityOS/serenity](https://github.com/SerenityOS/serenity) - The Serenity Operating System 🐞.
   - Updated on `Tue 09 June 2026 at 07:02 PM` | 33402⭐ | `BSD 2-Clause "Simplified" License`
- [react/react-native](https://github.com/react/react-native) - A framework for building native applications using React.
   - Updated on `Tue 09 June 2026 at 05:33 PM` | 125962⭐ | `MIT License`
- [fantasy-peak/cpp-freegpt-webui](https://github.com/fantasy-peak/cpp-freegpt-webui) - GPT 3.5/4 with a Chat Web UI. No API key required. Web server developed with c++.
   - Updated on `Wed 13 March 2024 at 04:54 AM` | 238⭐ | `GNU General Public License v3.0`
- [ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp) - LLM inference in C/C++.
   - Updated on `Tue 09 June 2026 at 07:33 PM` | 115795⭐ | `MIT License`
- [nomic-ai/gpt4all](https://github.com/nomic-ai/gpt4all) - GPT4All: Run Local LLMs on Any Device. Open-source and available for commercial use.
   - Updated on `Tue 27 May 2025 at 08:05 PM` | 77359⭐ | `MIT License`
- [tesseract-ocr/tesseract](https://github.com/tesseract-ocr/tesseract) - Tesseract Open Source OCR Engine (main repository).
   - Updated on `Thu 04 June 2026 at 10:24 AM` | 74585⭐ | `Apache License 2.0`
- [Nukem9/dlssg-to-fsr3](https://github.com/Nukem9/dlssg-to-fsr3) - Adds AMD FSR 3 Frame Generation to games by replacing Nvidia DLSS Frame Generation (nvngx_dlssg).
   - Updated on `Sun 16 March 2025 at 06:56 PM` | 4954⭐ | `GNU General Public License v3.0`
- [mozilla-ai/llamafile](https://github.com/mozilla-ai/llamafile) - Distribute and run LLMs with a single file.
   - Updated on `Tue 09 June 2026 at 12:15 PM` | 24761⭐ | `Other`
- [pybind/pybind11](https://github.com/pybind/pybind11) - Seamless operability between C++11 and Python.
   - Updated on `Sat 06 June 2026 at 06:06 PM` | 17899⭐ | `Other`
- [DDoSolitary/LxRunOffline](https://github.com/DDoSolitary/LxRunOffline) - A full-featured utility for managing Windows Subsystem for Linux (WSL).
   - Updated on `Wed 02 February 2022 at 10:39 AM` | 4268⭐ | `MIT License`
- [FlaxEngine/FlaxEngine](https://github.com/FlaxEngine/FlaxEngine) - Flax Engine – multi-platform 3D game engine.
   - Updated on `Tue 09 June 2026 at 12:06 PM` | 6840⭐ | `Other`
- [openappsec/openappsec](https://github.com/openappsec/openappsec) - open-appsec is a machine learning security engine that preemptively and automatically prevents threats against Web Application & APIs. This repo include the main code and logic.
   - Updated on `Tue 26 May 2026 at 08:56 AM` | 1634⭐ | `Apache License 2.0`
- [hrydgard/ppsspp](https://github.com/hrydgard/ppsspp) - A PSP emulator for Android, Windows, Mac, Linux and iOS, written in C++. Want to contribute? Join us on Discord at https://discord.gg/5NJB6dD or just send pull requests / issues.
   - Updated on `Tue 09 June 2026 at 09:50 PM` | 13933⭐ | `Other`
- [Alkl58/RcloneBrowser](https://github.com/Alkl58/RcloneBrowser) - Simple cross platform GUI for rclone. Supports macOS, GNU/Linux, BSD family and Windows.
   - Updated on `Fri 24 March 2023 at 10:54 AM` | 120⭐ | `MIT License`
- [tensorflow/tensorflow](https://github.com/tensorflow/tensorflow) - An Open Source Machine Learning Framework for Everyone.
   - Updated on `Wed 10 June 2026 at 12:18 AM` | 195621⭐ | `Apache License 2.0`
- [organicmaps/organicmaps](https://github.com/organicmaps/organicmaps) - 🍃 Organic Maps is a free Android & iOS offline maps app for more than 6M travelers, tourists, hikers, and cyclists. It uses crowd-sourced OpenStreetMap data and is developed with love by the community.
   - Updated on `Tue 09 June 2026 at 09:22 PM` | 14249⭐ | `Other`
- [filecxx/FileCentipede](https://github.com/filecxx/FileCentipede) - Cross-platform internet upload/download manager for HTTP(S), FTP(S), SSH, magnet-link, BitTorrent, m3u8, ed2k, and online videos.  WebDAV client, FTP client, SSH client.
   - Updated on `Tue 02 June 2026 at 10:19 AM` | 10879⭐ | `❌`
- [uazo/cromite](https://github.com/uazo/cromite) - Cromite a Bromite fork with ad blocking and privacy enhancements; take back your browser!.
   - Updated on `Tue 09 June 2026 at 06:47 AM` | 7619⭐ | `GNU General Public License v3.0`
- [PotatoOfDoom/CyberFSR2](https://github.com/PotatoOfDoom/CyberFSR2) - FidelityFx Super Resolution 2.0 for Cyberpunk.
   - Updated on `Tue 05 September 2023 at 08:34 PM` | 617⭐ | `MIT License`
- [jellyfin-archive/jellyfin-desktop-qt](https://github.com/jellyfin-archive/jellyfin-desktop-qt) - Jellyfin Desktop Client.
   - Updated on `Sat 28 March 2026 at 02:13 AM` | 5495⭐ | `GNU General Public License v2.0`
- [ggml-org/whisper.cpp](https://github.com/ggml-org/whisper.cpp) - Port of OpenAI's Whisper model in C/C++.
   - Updated on `Tue 09 June 2026 at 10:51 AM` | 50601⭐ | `MIT License`
- [LizardByte/Sunshine](https://github.com/LizardByte/Sunshine) - Self-hosted game stream host for Moonlight.
   - Updated on `Tue 09 June 2026 at 03:23 PM` | 37826⭐ | `GNU General Public License v3.0`
- [Rafiuth/Soggfy](https://github.com/Rafiuth/Soggfy) - Spotify ogg dumper.
   - Updated on `Tue 02 July 2024 at 03:17 PM` | 1453⭐ | `Creative Commons Zero v1.0 Universal`
- [MOVZX/CyberFSR2](https://github.com/MOVZX/CyberFSR2) - CyberFSR/PotatoFSR (FSR 2.1.2) for Games.
   - Updated on `Thu 22 December 2022 at 12:05 PM` | 205⭐ | `MIT License`
- [transmission/transmission](https://github.com/transmission/transmission) - Official Transmission BitTorrent client repository.
   - Updated on `Wed 10 June 2026 at 12:01 AM` | 14815⭐ | `Other`
- [cloudflare/workerd](https://github.com/cloudflare/workerd) - The JavaScript / Wasm runtime that powers Cloudflare Workers.
   - Updated on `Tue 09 June 2026 at 09:15 PM` | 8295⭐ | `Apache License 2.0`
- [weserv/images](https://github.com/weserv/images) - Source code of wsrv.nl (formerly images.weserv.nl), to be used on your own server(s).
   - Updated on `Mon 08 June 2026 at 04:18 PM` | 2674⭐ | `BSD 3-Clause "New" or "Revised" License`
- [c0re100/qBittorrent-Enhanced-Edition](https://github.com/c0re100/qBittorrent-Enhanced-Edition) - [Unofficial] qBittorrent Enhanced, based on qBittorrent.
   - Updated on `Mon 01 June 2026 at 04:19 AM` | 25939⭐ | `Other`
- [DroneDB/DroneDB](https://github.com/DroneDB/DroneDB) - Free and open source software for geospatial data storage.
   - Updated on `Tue 02 June 2026 at 12:12 AM` | 273⭐ | `Mozilla Public License 2.0`
- [epoupon/fileshelter](https://github.com/epoupon/fileshelter) - FileShelter is a “one-click” file sharing web application.
   - Updated on `Sun 01 March 2026 at 04:45 PM` | 536⭐ | `GNU General Public License v3.0`
- [sc0ty/subsync](https://github.com/sc0ty/subsync) - Subtitle Speech Synchronizer.
   - Updated on `Tue 01 October 2024 at 01:47 PM` | 1425⭐ | `GNU General Public License v3.0`
- [godotengine/godot](https://github.com/godotengine/godot) - Godot Engine – Multi-platform 2D and 3D game engine.
   - Updated on `Tue 09 June 2026 at 04:22 PM` | 112308⭐ | `MIT License`
- [gerbera/gerbera](https://github.com/gerbera/gerbera) - UPnP Media Server for 2026: Stream your digital media through your home network and consume it on all kinds of UPnP supporting devices 📱💻📺.
   - Updated on `Tue 09 June 2026 at 05:42 PM` | 1372⭐ | `Other`
- [tstack/lnav](https://github.com/tstack/lnav) - Log file navigator.
   - Updated on `Sat 30 May 2026 at 05:19 PM` | 10328⭐ | `BSD 2-Clause "Simplified" License`
- [xbmc/xbmc](https://github.com/xbmc/xbmc) - Kodi is an award-winning free and open source home theater/media center software and entertainment hub for digital media. With its beautiful interface and powerful skinning engine, it's available for .
   - Updated on `Tue 09 June 2026 at 12:52 PM` | 20833⭐ | `Other`
- [k4yt3x/video2x](https://github.com/k4yt3x/video2x) - A machine learning-based video super resolution and frame interpolation framework. Est. Hack the Valley II, 2018.
   - Updated on `Sat 07 March 2026 at 03:39 PM` | 20145⭐ | `GNU Affero General Public License v3.0`
- [rakshasa/rtorrent](https://github.com/rakshasa/rtorrent) - rTorrent BitTorrent client.
   - Updated on `Tue 09 June 2026 at 08:50 AM` | 4796⭐ | `GNU General Public License v2.0`
- [qbittorrent/qBittorrent](https://github.com/qbittorrent/qBittorrent) - qBittorrent BitTorrent client.
   - Updated on `Tue 09 June 2026 at 11:49 AM` | 38066⭐ | `Other`
- [moonlight-stream/moonlight-qt](https://github.com/moonlight-stream/moonlight-qt) - GameStream client for PCs (Windows, Mac, Linux, and Steam Link).
   - Updated on `Tue 09 June 2026 at 05:01 PM` | 17482⭐ | `GNU General Public License v3.0`
- [microsoft/terminal](https://github.com/microsoft/terminal) - The new Windows Terminal and the original Windows console host, all in the same place!.
   - Updated on `Tue 09 June 2026 at 09:55 PM` | 103536⭐ | `MIT License`
- [aria2/aria2](https://github.com/aria2/aria2) - aria2 is a lightweight multi-protocol & multi-source, cross platform download utility operated in command-line. It supports HTTP/HTTPS, FTP, SFTP, BitTorrent and Metalink.
   - Updated on `Fri 15 May 2026 at 10:21 AM` | 41160⭐ | `GNU General Public License v2.0`
 
**[`^        back to top        ^`](#)**

## CSS 

- [Diolinux/PhotoGIMP](https://github.com/Diolinux/PhotoGIMP) - A Patch for GIMP 3+ for Photoshop Users.
   - Updated on `Sun 17 May 2026 at 07:36 PM` | 11557⭐ | `GNU General Public License v3.0`
- [Naereen/Nginx-Fancyindex-Theme](https://github.com/Naereen/Nginx-Fancyindex-Theme) - A :iphone: responsive theme for Nginx Fancyindex module :wrench:. Minimal, modern and simple. ☀ Light & 😎 Dark themes. Comes with a search form :mag_right:, aims to handle thousands of files without a.
   - Updated on `Thu 29 January 2026 at 11:43 PM` | 565⭐ | `MIT License`
- [iqbaladinur/bulmin](https://github.com/iqbaladinur/bulmin) - Free admin template based on bulma css.
   - Updated on `Tue 29 January 2019 at 11:15 AM` | 9⭐ | `MIT License`
- [necolas/normalize.css](https://github.com/necolas/normalize.css) - A modern alternative to CSS resets.
   - Updated on `Wed 12 June 2024 at 08:36 PM` | 53536⭐ | `MIT License`
- [jgthms/bulma](https://github.com/jgthms/bulma) - Modern CSS framework based on Flexbox.
   - Updated on `Sun 01 March 2026 at 08:13 PM` | 50074⭐ | `MIT License`
- [fallenbagel/Hint-of-Colors](https://github.com/fallenbagel/Hint-of-Colors) - Heavily inspired by Hint-of-Green but with more colors and heavily modified.
   - Updated on `Wed 14 August 2024 at 06:46 PM` | 20⭐ | `❌`
- [DanielHalevi/JellyTheme](https://github.com/DanielHalevi/JellyTheme) - A heavily modified theme for Jellyfin.
   - Updated on `Tue 11 July 2023 at 09:30 AM` | 86⭐ | `❌`
- [mrrfv/open-android-backup](https://github.com/mrrfv/open-android-backup) - Back up your device without vendor lock-ins, using insecure software or root. Supports encryption and compression out of the box. Works cross-platform.
   - Updated on `Fri 24 April 2026 at 11:39 PM` | 1306⭐ | `GNU General Public License v3.0`
- [jniggemann/filebrowser-css](https://github.com/jniggemann/filebrowser-css) - Custom stylesheet for filebrowser that adds colors and changes icons.
   - Updated on `Tue 26 April 2022 at 10:06 AM` | 79⭐ | `GNU Affero General Public License v3.0`
- [themepark-dev/theme.park](https://github.com/themepark-dev/theme.park) - A collection of themes/skins for 50 selfhosted apps!.
   - Updated on `Thu 28 May 2026 at 06:37 PM` | 3055⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Clojure 

- [penpot/penpot](https://github.com/penpot/penpot) - Penpot: The open-source design tool for design and code collaboration.
   - Updated on `Tue 09 June 2026 at 05:09 PM` | 49445⭐ | `Mozilla Public License 2.0`
- [logseq/logseq](https://github.com/logseq/logseq) - A privacy-first, open-source platform for knowledge management and collaboration. Download link:  http://github.com/logseq/logseq/releases. roadmap: https://logseq.io/p/NX4mc_ggEV.
   - Updated on `Tue 09 June 2026 at 04:28 PM` | 43296⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Crystal 

- [iv-org/invidious](https://github.com/iv-org/invidious) - Invidious is an alternative front-end to YouTube.
   - Updated on `Tue 09 June 2026 at 08:25 PM` | 20387⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Cython 

- [MagicStack/uvloop](https://github.com/MagicStack/uvloop) - Ultra fast asyncio event loop.
   - Updated on `Mon 04 May 2026 at 04:01 PM` | 11814⭐ | `Apache License 2.0`
 
**[`^        back to top        ^`](#)**

## Dart 

- [jidukrishna/otp_listener](https://github.com/jidukrishna/otp_listener) - .
   - Updated on `Sat 06 June 2026 at 03:38 PM` | 29⭐ | `❌`
- [edde746/plezy](https://github.com/edde746/plezy) - Modern cross-platform Plex & Jellyfin client built with Flutter.
   - Updated on `Tue 09 June 2026 at 01:18 PM` | 2487⭐ | `GNU General Public License v3.0`
- [jellyflix-app/jellyflix](https://github.com/jellyflix-app/jellyflix) - Jellyfix is an easy-to-use Jellyfin client for movies and shows. It's cross-platform and supports Mobile (iOS, Android) and Desktop (Windows, Mac, Linux).
   - Updated on `Sat 23 May 2026 at 04:07 PM` | 460⭐ | `GNU General Public License v3.0`
- [BasedHardware/omi](https://github.com/BasedHardware/omi) - AI that sees your screen, listens to your conversations and tells you what to do.
   - Updated on `Tue 09 June 2026 at 09:35 PM` | 12756⭐ | `MIT License`
- [RIP-Comm/sossoldi](https://github.com/RIP-Comm/sossoldi) - "Sossoldi" is a wealth management / personal finance / Net Worth tracking app, made with Flutter.
   - Updated on `Tue 26 May 2026 at 06:00 PM` | 1357⭐ | `MIT License`
- [Rezivure/Grid-Mobile](https://github.com/Rezivure/Grid-Mobile) - Grid: Private Location Sharing mobile app for iOS/Android. E2EE with Matrix.
   - Updated on `Sun 07 June 2026 at 01:18 PM` | 485⭐ | `Other`
- [DonutWare/Fladder](https://github.com/DonutWare/Fladder) - Fladder - A cross-platform Jellyfin Frontend built on top of Flutter.
   - Updated on `Mon 08 June 2026 at 12:45 AM` | 2257⭐ | `GNU General Public License v3.0`
- [JGeek00/adguard-home-manager](https://github.com/JGeek00/adguard-home-manager) - AdGuard Home client created with Flutter.
   - Updated on `Sat 29 November 2025 at 02:37 PM` | 472⭐ | `Apache License 2.0`
- [hiddify/hiddify-app](https://github.com/hiddify/hiddify-app) - Multi-platform auto-proxy client, supporting Sing-box, X-ray, TUIC, Hysteria, Reality, Trojan, SSH etc. It’s an open-source, secure and ad-free.
   - Updated on `Sun 31 May 2026 at 08:56 PM` | 30555⭐ | `Other`
- [JHubi1/ollama-app](https://github.com/JHubi1/ollama-app) - A modern and easy-to-use client for Ollama.
   - Updated on `Tue 28 October 2025 at 06:23 AM` | 1769⭐ | `Apache License 2.0`
- [krille-chan/fluffychat](https://github.com/krille-chan/fluffychat) - The cutest instant messenger in the [matrix].
   - Updated on `Tue 09 June 2026 at 05:18 PM` | 2865⭐ | `GNU Affero General Public License v3.0`
- [flutter/flutter](https://github.com/flutter/flutter) - Flutter makes it easy and fast to build beautiful apps for mobile and beyond.
   - Updated on `Tue 09 June 2026 at 11:45 PM` | 176875⭐ | `BSD 3-Clause "New" or "Revised" License`
- [ente-io/ente](https://github.com/ente-io/ente) - 💚 End-to-end encrypted cloud for everything.
   - Updated on `Wed 10 June 2026 at 12:09 AM` | 27026⭐ | `GNU Affero General Public License v3.0`
- [Solido/awesome-flutter](https://github.com/Solido/awesome-flutter) - An awesome list that curates the best Flutter libraries, tools, tutorials, articles and more.
   - Updated on `Tue 21 April 2026 at 07:06 PM` | 60265⭐ | `❌`
- [darkmoonight/Rain](https://github.com/darkmoonight/Rain) - 🌦️ Weather application.
   - Updated on `Tue 09 June 2026 at 09:05 PM` | 1022⭐ | `MIT License`
- [localsend/localsend](https://github.com/localsend/localsend) - An open-source cross-platform alternative to AirDrop.
   - Updated on `Fri 05 June 2026 at 10:03 AM` | 83090⭐ | `Apache License 2.0`
- [namanshergill/diohub](https://github.com/namanshergill/diohub) - An open-source unofficial GitHub mobile client, that aims to deliver the ultimate GitHub experience on mobile devices.
   - Updated on `Wed 22 April 2026 at 09:08 AM` | 943⭐ | `GNU General Public License v3.0`
- [TomBursch/kitchenowl](https://github.com/TomBursch/kitchenowl) - KitchenOwl is a self-hosted grocery list and recipe manager. The backend is made with Flask and the frontend with Flutter. Easily add items to your shopping list before you go shopping. You can also c.
   - Updated on `Thu 04 June 2026 at 06:52 PM` | 3389⭐ | `GNU Affero General Public License v3.0`
- [jonjomckay/fritter](https://github.com/jonjomckay/fritter) - A privacy-friendly Twitter frontend for mobile devices.
   - Updated on `Fri 15 May 2026 at 10:11 AM` | 1447⭐ | `MIT License`
- [zbejas/portarius](https://github.com/zbejas/portarius) - Portarius is a free, open-source, cross-platform mobile app that allows you to manage your Portainer sessions.
   - Updated on `Thu 04 May 2023 at 11:11 AM` | 230⭐ | `Other`
- [KRTirtho/spotube](https://github.com/KRTirtho/spotube) - 🎧 Open source music streaming app! Available for both desktop & mobile!.
   - Updated on `Fri 05 June 2026 at 05:10 AM` | 46884⭐ | `Other`
- [go-vikunja/app](https://github.com/go-vikunja/app) - Cross-Platform app for Vikunja.
   - Updated on `Thu 04 June 2026 at 08:11 PM` | 556⭐ | `MIT License`
- [robertodoering/harpy](https://github.com/robertodoering/harpy) - a Twitter app built with Flutter 🦅.
   - Updated on `Thu 01 August 2024 at 11:32 PM` | 2073⭐ | `GNU General Public License v3.0`
- [austinried/subtracks](https://github.com/austinried/subtracks) - A music streaming app for Subsonic-compatible servers.
   - Updated on `Fri 02 January 2026 at 01:27 AM` | 787⭐ | `GNU General Public License v3.0`
- [jagandeepbrar/lunasea](https://github.com/jagandeepbrar/lunasea) - Self-hosted software controller built using Flutter.
   - Updated on `Thu 03 April 2025 at 01:54 AM` | 1468⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Dockerfile 

- [mikeroyal/Self-Hosting-Guide](https://github.com/mikeroyal/Self-Hosting-Guide) - Self-Hosting Guide. Learn all about  locally hosting (on premises & private web servers) and managing software applications by yourself or your organization. Including Cloud, LLMs, WireGuard, Automati.
   - Updated on `Fri 27 June 2025 at 01:51 AM` | 20207⭐ | `❌`
- [erikbrgr/scheduler](https://github.com/erikbrgr/scheduler) - Repository for the article Cron + Docker = The Easiest Job Scheduler You'll Ever Create on Medium.
   - Updated on `Mon 24 April 2023 at 09:02 AM` | 18⭐ | `❌`
- [nfrastack/container-db-backup](https://github.com/nfrastack/container-db-backup) - Backup multiple database types on a scheduled basis with many customizable options.
   - Updated on `Fri 13 March 2026 at 03:36 PM` | 1502⭐ | `MIT License`
- [Wonderfall/docker-mastodon](https://github.com/Wonderfall/docker-mastodon) - All-in-one unofficial Mastodon Docker image, if you want that.
   - Updated on `Mon 01 June 2026 at 05:35 AM` | 55⭐ | `MIT License`
- [linuxserver/docker-swag](https://github.com/linuxserver/docker-swag) - Nginx webserver and reverse proxy with php support and a built-in Certbot (Let's Encrypt) client. It also contains fail2ban for intrusion prevention.
   - Updated on `Sat 06 June 2026 at 06:45 AM` | 3677⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Elixir 

- [plausible/analytics](https://github.com/plausible/analytics) - Open source, privacy-first web analytics. Lightweight, cookie-free Google Analytics alternative. Self-hosted or cloud.
   - Updated on `Tue 09 June 2026 at 02:22 PM` | 26991⭐ | `GNU Affero General Public License v3.0`
- [zoonk/uneebee](https://github.com/zoonk/uneebee) - Platform for creating interactive courses.
   - Updated on `Thu 30 January 2025 at 05:03 PM` | 1350⭐ | `GNU Affero General Public License v3.0`
- [rauversion/rauversion-phx](https://github.com/rauversion/rauversion-phx) - Self hosted music industries built on Elixir Phoenix.
   - Updated on `Tue 09 January 2024 at 12:41 AM` | 360⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Elm 

- [eikek/sharry](https://github.com/eikek/sharry) - Sharry is a self-hosted file sharing web application.
   - Updated on `Thu 04 June 2026 at 08:11 PM` | 1304⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Fluent 

- [qarmin/czkawka](https://github.com/qarmin/czkawka) - Multi functional app to find duplicates, empty folders, similar images etc.
   - Updated on `Sun 07 June 2026 at 07:57 PM` | 31405⭐ | `Other`
 
**[`^        back to top        ^`](#)**

## Go 

- [nicholas-fedor/watchtower](https://github.com/nicholas-fedor/watchtower) - Automate Docker container image updates.
   - Updated on `Tue 09 June 2026 at 09:57 PM` | 3779⭐ | `Apache License 2.0`
- [igarridot/Pentaract](https://github.com/igarridot/Pentaract) - Unlimited file cloud storage based on using Telegram API.
   - Updated on `Sun 07 June 2026 at 10:43 AM` | 3⭐ | `Apache License 2.0`
- [bc183/otun](https://github.com/bc183/otun) - 🚀 Lightweight, self-hosted ngrok alternative. Expose localhost to the internet with one command.
   - Updated on `Sun 19 April 2026 at 03:56 PM` | 8⭐ | `MIT License`
- [swissmakers/fail2ban-ui](https://github.com/swissmakers/fail2ban-ui) - Fail2Ban UI is a swissmade web interface for operating Fail2Ban across one or more Linux hosts. It provides a central place to review bans, search and unban IPs, manage jails and filters, and receive .
   - Updated on `Mon 25 May 2026 at 09:19 AM` | 291⭐ | `GNU General Public License v3.0`
- [dockers-x/cfui](https://github.com/dockers-x/cfui) - cloudflared webUI.
   - Updated on `Mon 08 June 2026 at 10:25 AM` | 27⭐ | `MIT License`
- [juanfont/headscale](https://github.com/juanfont/headscale) - An open source, self-hosted implementation of the Tailscale control server.
   - Updated on `Tue 09 June 2026 at 01:21 PM` | 39860⭐ | `BSD 3-Clause "New" or "Revised" License`
- [minio/minio](https://github.com/minio/minio) - MinIO is a high-performance, S3 compatible object store, open sourced under GNU AGPLv3 license.
   - Updated on `Fri 24 April 2026 at 05:54 PM` | 61249⭐ | `GNU Affero General Public License v3.0`
- [eduardolat/pgbackweb](https://github.com/eduardolat/pgbackweb) - 🐘 Effortless PostgreSQL backups with a user-friendly web interface! 🌐💾.
   - Updated on `Fri 21 November 2025 at 11:53 PM` | 2585⭐ | `GNU Affero General Public License v3.0`
- [abhinavxd/libredesk](https://github.com/abhinavxd/libredesk) - Open source, self-hosted omnichannel customer support desk. Live chat, email, and more in a single binary.
   - Updated on `Mon 08 June 2026 at 07:42 PM` | 2561⭐ | `GNU Affero General Public License v3.0`
- [getarcaneapp/arcane](https://github.com/getarcaneapp/arcane) - Modern Docker Management, Designed for Everyone.
   - Updated on `Tue 09 June 2026 at 11:55 PM` | 5701⭐ | `BSD 3-Clause "New" or "Revised" License`
- [cloudwego/eino](https://github.com/cloudwego/eino) - The ultimate LLM/AI application development framework in Go.
   - Updated on `Tue 09 June 2026 at 09:29 AM` | 11728⭐ | `Apache License 2.0`
- [Billionmail/BillionMail](https://github.com/Billionmail/BillionMail) - BillionMail gives you open-source MailServer, NewsLetter,  Email Marketing — fully self-hosted, dev-friendly, and free from monthly fees. Join the discord: https://discord.gg/asfXzBUhZr.
   - Updated on `Mon 20 April 2026 at 02:35 AM` | 15110⭐ | `GNU Affero General Public License v3.0`
- [danielmiessler/Fabric](https://github.com/danielmiessler/Fabric) - Fabric is an open-source framework for augmenting humans using AI. It provides a modular system for solving specific problems using a crowdsourced set of AI prompts that can be used anywhere.
   - Updated on `Tue 09 June 2026 at 09:57 PM` | 42226⭐ | `MIT License`
- [aldinokemal/go-whatsapp-web-multidevice](https://github.com/aldinokemal/go-whatsapp-web-multidevice) - GOWA - WhatsApp REST API with support for UI, Multi Account, Webhooks, and MCP, and Chatwoot. Built with Golang for efficient memory use.
   - Updated on `Tue 09 June 2026 at 04:20 PM` | 4040⭐ | `MIT License`
- [php/frankenphp](https://github.com/php/frankenphp) - 🧟 The modern PHP app server.
   - Updated on `Mon 08 June 2026 at 11:34 AM` | 11140⭐ | `MIT License`
- [gtsteffaniak/filebrowser](https://github.com/gtsteffaniak/filebrowser) - 📂 Web File Browser.
   - Updated on `Wed 10 June 2026 at 12:10 AM` | 7228⭐ | `Apache License 2.0`
- [WireGuard/wireguard-windows](https://github.com/WireGuard/wireguard-windows) - Download WireGuard for Windows at https://www.wireguard.com/install . This repo is a mirror only. Official repository is at https://git.zx2c4.com/wireguard-windows.
   - Updated on `Fri 29 May 2026 at 11:38 PM` | 2616⭐ | `MIT License`
- [canonical/chisel](https://github.com/canonical/chisel) - .
   - Updated on `Mon 01 June 2026 at 06:37 PM` | 409⭐ | `GNU Affero General Public License v3.0`
- [sweepies/tok-dl](https://github.com/sweepies/tok-dl) - Tok-DL: A TikTok Downloader that actually works.
   - Updated on `Tue 29 April 2025 at 01:48 AM` | 60⭐ | `GNU General Public License v3.0`
- [aceberg/WatchYourPorts](https://github.com/aceberg/WatchYourPorts) - Open ports inventory for local servers. Exports data to InfluxDB2/Grafana.
   - Updated on `Fri 19 September 2025 at 08:27 AM` | 202⭐ | `MIT License`
- [yorukot/superfile](https://github.com/yorukot/superfile) - Pretty fancy and modern terminal file manager.
   - Updated on `Mon 08 June 2026 at 05:00 PM` | 17472⭐ | `MIT License`
- [trufflesecurity/trufflehog](https://github.com/trufflesecurity/trufflehog) - Find, verify, and analyze leaked credentials.
   - Updated on `Tue 09 June 2026 at 11:32 PM` | 26705⭐ | `GNU Affero General Public License v3.0`
- [sbondCo/Watcharr](https://github.com/sbondCo/Watcharr) - Open source, self-hostable watched list for all your content (movies, tv series, anime, games) with user authentication, modern and clean UI and a very simple setup.
   - Updated on `Thu 19 March 2026 at 01:31 AM` | 1386⭐ | `GNU General Public License v3.0`
- [dagger/dagger](https://github.com/dagger/dagger) - Automation engine to build, test and ship any codebase. Runs locally, in CI, or directly in the cloud.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 15920⭐ | `Apache License 2.0`
- [mudler/LocalAI](https://github.com/mudler/LocalAI) - LocalAI is the open-source AI engine. Run any model - LLMs, vision, voice, image, video - on any hardware. No GPU required.
   - Updated on `Tue 09 June 2026 at 11:16 PM` | 46758⭐ | `MIT License`
- [src-d/hercules](https://github.com/src-d/hercules) - Gaining advanced insights from Git repository history.
   - Updated on `Tue 07 February 2023 at 08:25 AM` | 2795⭐ | `Other`
- [henrygd/beszel](https://github.com/henrygd/beszel) - Lightweight server monitoring with historical data, docker stats, and alerts.
   - Updated on `Mon 08 June 2026 at 03:20 PM` | 22535⭐ | `MIT License`
- [hectorm/cetusguard](https://github.com/hectorm/cetusguard) - CetusGuard is a tool that protects the Docker daemon socket by filtering calls to its API endpoints.
   - Updated on `Wed 01 April 2026 at 01:49 PM` | 87⭐ | `MIT License`
- [samber/lo](https://github.com/samber/lo) - 💥  A Lodash-style Go library based on Go 1.18+ Generics (map, filter, contains, find...).
   - Updated on `Tue 02 June 2026 at 07:06 AM` | 21319⭐ | `MIT License`
- [rramiachraf/dumb](https://github.com/rramiachraf/dumb) - Private alternative front-end for Genius.
   - Updated on `Wed 01 April 2026 at 04:17 AM` | 399⭐ | `MIT License`
- [rancher/rancher](https://github.com/rancher/rancher) - Complete container management platform.
   - Updated on `Tue 09 June 2026 at 07:06 PM` | 25653⭐ | `Apache License 2.0`
- [Xpl0itU/passiveMachine](https://github.com/Xpl0itU/passiveMachine) - An adaptable, self-updating Docker stack designed for multiple platforms, utilizing Docker to manage a suite of passive income applications such as Honeygain, EarnApp, PawnsApp, PacketStream, Peer2Pro.
   - Updated on `Thu 06 June 2024 at 01:42 PM` | 60⭐ | `GNU General Public License v3.0`
- [ddworken/hishtory](https://github.com/ddworken/hishtory) - Your shell history: synced, queryable, and in context.
   - Updated on `Wed 18 March 2026 at 11:18 PM` | 3087⭐ | `MIT License`
- [openziti/zrok](https://github.com/openziti/zrok) - Secure internet sharing made simple.
   - Updated on `Wed 03 June 2026 at 02:59 PM` | 4462⭐ | `Apache License 2.0`
- [go-shiori/shiori](https://github.com/go-shiori/shiori) - Simple bookmark manager built with Go.
   - Updated on `Sun 01 February 2026 at 10:14 AM` | 11492⭐ | `MIT License`
- [glanceapp/glance](https://github.com/glanceapp/glance) - A self-hosted dashboard that puts all your feeds in one place.
   - Updated on `Sat 30 May 2026 at 08:36 PM` | 34943⭐ | `GNU Affero General Public License v3.0`
- [muesli/duf](https://github.com/muesli/duf) - Disk Usage/Free Utility - a better 'df' alternative.
   - Updated on `Tue 13 January 2026 at 11:07 AM` | 15234⭐ | `Other`
- [coreybutler/nvm-windows](https://github.com/coreybutler/nvm-windows) - A node.js version management utility for Windows. Ironically written in Go.
   - Updated on `Fri 17 April 2026 at 05:58 PM` | 46734⭐ | `MIT License`
- [miniscruff/changie](https://github.com/miniscruff/changie) - Automated changelog tool for preparing releases with lots of customization options.
   - Updated on `Sat 30 May 2026 at 01:03 AM` | 882⭐ | `MIT License`
- [yunginnanet/HellPot](https://github.com/yunginnanet/HellPot) - HellPot is a cross-platform portal to endless suffering meant to punish unruly HTTP bots.
   - Updated on `Fri 19 December 2025 at 10:02 PM` | 1112⭐ | `MIT License`
- [amalshaji/portr](https://github.com/amalshaji/portr) - Expose local http, tcp or websocket connections to the public internet.
   - Updated on `Mon 08 June 2026 at 02:07 PM` | 3136⭐ | `GNU Affero General Public License v3.0`
- [goharbor/harbor](https://github.com/goharbor/harbor) - An open source trusted cloud native registry project that stores, signs, and scans content.
   - Updated on `Tue 09 June 2026 at 04:01 PM` | 28666⭐ | `Apache License 2.0`
- [GopeedLab/gopeed](https://github.com/GopeedLab/gopeed) - A fast, modern download manager for HTTP, BitTorrent, Magnet, and ed2k. Cross-platform, built with Golang and Flutter.
   - Updated on `Thu 07 May 2026 at 08:57 AM` | 24821⭐ | `GNU General Public License v3.0`
- [plandex-ai/plandex](https://github.com/plandex-ai/plandex) - Open source AI coding agent. Designed for large projects and real world tasks.
   - Updated on `Fri 03 October 2025 at 09:49 PM` | 15448⭐ | `MIT License`
- [garethgeorge/backrest](https://github.com/garethgeorge/backrest) - Backrest is a web UI and orchestrator for restic backup.
   - Updated on `Tue 12 May 2026 at 12:39 AM` | 6660⭐ | `GNU General Public License v3.0`
- [hatchet-dev/hatchet](https://github.com/hatchet-dev/hatchet) - 🪓 An orchestration engine for background tasks, AI agents, and durable workflows.
   - Updated on `Tue 09 June 2026 at 09:16 PM` | 7314⭐ | `MIT License`
- [yusufcanb/tlm](https://github.com/yusufcanb/tlm) - Local CLI Copilot, powered by Ollama. 💻🦙.
   - Updated on `Sun 22 February 2026 at 08:05 PM` | 1482⭐ | `Apache License 2.0`
- [bruin-data/ingestr](https://github.com/bruin-data/ingestr) - ingestr is a CLI tool to copy data between any databases with a single command seamlessly.
   - Updated on `Tue 09 June 2026 at 05:25 PM` | 3706⭐ | `Other`
- [tgdrive/teldrive](https://github.com/tgdrive/teldrive) - Teldrive.
   - Updated on `Fri 05 June 2026 at 05:39 AM` | 2927⭐ | `MIT License`
- [sundowndev/phoneinfoga](https://github.com/sundowndev/phoneinfoga) - Information gathering framework for phone numbers.
   - Updated on `Tue 06 January 2026 at 11:08 AM` | 16601⭐ | `GNU General Public License v3.0`
- [bytebase/bytebase](https://github.com/bytebase/bytebase) - World's most advanced database DevSecOps solution for Developer, Security, DBA and Platform Engineering teams. The GitHub/GitLab for database DevSecOps.
   - Updated on `Tue 09 June 2026 at 10:48 AM` | 14124⭐ | `Other`
- [go-gost/gost](https://github.com/go-gost/gost) - GO Simple Tunnel - a simple tunnel written in golang.
   - Updated on `Sat 06 June 2026 at 03:26 PM` | 6929⭐ | `MIT License`
- [apernet/hysteria](https://github.com/apernet/hysteria) - Hysteria is a powerful, lightning fast and censorship resistant proxy.
   - Updated on `Tue 09 June 2026 at 07:48 PM` | 21730⭐ | `MIT License`
- [everywall/ladder](https://github.com/everywall/ladder) - Selfhosted alternative to 12ft.io. and 1ft.io. Proxy to remove CORS headers and modify HTML.
   - Updated on `Thu 14 May 2026 at 08:23 PM` | 8008⭐ | `GNU General Public License v3.0`
- [ollama/ollama](https://github.com/ollama/ollama) - Get up and running with Kimi-K2.6, GLM-5.1, MiniMax, DeepSeek, gpt-oss, Qwen, Gemma and other models.
   - Updated on `Tue 09 June 2026 at 10:04 PM` | 173713⭐ | `MIT License`
- [bitmagnet-io/bitmagnet](https://github.com/bitmagnet-io/bitmagnet) - A self-hosted BitTorrent indexer, DHT crawler, content classifier and torrent search engine with web UI, GraphQL API and Servarr stack integration.
   - Updated on `Thu 21 May 2026 at 12:13 PM` | 4088⭐ | `MIT License`
- [httpjamesm/AnonymousOverflow](https://github.com/httpjamesm/AnonymousOverflow) - View StackOverflow in privacy and without the clutter.
   - Updated on `Wed 03 December 2025 at 01:14 AM` | 373⭐ | `Mozilla Public License 2.0`
- [loft-sh/devpod](https://github.com/loft-sh/devpod) - Codespaces but open-source, client-only and unopinionated: Works with any IDE and lets you use any cloud, kubernetes or just localhost docker.
   - Updated on `Fri 14 November 2025 at 06:33 AM` | 14952⭐ | `Mozilla Public License 2.0`
- [semaphoreui/semaphore](https://github.com/semaphoreui/semaphore) - Modern UI and powerful API for Ansible, Terraform/OpenTofu/Terragrunt, PowerShell and other DevOps tools.
   - Updated on `Wed 10 June 2026 at 12:09 AM` | 13728⭐ | `MIT License`
- [mickael-kerjean/filestash](https://github.com/mickael-kerjean/filestash) - :file_folder: File Management Platform / Universal Data Access Layer (without FUSE).
   - Updated on `Mon 08 June 2026 at 12:25 PM` | 14293⭐ | `GNU Affero General Public License v3.0`
- [safing/spn](https://github.com/safing/spn) - Safing Privacy Network.
   - Updated on `Fri 29 March 2024 at 10:08 AM` | 258⭐ | `GNU Affero General Public License v3.0`
- [anchore/grype](https://github.com/anchore/grype) - A vulnerability scanner for container images and filesystems.
   - Updated on `Tue 09 June 2026 at 05:39 PM` | 12361⭐ | `Apache License 2.0`
- [thomiceli/opengist](https://github.com/thomiceli/opengist) - Self-hosted pastebin powered by Git, open-source alternative to Github Gist.
   - Updated on `Tue 09 June 2026 at 07:26 PM` | 3201⭐ | `GNU Affero General Public License v3.0`
- [Hound-Media-Server/hound](https://github.com/Hound-Media-Server/hound) - All-in-one Media Server—Find, watch, track, and manage your media.
   - Updated on `Tue 09 June 2026 at 04:59 AM` | 437⭐ | `GNU Affero General Public License v3.0`
- [seriousm4x/UpSnap](https://github.com/seriousm4x/UpSnap) - A simple wake on lan web app written with SvelteKit, Go and PocketBase.
   - Updated on `Mon 01 June 2026 at 08:03 PM` | 5777⭐ | `MIT License`
- [aetaric/checkrr](https://github.com/aetaric/checkrr) - Checkrr Scans your library files for corrupt media and optionally replaces the files via sonarr and radarr.
   - Updated on `Wed 03 June 2026 at 09:11 PM` | 552⭐ | `MIT License`
- [dweymouth/supersonic](https://github.com/dweymouth/supersonic) - A lightweight and full-featured cross-platform desktop client for self-hosted music servers.
   - Updated on `Tue 09 June 2026 at 10:41 PM` | 2198⭐ | `GNU General Public License v3.0`
- [cupcakearmy/autorestic](https://github.com/cupcakearmy/autorestic) - Config driven, easy backup cli for restic.
   - Updated on `Sun 05 April 2026 at 10:09 PM` | 1832⭐ | `Apache License 2.0`
- [vmorganp/Lazytainer](https://github.com/vmorganp/Lazytainer) - Docker container lazy loading.
   - Updated on `Mon 10 November 2025 at 11:06 PM` | 748⭐ | `MIT License`
- [schollz/croc](https://github.com/schollz/croc) - Easily and securely send things from one computer to another :crocodile: :package:.
   - Updated on `Tue 09 June 2026 at 08:32 AM` | 35231⭐ | `MIT License`
- [howeyc/crocgui](https://github.com/howeyc/crocgui) - GUI for croc.
   - Updated on `Sun 15 March 2026 at 08:03 PM` | 304⭐ | `ISC License`
- [aceberg/git-syr](https://github.com/aceberg/git-syr) - Sync Your Repos - pull or push your git repos regularly. For dotfiles backups or note taking in git repo.
   - Updated on `Fri 20 September 2024 at 11:34 AM` | 25⭐ | `MIT License`
- [nektos/act](https://github.com/nektos/act) - Run your GitHub Actions locally 🚀.
   - Updated on `Mon 01 June 2026 at 03:30 AM` | 70815⭐ | `MIT License`
- [slimtoolkit/slim](https://github.com/slimtoolkit/slim) - Slim(toolkit): Don't change anything in your container image and minify it by up to 30x (and for compiled languages even more) making it secure too! (free and open source).
   - Updated on `Thu 28 May 2026 at 04:47 PM` | 23301⭐ | `Apache License 2.0`
- [usememos/memos](https://github.com/usememos/memos) - Open-source, self-hosted note-taking tool built for quick capture. Markdown-native, lightweight, and fully yours.
   - Updated on `Tue 09 June 2026 at 03:55 PM` | 60543⭐ | `MIT License`
- [sablierapp/sablier](https://github.com/sablierapp/sablier) - Start your containers on demand, shut them down automatically when there's no activity. Docker, Docker Swarm Mode, Podman, Kubernetes and Proxmox LXC compatible.
   - Updated on `Tue 09 June 2026 at 06:53 PM` | 2760⭐ | `GNU Affero General Public License v3.0`
- [sorenisanerd/gotty](https://github.com/sorenisanerd/gotty) - Share your terminal as a web application.
   - Updated on `Sat 06 June 2026 at 09:22 AM` | 2502⭐ | `MIT License`
- [estesp/manifest-tool](https://github.com/estesp/manifest-tool) - Command line tool to create and query container image manifest list/indexes.
   - Updated on `Mon 08 June 2026 at 04:22 PM` | 837⭐ | `Apache License 2.0`
- [google/cadvisor](https://github.com/google/cadvisor) - Analyzes resource usage and performance characteristics of running containers.
   - Updated on `Tue 02 June 2026 at 10:10 AM` | 19183⭐ | `Other`
- [aceberg/WatchYourLAN](https://github.com/aceberg/WatchYourLAN) - Lightweight network IP scanner written in Go. With notifications, history, export to Grafana.
   - Updated on `Wed 10 September 2025 at 12:08 PM` | 7028⭐ | `MIT License`
- [rule110-io/surge](https://github.com/rule110-io/surge) - Surge is a p2p filesharing app designed to utilize blockchain technologies to enable 100% anonymous file transfers. Surge is end-to-end encrypted, decentralized and open source.
   - Updated on `Thu 15 June 2023 at 01:35 PM` | 462⭐ | `Apache License 2.0`
- [achhabra2/riftshare](https://github.com/achhabra2/riftshare) - Easy, Secure, Free file sharing.
   - Updated on `Fri 17 June 2022 at 03:10 PM` | 141⭐ | `GNU General Public License v3.0`
- [enchant97/hasty-paste](https://github.com/enchant97/hasty-paste) - Paste it all, with haste.
   - Updated on `Mon 04 May 2026 at 12:56 PM` | 256⭐ | `GNU Affero General Public License v3.0`
- [autobrr/autobrr](https://github.com/autobrr/autobrr) - Modern, easy to use download automation for torrents and usenet.
   - Updated on `Mon 08 June 2026 at 01:04 PM` | 2813⭐ | `GNU General Public License v2.0`
- [casdoor/casdoor](https://github.com/casdoor/casdoor) - An open-source Agent-first Identity and Access Management (IAM) /LLM MCP & agent gateway and auth server with web UI supporting OpenClaw, MCP, OAuth, OIDC, SAML, CAS, LDAP, SCIM, WebAuthn, TOTP, MFA, .
   - Updated on `Tue 09 June 2026 at 01:43 PM` | 13752⭐ | `Apache License 2.0`
- [docker/compose](https://github.com/docker/compose) - Define and run multi-container applications with Docker.
   - Updated on `Tue 09 June 2026 at 03:57 PM` | 37491⭐ | `Apache License 2.0`
- [wailsapp/wails](https://github.com/wailsapp/wails) - Create beautiful applications using Go.
   - Updated on `Tue 09 June 2026 at 05:37 PM` | 34710⭐ | `MIT License`
- [Forceu/Gokapi](https://github.com/Forceu/Gokapi) - Lightweight selfhosted Firefox Send alternative without public upload. AWS S3 supported.
   - Updated on `Wed 20 May 2026 at 02:23 PM` | 2760⭐ | `GNU Affero General Public License v3.0`
- [mprimi/nasefa](https://github.com/mprimi/nasefa) - Send and receive files using NATS.
   - Updated on `Wed 29 June 2022 at 10:48 PM` | 48⭐ | `MIT License`
- [OliveTin/OliveTin](https://github.com/OliveTin/OliveTin) - OliveTin gives safe and simple access to predefined shell commands from a web interface.
   - Updated on `Sat 06 June 2026 at 04:59 PM` | 3632⭐ | `GNU Affero General Public License v3.0`
- [mcuadros/ofelia](https://github.com/mcuadros/ofelia) - A docker job scheduler (aka. crontab for docker).
   - Updated on `Tue 09 June 2026 at 08:55 PM` | 3885⭐ | `MIT License`
- [hrfee/jfa-go](https://github.com/hrfee/jfa-go) - a bit-of-everything user management app for Jellyfin.
   - Updated on `Sun 15 March 2026 at 01:48 PM` | 1150⭐ | `Other`
- [release-argus/Argus](https://github.com/release-argus/Argus) - Argus is a lightweight monitor to notify of new software releases via Gotify/Slack/other messages and/or WebHooks.
   - Updated on `Tue 09 June 2026 at 02:05 PM` | 601⭐ | `Apache License 2.0`
- [IceWhaleTech/CasaOS](https://github.com/IceWhaleTech/CasaOS) - CasaOS - A simple, easy-to-use, elegant open-source Personal Cloud system.
   - Updated on `Wed 06 August 2025 at 08:54 AM` | 34047⭐ | `Apache License 2.0`
- [fatedier/frp](https://github.com/fatedier/frp) - A fast reverse proxy to help you expose a local server behind a NAT or firewall to the internet.
   - Updated on `Wed 03 June 2026 at 06:08 AM` | 107212⭐ | `Apache License 2.0`
- [owncast/owncast](https://github.com/owncast/owncast) - Take control over your live stream video by running it yourself.  Streaming + chat out of the box.
   - Updated on `Tue 09 June 2026 at 11:19 PM` | 11323⭐ | `MIT License`
- [crowdsecurity/crowdsec](https://github.com/crowdsecurity/crowdsec) - CrowdSec - the open-source and participative security solution offering crowdsourced protection against malicious IPs and access to the most advanced real-world CTI.
   - Updated on `Tue 09 June 2026 at 04:45 PM` | 13758⭐ | `MIT License`
- [syncthing/syncthing](https://github.com/syncthing/syncthing) - Open Source Continuous File Synchronization.
   - Updated on `Tue 09 June 2026 at 01:36 AM` | 85177⭐ | `Mozilla Public License 2.0`
- [amir20/dozzle](https://github.com/amir20/dozzle) - Realtime log viewer for containers.  Supports Docker, Swarm and K8s.
   - Updated on `Mon 08 June 2026 at 02:20 AM` | 13227⭐ | `MIT License`
- [filebrowser/filebrowser](https://github.com/filebrowser/filebrowser) - 📂 Web File Browser.
   - Updated on `Tue 09 June 2026 at 10:16 PM` | 35021⭐ | `Apache License 2.0`
- [gogdl-ng/gogdl-ng](https://github.com/gogdl-ng/gogdl-ng) - Self-hostable application to download files in a folder from Google Drive.
   - Updated on `Tue 01 August 2023 at 04:38 PM` | 75⭐ | `MIT License`
- [mishankov/web-tail](https://github.com/mishankov/web-tail) - 📖 Web based log monitoring tool.
   - Updated on `Thu 04 June 2026 at 05:52 AM` | 20⭐ | `MIT License`
- [cshum/imagor](https://github.com/cshum/imagor) - Fast, secure image processing server and Go library, using libvips.
   - Updated on `Fri 05 June 2026 at 10:12 AM` | 3964⭐ | `Apache License 2.0`
- [photoprism/photoprism](https://github.com/photoprism/photoprism) - AI-Powered Photos App for the Decentralized Web 🌈💎✨.
   - Updated on `Wed 10 June 2026 at 12:22 AM` | 39773⭐ | `Other`
- [dogbutcat/gclone](https://github.com/dogbutcat/gclone) - fork from https://github.com/donwa/gclone.
   - Updated on `Sun 05 April 2026 at 03:39 PM` | 195⭐ | `❌`
- [kopia/kopia](https://github.com/kopia/kopia) - Cross-platform backup tool for Windows, macOS & Linux with fast, incremental backups, client-side end-to-end encryption, compression and data deduplication. CLI and GUI included.
   - Updated on `Tue 09 June 2026 at 11:43 PM` | 13405⭐ | `Apache License 2.0`
- [AdguardTeam/AdGuardHome](https://github.com/AdguardTeam/AdGuardHome) - Network-wide ads & trackers blocking DNS server.
   - Updated on `Tue 09 June 2026 at 05:48 PM` | 34757⭐ | `GNU General Public License v3.0`
- [gotify/server](https://github.com/gotify/server) - A simple server for sending and receiving messages in real-time per WebSocket. (Includes a sleek web-ui).
   - Updated on `Mon 08 June 2026 at 10:06 PM` | 15105⭐ | `Other`
- [lightglitch/seekerr](https://github.com/lightglitch/seekerr) - Tool to add new movies to Radarr based on RSS, IMDB and Trakt lists filtering by ratings.
   - Updated on `Mon 17 April 2023 at 12:58 AM` | 65⭐ | `MIT License`
- [knadh/listmonk](https://github.com/knadh/listmonk) - High performance, self-hosted, newsletter and mailing list manager with a modern dashboard. Single binary app.
   - Updated on `Thu 04 June 2026 at 04:39 AM` | 21445⭐ | `GNU Affero General Public License v3.0`
- [binwiederhier/ntfy](https://github.com/binwiederhier/ntfy) - Send push notifications to your phone or desktop using PUT/POST.
   - Updated on `Tue 09 June 2026 at 09:44 PM` | 30679⭐ | `Apache License 2.0`
- [influxdata/telegraf](https://github.com/influxdata/telegraf) - Agent for collecting, processing, aggregating, and writing metrics, logs, and other arbitrary data.
   - Updated on `Tue 09 June 2026 at 02:41 PM` | 17610⭐ | `MIT License`
- [harness/harness](https://github.com/harness/harness) - Harness Open Source is an end-to-end developer platform with Source Control Management, CI/CD Pipelines, Hosted Developer Environments, and Artifact Registries.
   - Updated on `Tue 09 June 2026 at 05:36 PM` | 36622⭐ | `Apache License 2.0`
- [qdm12/ddns-updater](https://github.com/qdm12/ddns-updater) - Container to update DNS records periodically with WebUI for many DNS providers.
   - Updated on `Tue 09 June 2026 at 10:32 AM` | 3097⭐ | `MIT License`
- [horahoradev/horahora](https://github.com/horahoradev/horahora) - Multi-user video download manager supporting hundreds of sites.
   - Updated on `Tue 14 November 2023 at 07:56 PM` | 846⭐ | `MIT License`
- [passteque/gluetun](https://github.com/passteque/gluetun) - VPN client in a thin Docker container for multiple VPN providers, written in Go, and using OpenVPN or Wireguard, DNS over TLS, with a few proxy servers built-in.
   - Updated on `Tue 09 June 2026 at 09:11 PM` | 14504⭐ | `MIT License`
- [restic/restic](https://github.com/restic/restic) - Fast, secure, efficient backup program.
   - Updated on `Tue 09 June 2026 at 08:43 PM` | 33941⭐ | `BSD 2-Clause "Simplified" License`
- [woodpecker-ci/woodpecker](https://github.com/woodpecker-ci/woodpecker) - Woodpecker is a simple, yet powerful CI/CD engine with great extensibility.
   - Updated on `Tue 09 June 2026 at 07:04 PM` | 7228⭐ | `Apache License 2.0`
- [varbhat/exatorrent](https://github.com/varbhat/exatorrent) - 🧲 Easy to Use Torrent Client. Can be hosted in Cloud. Files can be streamed in Browser/Media Player.
   - Updated on `Mon 12 May 2025 at 11:32 PM` | 1979⭐ | `GNU General Public License v3.0`
- [go-gitea/gitea](https://github.com/go-gitea/gitea) - Git with a cup of tea! Painless self-hosted all-in-one software development service, including Git hosting, code review, team collaboration, package registry and CI/CD.
   - Updated on `Tue 09 June 2026 at 05:34 PM` | 56206⭐ | `MIT License`
- [statping-ng/statping-ng](https://github.com/statping-ng/statping-ng) - An updated drop-in for statping. A Status Page for monitoring your websites and applications with beautiful graphs, analytics, and plugins. Run on any type of environment.
   - Updated on `Wed 04 June 2025 at 08:21 PM` | 1978⭐ | `GNU General Public License v3.0`
- [crazy-max/diun](https://github.com/crazy-max/diun) - Receive notifications when an image is updated on a Docker registry.
   - Updated on `Mon 08 June 2026 at 01:04 PM` | 4702⭐ | `MIT License`
- [AnalogJ/scrutiny](https://github.com/AnalogJ/scrutiny) - Hard Drive S.M.A.R.T Monitoring, Historical Trends & Real World Failure Thresholds.
   - Updated on `Mon 01 June 2026 at 07:30 PM` | 7866⭐ | `MIT License`
- [rh12503/triangula](https://github.com/rh12503/triangula) - Generate high-quality triangulated and polygonal art from images.
   - Updated on `Sat 21 March 2026 at 12:05 AM` | 3877⭐ | `MIT License`
- [Unpackerr/unpackerr](https://github.com/Unpackerr/unpackerr) - Extracts downloads for Radarr, Sonarr, Lidarr, Readarr, and/or a Watch folder - Deletes extracted files after import.
   - Updated on `Tue 09 June 2026 at 11:53 PM` | 1412⭐ | `MIT License`
- [Narasimha1997/fake-sms](https://github.com/Narasimha1997/fake-sms) - A simple command line tool using which you can skip phone number based SMS verification by using a temporary phone number that acts like a proxy.
   - Updated on `Tue 01 August 2023 at 03:34 PM` | 2786⭐ | `GNU General Public License v2.0`
- [authelia/authelia](https://github.com/authelia/authelia) - The Single Sign-On Multi-Factor portal for web apps, now OpenID Certified™.
   - Updated on `Tue 09 June 2026 at 06:01 PM` | 28013⭐ | `Apache License 2.0`
- [navidrome/navidrome](https://github.com/navidrome/navidrome) - 🎧 Your Personal Streaming Service.
   - Updated on `Tue 09 June 2026 at 11:31 PM` | 21584⭐ | `GNU General Public License v3.0`
- [stashapp/stash](https://github.com/stashapp/stash) - An organizer for your porn, written in Go.  Documentation:  https://docs.stashapp.cc.
   - Updated on `Tue 09 June 2026 at 12:01 AM` | 12458⭐ | `GNU Affero General Public License v3.0`
- [azimjohn/jprq](https://github.com/azimjohn/jprq) - join public router. quickly.
   - Updated on `Tue 09 June 2026 at 02:41 PM` | 1514⭐ | `❌`
- [pyed/rtelegram](https://github.com/pyed/rtelegram) - Control rTorrent via a Telegram bot.
   - Updated on `Sat 14 March 2026 at 07:05 AM` | 47⭐ | `Apache License 2.0`
- [gohugoio/hugo](https://github.com/gohugoio/hugo) - The world’s fastest framework for building websites.
   - Updated on `Tue 09 June 2026 at 12:34 PM` | 88494⭐ | `Apache License 2.0`
- [prasmussen/gdrive](https://github.com/prasmussen/gdrive) - Google Drive CLI Client.
   - Updated on `Wed 19 April 2023 at 06:46 PM` | 8969⭐ | `MIT License`
- [Shpota/goxygen](https://github.com/Shpota/goxygen) - Generate a modern Web project with Go and Angular, React, or Vue in seconds 🎲.
   - Updated on `Wed 18 December 2024 at 09:02 PM` | 3596⭐ | `Apache License 2.0`
- [cli/cli](https://github.com/cli/cli) - GitHub’s official command line tool.
   - Updated on `Tue 09 June 2026 at 09:31 AM` | 44765⭐ | `MIT License`
- [dutchcoders/transfer.sh](https://github.com/dutchcoders/transfer.sh) - Easy and fast file sharing from the command-line.
   - Updated on `Thu 20 November 2025 at 12:30 AM` | 15847⭐ | `MIT License`
- [rclone/rclone](https://github.com/rclone/rclone) - "rsync for cloud storage" - Google Drive, S3, Dropbox, Backblaze B2, One Drive, Swift, Hubic, Wasabi, Google Cloud Storage, Azure Blob, Azure Files, Yandex Files.
   - Updated on `Tue 09 June 2026 at 02:29 PM` | 57844⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## HTML 

- [DigitalPlatDev/FreeDomain](https://github.com/DigitalPlatDev/FreeDomain) - DigitalPlat FreeDomain: Free Domain For Everyone.
   - Updated on `Fri 24 April 2026 at 01:50 PM` | 176030⭐ | `GNU Affero General Public License v3.0`
- [adityatelange/hugo-PaperMod](https://github.com/adityatelange/hugo-PaperMod) - A fast, clean, responsive Hugo theme.
   - Updated on `Sun 10 May 2026 at 05:30 AM` | 13640⭐ | `MIT License`
- [petersem/posterr](https://github.com/petersem/posterr) - A digital poster app for Plex, Sonarr, Radarr, and Readarr.
   - Updated on `Fri 20 March 2026 at 01:09 AM` | 632⭐ | `MIT License`
- [alohe/maker-tag](https://github.com/alohe/maker-tag) - Maker tag is a customizable HTML and CSS based widget that displays a text tag on a web page. The widget is designed to be added to any website with minimal configuration.
   - Updated on `Wed 12 July 2023 at 11:54 AM` | 8⭐ | `Apache License 2.0`
- [ripienaar/free-for-dev](https://github.com/ripienaar/free-for-dev) - A list of SaaS, PaaS and IaaS offerings that have free tiers of interest to devops and infradev.
   - Updated on `Tue 09 June 2026 at 06:16 PM` | 122985⭐ | `❌`
- [danmindru/responsive-html-email-signature](https://github.com/danmindru/responsive-html-email-signature) - ✨ Template generator for (responsive) emails & email signatures.
   - Updated on `Fri 29 May 2026 at 07:29 PM` | 877⭐ | `MIT License`
- [datarhei/restreamer](https://github.com/datarhei/restreamer) - The Restreamer is a complete streaming server solution for self-hosting. It has a visually appealing user interface and no ongoing license costs. Upload your live stream to YouTube, Twitch, Facebook, .
   - Updated on `Fri 22 May 2026 at 06:57 AM` | 5070⭐ | `Apache License 2.0`
- [docker/awesome-compose](https://github.com/docker/awesome-compose) - Awesome Docker Compose samples.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 45505⭐ | `Creative Commons Zero v1.0 Universal`
- [roshanconnor123/Gdutils_Tgbot](https://github.com/roshanconnor123/Gdutils_Tgbot) - Fully Functional Tgbot of Gdutils, English version of Iwestlins original repo.
   - Updated on `Sat 24 October 2020 at 09:52 PM` | 84⭐ | `❌`
- [Capevace/youtube-dl-ui](https://github.com/Capevace/youtube-dl-ui) - Basic web UI for youtube-dl. Download a URL in highest quality, choosing between video or audio-only.
   - Updated on `Sat 04 March 2023 at 08:44 PM` | 4⭐ | `MIT License`
- [developeranaz/qbittorrent-to-rclone-heroku](https://github.com/developeranaz/qbittorrent-to-rclone-heroku) - .
   - Updated on `Sun 27 November 2022 at 03:12 PM` | 100⭐ | `GNU General Public License v3.0`
- [bjoerns1983/Emby.Plugin.TelegramNotification](https://github.com/bjoerns1983/Emby.Plugin.TelegramNotification) - Emby Server Plugin for pushing Notifications to a Telegram Bot.
   - Updated on `Wed 01 October 2025 at 02:28 PM` | 125⭐ | `GNU General Public License v2.0`
- [maple3142/GDIndex](https://github.com/maple3142/GDIndex) - A Google Drive Index built with Vue Running on CloudFlare Workers.
   - Updated on `Mon 10 July 2023 at 06:28 PM` | 1877⭐ | `MIT License`
- [htr-tech/zphisher](https://github.com/htr-tech/zphisher) - An automated phishing tool with 30+ templates. This Tool is made for educational purpose only ! Author will not be responsible for any misuse of this toolkit !.
   - Updated on `Wed 21 August 2024 at 08:14 AM` | 16152⭐ | `GNU General Public License v3.0`
- [maple3142/aria2c-ariang](https://github.com/maple3142/aria2c-ariang) - .
   - Updated on `Sat 04 March 2023 at 05:19 AM` | 308⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Haskell 

- [PostgREST/postgrest](https://github.com/PostgREST/postgrest) - REST API for any Postgres database.
   - Updated on `Tue 09 June 2026 at 01:14 PM` | 27211⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Java 

- [opendataloader-project/opendataloader-pdf](https://github.com/opendataloader-project/opendataloader-pdf) - PDF Parser for AI-ready data. Automate PDF accessibility. Open-source.
   - Updated on `Tue 09 June 2026 at 08:02 PM` | 24267⭐ | `Apache License 2.0`
- [forrestguice/SuntimesWidget](https://github.com/forrestguice/SuntimesWidget) - Android app (and widget collection) that displays sunlight and moonlight times.
   - Updated on `Sun 07 June 2026 at 08:46 PM` | 432⭐ | `GNU General Public License v3.0`
- [tess1o/geopulse](https://github.com/tess1o/geopulse) - A self-hosted, privacy-first location timeline platform: an open-source alternative to Google Timeline with automatic trip detection, Immich integration, and rich analytics.
   - Updated on `Fri 22 May 2026 at 08:47 PM` | 880⭐ | `Other`
- [mamiiblt/instafel](https://github.com/mamiiblt/instafel) - Instafel provides fast & up-to-date Instagram Alpha experience for everyone!.
   - Updated on `Mon 08 June 2026 at 08:08 PM` | 1130⭐ | `MIT License`
- [jarvis2f/telegram-files](https://github.com/jarvis2f/telegram-files) - A self-hosted Telegram file downloader for continuous, stable, and unattended downloads.
   - Updated on `Wed 04 February 2026 at 02:45 AM` | 2272⭐ | `MIT License`
- [paroj/DSub2000](https://github.com/paroj/DSub2000) - Android client for Subsonic servers. This is a fork of the original DSub project.
   - Updated on `Fri 05 June 2026 at 11:42 AM` | 128⭐ | `GNU General Public License v3.0`
- [Anuken/Mindustry](https://github.com/Anuken/Mindustry) - The automation tower defense RTS.
   - Updated on `Tue 09 June 2026 at 03:47 AM` | 27829⭐ | `GNU General Public License v3.0`
- [Team-xManager/xManager](https://github.com/Team-xManager/xManager) - Ad-Free, New Features & Freedom.
   - Updated on `Fri 15 August 2025 at 01:13 PM` | 12167⭐ | `GNU General Public License v3.0`
- [ReVanced/GmsCore](https://github.com/ReVanced/GmsCore) - Free implementation of Play Services.
   - Updated on `Wed 18 February 2026 at 01:16 PM` | 4437⭐ | `Apache License 2.0`
- [slapperwan/gh4a](https://github.com/slapperwan/gh4a) - Github client for Android.
   - Updated on `Tue 09 December 2025 at 10:30 AM` | 1975⭐ | `Apache License 2.0`
- [diffplug/spotless](https://github.com/diffplug/spotless) - Keep your code spotless.
   - Updated on `Tue 09 June 2026 at 09:38 PM` | 5508⭐ | `Apache License 2.0`
- [LawnchairLauncher/lawnchair](https://github.com/LawnchairLauncher/lawnchair) - No clever tagline needed.
   - Updated on `Tue 09 June 2026 at 05:26 AM` | 12912⭐ | `Other`
- [Nekogram/Nekogram](https://github.com/Nekogram/Nekogram) - Open-source third-party Telegram client with not many but useful modifications.
   - Updated on `Sat 23 May 2026 at 01:00 PM` | 3664⭐ | `GNU General Public License v2.0`
- [bepass-org/oblivion](https://github.com/bepass-org/oblivion) - Unofficial warp client for android.
   - Updated on `Mon 02 March 2026 at 08:44 AM` | 4670⭐ | `❌`
- [artipie/artipie](https://github.com/artipie/artipie) - Binary Artifact Management Tool.
   - Updated on `Wed 17 September 2025 at 06:30 AM` | 685⭐ | `MIT License`
- [eclipse-openvsx/openvsx](https://github.com/eclipse-openvsx/openvsx) - An open-source registry for VS Code extensions.
   - Updated on `Fri 05 June 2026 at 02:57 PM` | 1969⭐ | `Eclipse Public License 2.0`
- [NextAlone/Nagram](https://github.com/NextAlone/Nagram) - The third-party Telegram android app.
   - Updated on `Wed 03 June 2026 at 02:56 PM` | 2859⭐ | `GNU General Public License v3.0`
- [xpipe-io/xpipe](https://github.com/xpipe-io/xpipe) - Access your entire server infrastructure from your local desktop.
   - Updated on `Tue 09 June 2026 at 08:11 PM` | 14181⭐ | `Apache License 2.0`
- [languagetool-org/languagetool](https://github.com/languagetool-org/languagetool) - Style and Grammar Checker for 25+ Languages.
   - Updated on `Tue 09 June 2026 at 03:10 PM` | 14561⭐ | `GNU Lesser General Public License v2.1`
- [newhinton/Round-Sync](https://github.com/newhinton/Round-Sync) - An android cloud file manager, powered by rclone. Visit https://roundsync.com for more information!.
   - Updated on `Sun 16 November 2025 at 12:31 PM` | 2236⭐ | `GNU General Public License v3.0`
- [M66B/NetGuard](https://github.com/M66B/NetGuard) - A simple way to block access to the internet per app.
   - Updated on `Fri 22 May 2026 at 05:21 AM` | 3688⭐ | `GNU General Public License v3.0`
- [slowscript/warpinator-android](https://github.com/slowscript/warpinator-android) - An unofficial implementation of Warpinator for Android.
   - Updated on `Sat 27 December 2025 at 09:17 PM` | 578⭐ | `GNU General Public License v3.0`
- [Etar-Group/Etar-Calendar](https://github.com/Etar-Group/Etar-Calendar) - Android open source calendar.
   - Updated on `Sun 07 June 2026 at 01:37 AM` | 2528⭐ | `GNU General Public License v3.0`
- [mendhak/gpslogger](https://github.com/mendhak/gpslogger) - :satellite: Lightweight GPS Logging Application For Android.
   - Updated on `Sat 06 June 2026 at 04:28 PM` | 2518⭐ | `Other`
- [osmandapp/OsmAnd](https://github.com/osmandapp/OsmAnd) - OsmAnd.
   - Updated on `Tue 09 June 2026 at 04:44 PM` | 5778⭐ | `Other`
- [shayaantx/botdarr](https://github.com/shayaantx/botdarr) - Slack/Discord/Telegram/Matrix bot for accessing radarr, sonarr, and lidarr.
   - Updated on `Wed 08 April 2026 at 12:26 AM` | 199⭐ | `GNU General Public License v3.0`
- [tink-crypto/tink](https://github.com/tink-crypto/tink) - Tink is a multi-language, cross-platform, open source library that provides cryptographic APIs that are secure, easy to use correctly, and hard(er) to misuse.
   - Updated on `Wed 17 April 2024 at 11:08 AM` | 13538⭐ | `Apache License 2.0`
- [zood/george](https://github.com/zood/george) - Zood Location client for Android.
   - Updated on `Tue 20 January 2026 at 01:54 AM` | 156⭐ | `GNU Affero General Public License v3.0`
- [dkanada/gelli](https://github.com/dkanada/gelli) - Native Music Player for Jellyfin.
   - Updated on `Sun 11 June 2023 at 10:38 AM` | 536⭐ | `GNU General Public License v3.0`
- [supertokens/supertokens-core](https://github.com/supertokens/supertokens-core) - Open source alternative to Auth0 / Firebase Auth / AWS Cognito.
   - Updated on `Mon 08 June 2026 at 03:40 PM` | 15083⭐ | `Other`
- [bilde2910/Hauk](https://github.com/bilde2910/Hauk) - Open-source realtime location sharing.
   - Updated on `Wed 26 June 2024 at 04:08 PM` | 914⭐ | `Apache License 2.0`
- [Docile-Alligator/Infinity-For-Reddit](https://github.com/Docile-Alligator/Infinity-For-Reddit) - A Reddit client for Android.
   - Updated on `Mon 08 June 2026 at 01:56 AM` | 5365⭐ | `GNU Affero General Public License v3.0`
- [beemdevelopment/Aegis](https://github.com/beemdevelopment/Aegis) - A free, secure and open source app for Android to manage your 2-step verification tokens.
   - Updated on `Tue 24 February 2026 at 07:51 PM` | 12579⭐ | `GNU General Public License v3.0`
- [sevdokimov/log-viewer](https://github.com/sevdokimov/log-viewer) - Web UI for viewing logs.
   - Updated on `Mon 27 May 2024 at 02:34 PM` | 677⭐ | `Apache License 2.0`
- [kekru/docker-remote-api-tls](https://github.com/kekru/docker-remote-api-tls) - Docker Image that forwards to the Docker API Socket and requires TLS Client authentication.
   - Updated on `Mon 26 January 2026 at 09:39 PM` | 81⭐ | `MIT License`
- [theonedev/onedev](https://github.com/theonedev/onedev) - Git Server with CI/CD, Kanban, and Packages. Seamless integration. Unparalleled experience.
   - Updated on `Mon 08 June 2026 at 01:29 PM` | 15036⭐ | `MIT License`
- [Mamiglia/Reddit-Wallpaper](https://github.com/Mamiglia/Reddit-Wallpaper) - Tool to download and automatically set a random wallpaper from reddit.
   - Updated on `Tue 19 July 2022 at 05:41 PM` | 39⭐ | `MIT License`
- [anthonyraymond/joal](https://github.com/anthonyraymond/joal) - An open source command line RatioMaster with an optional WebUI.
   - Updated on `Mon 01 June 2026 at 11:24 AM` | 807⭐ | `Apache License 2.0`
- [tonikelope/megabasterd](https://github.com/tonikelope/megabasterd) - Yet another unofficial (and ugly) cross-platform MEGA downloader/uploader/streaming suite.
   - Updated on `Thu 04 June 2026 at 08:27 AM` | 5537⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## JavaScript 

- [projectvelox/claude-usage-widget](https://github.com/projectvelox/claude-usage-widget) - Always-on-top floating widget that shows your claude.ai plan usage in real time. Cloudflare-immune OAuth auth via ~/.claude/.credentials.json. Windows-first, portable EXE, 7-day history graph, configu.
   - Updated on `Mon 08 June 2026 at 05:00 AM` | 7⭐ | `MIT License`
- [notBlubbll/free-buff-lol](https://github.com/notBlubbll/free-buff-lol) - .
   - Updated on `Tue 09 June 2026 at 10:48 PM` | 15⭐ | `❌`
- [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem) - Persistent Context Across Sessions for Every Agent –  Captures everything your agent does during sessions, compresses it with AI, and injects relevant context back into future sessions. Works with Cla.
   - Updated on `Wed 10 June 2026 at 12:16 AM` | 81484⭐ | `Apache License 2.0`
- [notBlubbll/gc2oc](https://github.com/notBlubbll/gc2oc) - GithubCopilot2OpenCode- Ollama-emulating API proxy that routes Visual Studio 2026 Copilot Chat requests to opencode-cloudmodels.
   - Updated on `Sun 24 May 2026 at 04:29 AM` | 25⭐ | `❌`
- [decolua/9router](https://github.com/decolua/9router) - Unlimited FREE AI coding. Connect Claude Code, Codex, Cursor, Cline, Copilot, Antigravity to FREE Claude/GPT/Gemini via 40+ providers. Auto-fallback, RTK -40% tokens, never hit limits.
   - Updated on `Sat 06 June 2026 at 09:18 AM` | 17058⭐ | `MIT License`
- [JuliusBrussee/caveman](https://github.com/JuliusBrussee/caveman) - 🪨 why use many token when few token do trick — Claude Code skill that cuts 65% of tokens by talking like caveman.
   - Updated on `Wed 20 May 2026 at 10:44 AM` | 70614⭐ | `MIT License`
- [Pavelevich/llm-checker](https://github.com/Pavelevich/llm-checker) - Advanced CLI tool that scans your hardware and tells you exactly which LLM or sLLM models you can run locally, with full Ollama integration.
   - Updated on `Mon 08 June 2026 at 12:28 PM` | 2530⭐ | `Other`
- [PSerban93/Achievements](https://github.com/PSerban93/Achievements) - A desktop application built with Electron that monitors running games and displays achievements animated notifications.
   - Updated on `Sun 07 June 2026 at 09:17 PM` | 121⭐ | `MIT License`
- [lklynet/aurral](https://github.com/lklynet/aurral) - The best in open-source music discovery.
   - Updated on `Wed 10 June 2026 at 12:31 AM` | 1084⭐ | `MIT License`
- [darktakayanagi/Achievement-Watcher](https://github.com/darktakayanagi/Achievement-Watcher) - A sexy achievement file parser with real-time notification, automatic screenshot and playtime tracking. View every achievements earned on your PC whether it's coming from Steam, a Steam emulator, and .
   - Updated on `Sat 31 January 2026 at 06:45 PM` | 140⭐ | `GNU Lesser General Public License v3.0`
- [sassanix/Warracker](https://github.com/sassanix/Warracker) - 🛡️ Warracker is an open source, self-hostable warranty tracker to monitor expirations, store receipts, files. You own the data, your rules!.
   - Updated on `Mon 08 June 2026 at 04:01 AM` | 1444⭐ | `GNU Affero General Public License v3.0`
- [dockpeek/dockpeek](https://github.com/dockpeek/dockpeek) - Easily access your Docker container web interfaces and keep them up to date — across all your hosts.
   - Updated on `Wed 05 November 2025 at 10:56 AM` | 1797⭐ | `MIT License`
- [gorhill/uBlock](https://github.com/gorhill/uBlock) - uBlock Origin - An efficient blocker for Chromium and Firefox. Fast and lean.
   - Updated on `Wed 10 June 2026 at 12:14 AM` | 65361⭐ | `GNU General Public License v3.0`
- [mihail-pop/media-journal](https://github.com/mihail-pop/media-journal) - A web app for tracking your movies, TV shows, anime, manga, books, games and music.
   - Updated on `Mon 08 June 2026 at 12:01 PM` | 129⭐ | `GNU Affero General Public License v3.0`
- [advplyr/audiobookshelf](https://github.com/advplyr/audiobookshelf) - Self-hosted audiobook and podcast server.
   - Updated on `Thu 04 June 2026 at 08:13 PM` | 13145⭐ | `GNU General Public License v3.0`
- [gradio-app/fastrtc](https://github.com/gradio-app/fastrtc) - The python library for real-time communication.
   - Updated on `Mon 12 January 2026 at 03:53 PM` | 4600⭐ | `MIT License`
- [clusterzx/paperless-ai](https://github.com/clusterzx/paperless-ai) - An automated document analyzer for Paperless-ngx using OpenAI API, Ollama, Deepseek-r1, Azure and all OpenAI API compatible Services to automatically analyze and tag your documents.
   - Updated on `Tue 09 June 2026 at 11:21 AM` | 5740⭐ | `MIT License`
- [bigskysoftware/htmx](https://github.com/bigskysoftware/htmx) - &lt;/&gt; htmx - high power tools for HTML.
   - Updated on `Tue 09 June 2026 at 01:17 PM` | 48172⭐ | `Other`
- [songquanpeng/one-api](https://github.com/songquanpeng/one-api) - LLM API 管理 & 分发系统，支持 OpenAI、Azure、Anthropic Claude、Google Gemini、DeepSeek、字节豆包、ChatGLM、文心一言、讯飞星火、通义千问、360 智脑、腾讯混元等主流模型，统一 API 适配，可用于 key 管理与二次分发。单可执行文件，提供 Docker 镜像，一键部署，开箱即用。LLM API management & key .
   - Updated on `Fri 09 January 2026 at 03:26 AM` | 34771⭐ | `MIT License`
- [FortAwesome/Font-Awesome](https://github.com/FortAwesome/Font-Awesome) - The iconic SVG, font, and CSS toolkit.
   - Updated on `Tue 10 February 2026 at 06:21 PM` | 76645⭐ | `Other`
- [leaningtech/webvm](https://github.com/leaningtech/webvm) - Virtual Machine for the Web.
   - Updated on `Mon 01 June 2026 at 02:06 PM` | 16908⭐ | `Apache License 2.0`
- [ozgrozer/ai-renamer](https://github.com/ozgrozer/ai-renamer) - A Node.js CLI that uses Ollama and LM Studio models (Llava, Gemma, Llama etc.) to intelligently rename files by their contents.
   - Updated on `Sun 09 February 2025 at 02:58 PM` | 2085⭐ | `GNU General Public License v3.0`
- [automatisch/automatisch](https://github.com/automatisch/automatisch) - The open source Zapier alternative. Build workflow automation without spending time and money.
   - Updated on `Wed 11 February 2026 at 07:53 PM` | 13856⭐ | `Other`
- [vercel/next.js](https://github.com/vercel/next.js) - The React Framework.
   - Updated on `Wed 10 June 2026 at 12:27 AM` | 139912⭐ | `MIT License`
- [SillyTavern/SillyTavern](https://github.com/SillyTavern/SillyTavern) - LLM Frontend for Power Users.
   - Updated on `Wed 20 May 2026 at 10:06 AM` | 29129⭐ | `GNU Affero General Public License v3.0`
- [simple-icons/simple-icons](https://github.com/simple-icons/simple-icons) - SVG icons for popular brands.
   - Updated on `Mon 08 June 2026 at 02:34 AM` | 25199⭐ | `Creative Commons Zero v1.0 Universal`
- [expressjs/express](https://github.com/expressjs/express) - Fast, unopinionated, minimalist web framework for node.
   - Updated on `Tue 02 June 2026 at 05:27 AM` | 69125⭐ | `MIT License`
- [moorer2k/delete-all-google-photos](https://github.com/moorer2k/delete-all-google-photos) - Simple javascript to autoselect ALL google photos and delete them.
   - Updated on `Mon 05 August 2024 at 02:42 AM` | 6⭐ | `❌`
- [SortableJS/Sortable](https://github.com/SortableJS/Sortable) - Reorderable drag-and-drop lists for modern browsers and touch devices. No jQuery or framework required.
   - Updated on `Tue 24 March 2026 at 10:02 PM` | 31134⭐ | `MIT License`
- [dessant/repo-lockdown](https://github.com/dessant/repo-lockdown) - GitHub Action that immediately closes and locks issues and pull requests.
   - Updated on `Sat 23 May 2026 at 05:51 AM` | 144⭐ | `MIT License`
- [TheRealJoelmatic/RemoveAdblockThing](https://github.com/TheRealJoelmatic/RemoveAdblockThing) - The intrusive "Ad blocker are not allowed on YouTube"  message is annoying. This open-source project aims to address this issue by providing a solution to bypass YouTube's ad blocker detection.
   - Updated on `Tue 24 December 2024 at 08:21 PM` | 6011⭐ | `MIT License`
- [CesiumGS/cesium](https://github.com/CesiumGS/cesium) - An open-source JavaScript library for world-class 3D globes and maps :earth_americas:.
   - Updated on `Tue 09 June 2026 at 08:19 PM` | 15357⭐ | `Apache License 2.0`
- [google/zx](https://github.com/google/zx) - A tool for writing better scripts.
   - Updated on `Sun 10 May 2026 at 04:34 PM` | 45523⭐ | `Apache License 2.0`
- [Leaflet/Leaflet](https://github.com/Leaflet/Leaflet) - 🍃 JavaScript library for mobile-friendly interactive maps 🇺🇦.
   - Updated on `Mon 08 June 2026 at 12:52 PM` | 45168⭐ | `BSD 2-Clause "Simplified" License`
- [PrismJS/prism](https://github.com/PrismJS/prism) - Lightweight, robust, elegant syntax highlighting.
   - Updated on `Tue 09 June 2026 at 02:14 PM` | 12996⭐ | `MIT License`
- [feathericons/feather](https://github.com/feathericons/feather) - Simply beautiful open-source icons.
   - Updated on `Tue 11 March 2025 at 02:18 AM` | 25932⭐ | `MIT License`
- [Dooy/chatgpt-web-midjourney-proxy](https://github.com/Dooy/chatgpt-web-midjourney-proxy) - One UI is all done with chatgpt web, midjourney, gpts,suno,luma,runway,viggle,flux,ideogram,realtime,pika,udio; Simultaneous support  Web / PWA / Linux / Win / MacOS platform.
   - Updated on `Mon 11 May 2026 at 01:12 AM` | 6718⭐ | `MIT License`
- [Mintplex-Labs/anything-llm](https://github.com/Mintplex-Labs/anything-llm) - Stop renting your intelligence. Own it with AnythingLLM. Everything you need for a powerful local-first agent experience.
   - Updated on `Wed 10 June 2026 at 12:12 AM` | 61327⭐ | `MIT License`
- [HermanMartinus/bearblog](https://github.com/HermanMartinus/bearblog) - Free, no-nonsense, super fast blogging.
   - Updated on `Mon 08 June 2026 at 10:35 AM` | 5022⭐ | `Other`
- [jhonderson/gone-man-switch](https://github.com/jhonderson/gone-man-switch) - Gone Man’s Switch.
   - Updated on `Mon 03 March 2025 at 04:59 AM` | 157⭐ | `GNU General Public License v3.0`
- [hakimel/reveal.js](https://github.com/hakimel/reveal.js) - The HTML Presentation Framework.
   - Updated on `Thu 21 May 2026 at 07:20 PM` | 71645⭐ | `MIT License`
- [prettier/prettier](https://github.com/prettier/prettier) - Prettier is an opinionated code formatter.
   - Updated on `Wed 10 June 2026 at 12:28 AM` | 51962⭐ | `MIT License`
- [iammodev/YAKC](https://github.com/iammodev/YAKC) - YAKC is a Key/Mouse Click Visualizer - Perfect for content creators, developers or presentations!.
   - Updated on `Thu 16 November 2023 at 10:27 PM` | 92⭐ | `MIT License`
- [webdiscus/parallax-3d-lens-effect](https://github.com/webdiscus/parallax-3d-lens-effect) - 3D parallax effect by mouse moving using CSS transform.
   - Updated on `Wed 30 October 2024 at 10:40 AM` | 70⭐ | `ISC License`
- [cdxgen/cdxgen](https://github.com/cdxgen/cdxgen) - Creates CycloneDX Bill of Materials (BOM) for your projects from source and container images. Supports many languages and package managers. Integrate in your CI/CD pipeline with automatic submission t.
   - Updated on `Tue 09 June 2026 at 07:17 PM` | 983⭐ | `Apache License 2.0`
- [atomiks/tippyjs](https://github.com/atomiks/tippyjs) - Tooltip, popover, dropdown, and menu library.
   - Updated on `Mon 27 May 2024 at 07:00 PM` | 12268⭐ | `MIT License`
- [nuejs/nue](https://github.com/nuejs/nue) - Fastest way to build modern websites.
   - Updated on `Tue 17 February 2026 at 05:11 PM` | 8821⭐ | `MIT License`
- [LINKIWI/orion-web](https://github.com/LINKIWI/orion-web) - Robust web visualization tool for OwnTracks location data.
   - Updated on `Sun 27 October 2019 at 12:30 AM` | 204⭐ | `MIT License`
- [pallets-eco/flask-debugtoolbar](https://github.com/pallets-eco/flask-debugtoolbar) - A toolbar overlay for debugging Flask applications.
   - Updated on `Mon 01 June 2026 at 08:16 PM` | 979⭐ | `BSD 3-Clause "New" or "Revised" License`
- [Oneill19/social-preview-generator](https://github.com/Oneill19/social-preview-generator) - Github social preview generator made using HTML, CSS and Javascript.
   - Updated on `Sun 24 July 2022 at 03:18 PM` | 4⭐ | `MIT License`
- [nodejs/node](https://github.com/nodejs/node) - Node.js JavaScript runtime ✨🐢🚀✨.
   - Updated on `Tue 09 June 2026 at 07:48 PM` | 117704⭐ | `Other`
- [sveltejs/svelte](https://github.com/sveltejs/svelte) - web development for the rest of us.
   - Updated on `Mon 08 June 2026 at 09:40 PM` | 87165⭐ | `MIT License`
- [Chalarangelo/30-seconds-of-code](https://github.com/Chalarangelo/30-seconds-of-code) - Coding articles to level up your development skills.
   - Updated on `Mon 08 June 2026 at 11:27 PM` | 128034⭐ | `Creative Commons Attribution 4.0 International`
- [FastForwardTeam/FastForward](https://github.com/FastForwardTeam/FastForward) - Don't waste your time with compliance. FastForward automatically skips annoying link shorteners.
   - Updated on `Mon 01 June 2026 at 04:42 AM` | 3764⭐ | `The Unlicense`
- [Not-Quite-RARBG/main](https://github.com/Not-Quite-RARBG/main) - Not Quite RARBG's main website.
   - Updated on `Fri 22 September 2023 at 11:58 AM` | 694⭐ | `GNU Affero General Public License v3.0`
- [azukaar/Cosmos-Server](https://github.com/azukaar/Cosmos-Server) - ☁️ The Most Secure and Easy Selfhosted Home Server. Take control of your data and privacy without sacrificing security and stability  (Authentication, anti-DDOS, anti-bot).
   - Updated on `Tue 26 May 2026 at 12:20 PM` | 5951⭐ | `Other`
- [CyferShepard/Jellystat](https://github.com/CyferShepard/Jellystat) - Jellystat is a free and open source Statistics App for Jellyfin.
   - Updated on `Sat 18 April 2026 at 05:05 PM` | 2333⭐ | `MIT License`
- [ai-shifu/ChatALL](https://github.com/ai-shifu/ChatALL) - Concurrently chat with ChatGPT, Bing Chat, Bard, Alpaca, Vicuna, Claude, ChatGLM, MOSS, 讯飞星火, 文心一言 and more, discover the best answers.
   - Updated on `Wed 11 February 2026 at 06:21 PM` | 16411⭐ | `Apache License 2.0`
- [owntracks/frontend](https://github.com/owntracks/frontend) - 🌍 Web interface for OwnTracks built with Vue.js.
   - Updated on `Wed 23 April 2025 at 04:52 PM` | 540⭐ | `MIT License`
- [spicetify/cli](https://github.com/spicetify/cli) - Command-line tool to customize Spotify client. Supports Windows, macOS, and Linux.
   - Updated on `Fri 01 May 2026 at 06:42 PM` | 23359⭐ | `GNU Lesser General Public License v2.1`
- [nosir/cleave.js](https://github.com/nosir/cleave.js) - Format input text content when you are typing...
   - Updated on `Sat 25 November 2023 at 10:40 AM` | 17876⭐ | `Apache License 2.0`
- [validatorjs/validator.js](https://github.com/validatorjs/validator.js) - String validation.
   - Updated on `Fri 24 April 2026 at 06:39 PM` | 23753⭐ | `MIT License`
- [chartjs/Chart.js](https://github.com/chartjs/Chart.js) - Simple HTML5 Charts using the &lt;canvas&gt; tag.
   - Updated on `Wed 27 May 2026 at 09:17 PM` | 67481⭐ | `MIT License`
- [aunefyren/wrapperr](https://github.com/aunefyren/wrapperr) - Website based application that summarizes Plex statistics from a given period and displays it in a nice format. Similar to the Spotify Wrapped concept.
   - Updated on `Sat 30 May 2026 at 10:55 AM` | 452⭐ | `GNU General Public License v3.0`
- [inuyaksa/jquery.nicescroll](https://github.com/inuyaksa/jquery.nicescroll) - nicescroll plugin for jquery - scrollbars like iphone/ipad.
   - Updated on `Thu 04 January 2024 at 07:55 AM` | 3574⭐ | `❌`
- [rfoel/bulma-toast](https://github.com/rfoel/bulma-toast) - Bulma's pure JavaScript extension to display toasts.
   - Updated on `Sat 02 March 2024 at 01:06 PM` | 167⭐ | `MIT License`
- [gluon-framework/gluon](https://github.com/gluon-framework/gluon) - A new framework for creating desktop apps from websites, using system installed browsers and NodeJS.
   - Updated on `Wed 08 November 2023 at 03:29 AM` | 3056⭐ | `MIT License`
- [jhuckaby/Cronicle](https://github.com/jhuckaby/Cronicle) - A simple, distributed task scheduler and runner with a web based UI.
   - Updated on `Tue 19 May 2026 at 05:35 PM` | 5741⭐ | `Other`
- [heysagnik/tweets](https://github.com/heysagnik/tweets) - .
   - Updated on `Sun 03 July 2022 at 07:54 AM` | 7⭐ | `GNU General Public License v3.0`
- [pulsar-edit/pulsar](https://github.com/pulsar-edit/pulsar) - A Community-led Hyper-Hackable Text Editor.
   - Updated on `Sat 06 June 2026 at 08:43 PM` | 4066⭐ | `Other`
- [grocy/grocy](https://github.com/grocy/grocy) - ERP beyond your fridge - Grocy is a web-based self-hosted groceries & household management solution for your home.
   - Updated on `Fri 05 June 2026 at 03:58 PM` | 9133⭐ | `MIT License`
- [steveseguin/vdo.ninja](https://github.com/steveseguin/vdo.ninja) - VDO.Ninja is a powerful tool that lets you bring remote video feeds into OBS or other studio software via WebRTC.
   - Updated on `Sun 07 June 2026 at 08:40 PM` | 3894⭐ | `Other`
- [adsbypasser/adsbypasser](https://github.com/adsbypasser/adsbypasser) - Bypass ad counter if possible.
   - Updated on `Mon 08 June 2026 at 01:30 PM` | 1725⭐ | `BSD 3-Clause "New" or "Revised" License`
- [facebook-adblock/facebook_adblock](https://github.com/facebook-adblock/facebook_adblock) - An open-source Ad Blocker for Facebook™.
   - Updated on `Sun 12 February 2023 at 02:59 PM` | 219⭐ | `GNU General Public License v3.0`
- [OhMyGuus/I-Still-Dont-Care-About-Cookies](https://github.com/OhMyGuus/I-Still-Dont-Care-About-Cookies) - Debloated fork of the extension "I don't care about cookies".
   - Updated on `Tue 09 June 2026 at 06:45 PM` | 4168⭐ | `GNU General Public License v3.0`
- [WebTools-NG/WebTools-NG](https://github.com/WebTools-NG/WebTools-NG) - WebTools Next Generation for Plex.
   - Updated on `Tue 07 May 2024 at 11:01 PM` | 897⭐ | `Mozilla Public License 2.0`
- [aplotor/expanse](https://github.com/aplotor/expanse) - selfhosted multi-user web app for externally storing Reddit items (saved, created, upvoted, downvoted, hidden) to bypass Reddit's 1000-item listing limits.
   - Updated on `Fri 30 May 2025 at 08:17 AM` | 425⭐ | `GNU Affero General Public License v3.0`
- [danrahn/MarkerEditorForPlex](https://github.com/danrahn/MarkerEditorForPlex) - View, edit, add, and delete intro, credits, and commercial markers for your Plex media.
   - Updated on `Mon 25 May 2026 at 08:15 PM` | 163⭐ | `MIT License`
- [gethomepage/homepage](https://github.com/gethomepage/homepage) - A highly customizable homepage (or startpage / application dashboard) with Docker and service API integrations.
   - Updated on `Tue 09 June 2026 at 06:06 PM` | 30601⭐ | `GNU General Public License v3.0`
- [Rob--W/cors-anywhere](https://github.com/Rob--W/cors-anywhere) - CORS Anywhere is a NodeJS reverse proxy which adds CORS headers to the proxied request.
   - Updated on `Fri 16 August 2024 at 09:14 PM` | 9374⭐ | `MIT License`
- [gchq/CyberChef](https://github.com/gchq/CyberChef) - The Cyber Swiss Army Knife - a web app for encryption, encoding, compression and data analysis.
   - Updated on `Tue 09 June 2026 at 08:44 AM` | 35057⭐ | `Apache License 2.0`
- [synclounge/synclounge](https://github.com/synclounge/synclounge) - Enjoy Plex with your friends. In Sync. Together.
   - Updated on `Tue 09 June 2026 at 02:29 PM` | 1131⭐ | `MIT License`
- [timvisee/send](https://github.com/timvisee/send) - :mailbox_with_mail: Simple, private file sharing. Mirror of https://gitlab.com/timvisee/send.
   - Updated on `Tue 01 July 2025 at 07:02 AM` | 5786⭐ | `Mozilla Public License 2.0`
- [alexivkin/forkrefresh](https://github.com/alexivkin/forkrefresh) - Refresh your github forks to be up to date with the source.
   - Updated on `Thu 28 March 2024 at 02:09 AM` | 31⭐ | `MIT License`
- [fosslife/delta](https://github.com/fosslife/delta) - A modern file uploader + URL shortner written in node for your private cloud. low memory overhead + secure.
   - Updated on `Sun 11 April 2021 at 09:26 AM` | 127⭐ | `MIT License`
- [vexorian/dizquetv](https://github.com/vexorian/dizquetv) - Create live TV channels from your own media. Access the streams using the simulated HDHomerun tuner or the generated M3U URl.
   - Updated on `Sat 02 May 2026 at 11:20 PM` | 1760⭐ | `zlib License`
- [GladysAssistant/Gladys](https://github.com/GladysAssistant/Gladys) - A privacy-first, open-source home assistant.
   - Updated on `Mon 08 June 2026 at 03:21 PM` | 3064⭐ | `Apache License 2.0`
- [EsperoTech/yaade](https://github.com/EsperoTech/yaade) - Yaade is an open-source, self-hosted, collaborative API development environment.
   - Updated on `Mon 08 September 2025 at 11:57 AM` | 1985⭐ | `MIT License`
- [raghavyuva/LucaMail](https://github.com/raghavyuva/LucaMail) - An Awesome cross platform, simple, os friendly Email Client for desktop.
   - Updated on `Fri 21 October 2022 at 10:33 AM` | 241⭐ | `GNU General Public License v3.0`
- [TiddlyWiki/TiddlyWiki5](https://github.com/TiddlyWiki/TiddlyWiki5) - A self-contained JavaScript wiki for the browser, Node.js, AWS Lambda etc.
   - Updated on `Tue 09 June 2026 at 03:36 PM` | 8594⭐ | `Other`
- [frncsdrk/avatar](https://github.com/frncsdrk/avatar) - Self-hosted service for creating random avatars. Development moved to codeberg.
   - Updated on `Tue 26 May 2026 at 08:03 AM` | 114⭐ | `MIT License`
- [imneonizer/autolycus](https://github.com/imneonizer/autolycus) - A seedr.cc inspired web application to download and stream torrent files from hosted web servers.
   - Updated on `Wed 26 February 2025 at 05:54 AM` | 134⭐ | `GNU General Public License v3.0`
- [mediacms-io/mediacms](https://github.com/mediacms-io/mediacms) - MediaCMS is a modern, fully featured open source video and media CMS, written in Python/Django and React, featuring a REST API.
   - Updated on `Sun 07 June 2026 at 02:56 PM` | 4954⭐ | `GNU Affero General Public License v3.0`
- [JasonHHouse/gaps](https://github.com/JasonHHouse/gaps) - Find the missing movies in your Plex Server.
   - Updated on `Sat 25 October 2025 at 11:52 AM` | 576⭐ | `MIT License`
- [DustinBrett/daedalOS](https://github.com/DustinBrett/daedalOS) - Desktop environment in the browser.
   - Updated on `Sun 31 May 2026 at 06:26 AM` | 12873⭐ | `MIT License`
- [pucherot/Pi.Alert](https://github.com/pucherot/Pi.Alert) - WIFI / LAN intruder detector. Check the devices connected and alert you with unknown devices. It also warns of the disconnection of "always connected" devices.
   - Updated on `Thu 08 February 2024 at 09:00 AM` | 2518⭐ | `GNU General Public License v3.0`
- [Xtrendence/FileDrop](https://github.com/Xtrendence/FileDrop) - Grade: 89%. COMP3006 coursework. An encrypted file sharing application that leverages web sockets to allow users to send and receive files.
   - Updated on `Mon 14 February 2022 at 09:58 PM` | 151⭐ | `GNU Affero General Public License v3.0`
- [agalwood/Motrix](https://github.com/agalwood/Motrix) - A full-featured download manager.
   - Updated on `Thu 11 July 2024 at 12:09 PM` | 51934⭐ | `MIT License`
- [menukaonline/goindex-extended](https://github.com/menukaonline/goindex-extended) - A Google Drive Index, which has a lot of extra features, running on Cloudflare Workers.
   - Updated on `Fri 12 April 2024 at 07:37 AM` | 493⭐ | `MIT License`
- [TannerReynolds/ShareX-Upload-Server](https://github.com/TannerReynolds/ShareX-Upload-Server) - AKA ShareS - Feature full & Stable ShareX and file server in node. Includes images, videos, code, text, markdown rendering, password protected uploads, logging via discord, administration through Disc.
   - Updated on `Fri 23 February 2024 at 01:30 PM` | 397⭐ | `GNU General Public License v3.0`
- [jeroenpardon/sui](https://github.com/jeroenpardon/sui) - a startpage for your server and / or new tab page.
   - Updated on `Fri 02 August 2024 at 10:42 AM` | 1573⭐ | `The Unlicense`
- [useful-forks/useful-forks.github.io](https://github.com/useful-forks/useful-forks.github.io) - Improving GitHub's Forks list discoverability through automatic filtering. The project offers an online tool and a Chrome extension.
   - Updated on `Sat 15 February 2025 at 12:08 AM` | 1319⭐ | `MIT License`
- [louislam/uptime-kuma](https://github.com/louislam/uptime-kuma) - A fancy self-hosted monitoring tool.
   - Updated on `Wed 10 June 2026 at 12:29 AM` | 87824⭐ | `MIT License`
- [psi-4ward/psitransfer](https://github.com/psi-4ward/psitransfer) - Simple open source self-hosted file sharing solution.
   - Updated on `Tue 02 June 2026 at 09:26 AM` | 1893⭐ | `BSD 2-Clause "Simplified" License`
- [victornpb/undiscord](https://github.com/victornpb/undiscord) - Undiscord - Delete all messages in a Discord server / channel or DM (Easy and fast) Bulk delete.
   - Updated on `Mon 22 December 2025 at 01:59 PM` | 6563⭐ | `Other`
- [tks18/gindex-v4](https://github.com/tks18/gindex-v4) - A Vue Js Based G Index with Improved Dark Mode, Search and Video Player.
   - Updated on `Mon 01 November 2021 at 03:39 PM` | 252⭐ | `GNU General Public License v3.0`
- [phpservermon/phpservermon](https://github.com/phpservermon/phpservermon) - PHP Server Monitor.
   - Updated on `Wed 23 April 2025 at 03:48 PM` | 2193⭐ | `GNU General Public License v3.0`
- [arachnys/cabot](https://github.com/arachnys/cabot) - Self-hosted, easily-deployable monitoring and alerts service - like a lightweight PagerDuty.
   - Updated on `Sun 10 September 2023 at 09:23 AM` | 5667⭐ | `MIT License`
- [pi-hole/web](https://github.com/pi-hole/web) - Pi-hole Dashboard for stats and more.
   - Updated on `Sat 06 June 2026 at 10:14 AM` | 2503⭐ | `Other`
- [mailcow/mailcow-dockerized](https://github.com/mailcow/mailcow-dockerized) - mailcow: dockerized - 🐮 + 🐋 = 💕.
   - Updated on `Wed 10 June 2026 at 12:08 AM` | 12914⭐ | `GNU General Public License v3.0`
- [LeeluPradhan/G-Index](https://github.com/LeeluPradhan/G-Index) - Google Drive Directory Index Running on CloudFlare Workers [Search + DarkMode] [ENGLISH].
   - Updated on `Thu 01 October 2020 at 12:06 PM` | 178⭐ | `MIT License`
- [streamaserver/streama](https://github.com/streamaserver/streama) - Self hosted streaming media server. https://docs.streama-project.com/.
   - Updated on `Sun 19 April 2026 at 07:39 PM` | 9816⭐ | `MIT License`
- [FaisalUmair/udemy-downloader-gui](https://github.com/FaisalUmair/udemy-downloader-gui) - A desktop application for downloading Udemy Courses.
   - Updated on `Sat 06 April 2024 at 04:06 PM` | 6321⭐ | `MIT License`
- [anthonyraymond/joal-desktop](https://github.com/anthonyraymond/joal-desktop) - A desktop application for JOAL. Plug and play, no additional download or configuration needed.
   - Updated on `Mon 18 November 2024 at 10:43 PM` | 214⭐ | `MIT License`
- [xan105/Achievement-Watcher](https://github.com/xan105/Achievement-Watcher) - A sexy achievement file parser with real-time notification, automatic screenshot and playtime tracking. View every achievements earned on your PC whether it's coming from Steam, a Steam emulator, and .
   - Updated on `Thu 20 June 2024 at 02:32 PM` | 951⭐ | `GNU Lesser General Public License v3.0`
- [request/request](https://github.com/request/request) - 🏊🏾 Simplified HTTP request client.
   - Updated on `Wed 14 August 2024 at 12:09 AM` | 25550⭐ | `Apache License 2.0`
- [gatsbyjs/gatsby](https://github.com/gatsbyjs/gatsby) - React-based framework with performance, scalability, and security built in.
   - Updated on `Tue 09 June 2026 at 05:29 PM` | 55940⭐ | `MIT License`
- [jlippold/tweakCompatible](https://github.com/jlippold/tweakCompatible) - Adds a compatibility checker to cydia.
   - Updated on `Thu 28 April 2022 at 02:42 PM` | 395⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Jinja 

- [spantaleev/matrix-docker-ansible-deploy](https://github.com/spantaleev/matrix-docker-ansible-deploy) - 🐳 Matrix (An open network for secure, decentralized communication) server setup using Ansible and Docker.
   - Updated on `Tue 09 June 2026 at 02:01 PM` | 6391⭐ | `GNU Affero General Public License v3.0`
- [Cloudbox/Community](https://github.com/Cloudbox/Community) - Cloudbox Community Project.
   - Updated on `Sat 25 November 2023 at 04:40 PM` | 279⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Jupyter Notebook 

- [mahmoud/awesome-python-applications](https://github.com/mahmoud/awesome-python-applications) - 💿 Free software that works great, and also happens to be open-source Python.
   - Updated on `Fri 25 April 2025 at 05:36 PM` | 17886⭐ | `❌`
- [sweepai/sweep](https://github.com/sweepai/sweep) - Sweep: AI coding assistant for JetBrains.
   - Updated on `Thu 18 September 2025 at 06:10 AM` | 7704⭐ | `Other`
- [01-ai/Yi](https://github.com/01-ai/Yi) - A series of large language models trained from scratch by developers @01-ai.
   - Updated on `Wed 27 November 2024 at 09:25 AM` | 7821⭐ | `Apache License 2.0`
- [meta-llama/llama-cookbook](https://github.com/meta-llama/llama-cookbook) - Welcome to the Llama Cookbook! This is your go to guide for Building with Llama: Getting started with Inference, Fine-Tuning, RAG. We also show you how to solve end to end problems using Llama model f.
   - Updated on `Tue 19 May 2026 at 06:42 PM` | 18346⭐ | `MIT License`
- [Ayanaminn/N46Whisper](https://github.com/Ayanaminn/N46Whisper) - Whisper based Japanese subtitle generator.
   - Updated on `Sun 23 February 2025 at 01:04 PM` | 1708⭐ | `MIT License`
- [tecepeipe/RcloneLab](https://github.com/tecepeipe/RcloneLab) - rclone, qBittorrent, JDownloader, YouTube-DL, netdata, Cloud Commander on Google Colab.
   - Updated on `Mon 25 July 2022 at 02:28 AM` | 5⭐ | `MIT License`
- [TheCaduceus/Multi-Cloud-Transfer-Tool](https://github.com/TheCaduceus/Multi-Cloud-Transfer-Tool) - The most advanced yet simple Jupyter Notebook to transfer/manage your data from/to any cloud remotely based on Rclone and other engines.⚡.
   - Updated on `Sat 30 March 2024 at 12:02 PM` | 353⭐ | `❌`
- [baronet2/Spotify-Playlist-Scraper](https://github.com/baronet2/Spotify-Playlist-Scraper) - Tools to load data from public Spotify playlists.
   - Updated on `Tue 02 March 2021 at 05:21 AM` | 4⭐ | `❌`
- [mohitjoshi155/Codemaster](https://github.com/mohitjoshi155/Codemaster) - Codemaster@BSSG.
   - Updated on `Mon 19 June 2023 at 07:31 PM` | 63⭐ | `❌`
- [yunooooo/Master-Colab](https://github.com/yunooooo/Master-Colab) - .
   - Updated on `Mon 01 June 2020 at 02:54 PM` | 40⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Kotlin 

- [damontecres/Wholphin](https://github.com/damontecres/Wholphin) - An OSS Android TV client for Jellyfin.
   - Updated on `Tue 09 June 2026 at 02:54 AM` | 2249⭐ | `GNU General Public License v2.0`
- [iamgio/quarkdown](https://github.com/iamgio/quarkdown) - 🪐 Markdown with superpowers: from ideas to papers, presentations, websites, books, and knowledge bases.
   - Updated on `Tue 09 June 2026 at 07:35 AM` | 15482⭐ | `GNU General Public License v3.0`
- [owenlejeune/ArrMatey](https://github.com/owenlejeune/ArrMatey) - .
   - Updated on `Sun 07 June 2026 at 11:15 PM` | 667⭐ | `MIT License`
- [AChep/keyguard-app](https://github.com/AChep/keyguard-app) - Alternative client for the Bitwarden® platform & KeePass (KDBX), created to provide the best user experience possible.
   - Updated on `Tue 09 June 2026 at 02:01 PM` | 2988⭐ | `Other`
- [amir1376/ab-download-manager](https://github.com/amir1376/ab-download-manager) - A Download Manager that speeds up your downloads.
   - Updated on `Sun 07 June 2026 at 11:56 PM` | 16021⭐ | `Apache License 2.0`
- [adeekshith/watomatic](https://github.com/adeekshith/watomatic) - Auto reply app to enable auto reply for messengers like WhatsApp and Facebook Messenger.
   - Updated on `Thu 14 May 2026 at 01:43 AM` | 546⭐ | `GNU General Public License v3.0`
- [T8RIN/ImageToolbox](https://github.com/T8RIN/ImageToolbox) - 🖼️ Image Toolbox is a powerful app for advanced image manipulation. It offers dozens of features, from basic tools like crop and draw to filters, OCR, and a wide range of image processing options.
   - Updated on `Wed 10 June 2026 at 12:17 AM` | 13110⭐ | `Apache License 2.0`
- [shrimqy/Sefirah-Android](https://github.com/shrimqy/Sefirah-Android) - Phone link / KDE Connect alternative.
   - Updated on `Fri 05 June 2026 at 03:37 PM` | 876⭐ | `GNU General Public License v3.0`
- [KieronQuinn/Smartspacer](https://github.com/KieronQuinn/Smartspacer) - Smartspacer is a customisable widget for Android, but with a difference: It can upgrade the built in At a Glance on Pixels - without root!.
   - Updated on `Sun 07 June 2026 at 01:20 AM` | 3374⭐ | `GNU General Public License v3.0`
- [Hamza417/Peristyle](https://github.com/Hamza417/Peristyle) - Advance wallpaper manager app for Android with cool glassmorphic UI, tags, auto wallpaper, custom effects and multiple folder support and a native live wallpaper picker.
   - Updated on `Sun 07 June 2026 at 07:03 PM` | 639⭐ | `Apache License 2.0`
- [mikepenz/Android-Iconics](https://github.com/mikepenz/Android-Iconics) - Android-Iconics - Use any icon font, or vector (.svg) as drawable in your application.
   - Updated on `Wed 08 April 2026 at 06:26 AM` | 5269⭐ | `Apache License 2.0`
- [mikepenz/AboutLibraries](https://github.com/mikepenz/AboutLibraries) - AboutLibraries automatically collects all dependencies and licenses of any gradle project (Kotlin MultiPlatform), and provides easy to integrate UI components for Android and Compose Multiplatform env.
   - Updated on `Sun 07 June 2026 at 10:18 PM` | 4333⭐ | `Apache License 2.0`
- [BobbyESP/Metadator](https://github.com/BobbyESP/Metadator) - An Android ID3 metadata editor and music player built with Jetpack Compose and Material 3.
   - Updated on `Fri 12 December 2025 at 01:45 PM` | 186⭐ | `GNU Affero General Public License v3.0`
- [Calvin-LL/Reorderable](https://github.com/Calvin-LL/Reorderable) - Reorder items in Lists and Grids in Jetpack Compose and Compose Multiplatform with drag and drop.
   - Updated on `Mon 20 April 2026 at 09:46 PM` | 1263⭐ | `Apache License 2.0`
- [connectbot/connectbot](https://github.com/connectbot/connectbot) - ConnectBot is the first SSH client for Android.
   - Updated on `Tue 09 June 2026 at 11:43 PM` | 3273⭐ | `Apache License 2.0`
- [chenxiaolong/RSAF](https://github.com/chenxiaolong/RSAF) - An Android Storage Access Framework document provider for rclone.
   - Updated on `Fri 05 June 2026 at 09:08 PM` | 843⭐ | `GNU General Public License v3.0`
- [BimmerGestalt/AAIdrive](https://github.com/BimmerGestalt/AAIdrive) - Implementations of some Android Auto features as unofficial IDrive apps.
   - Updated on `Sun 07 June 2026 at 07:49 PM` | 664⭐ | `MIT License`
- [JetBrains/kotlin](https://github.com/JetBrains/kotlin) - The Kotlin Programming Language.
   - Updated on `Wed 10 June 2026 at 12:32 AM` | 52839⭐ | `❌`
- [w2sv/FileNavigator](https://github.com/w2sv/FileNavigator) - The missing link between Android and a sorted file system.
   - Updated on `Tue 09 June 2026 at 11:36 AM` | 531⭐ | `GNU General Public License v3.0`
- [charlee-dev/PexWallpapers](https://github.com/charlee-dev/PexWallpapers) - Android application following best practices: Jetpack, Jetpack Compose, Modularity, Clean Architecture, Kotlin Coroutines, Tests, MVVM, DI, Static Analysis.
   - Updated on `Sun 03 April 2022 at 12:31 PM` | 167⭐ | `MIT License`
- [HeliBorg/HeliBoard](https://github.com/HeliBorg/HeliBoard) - Customizable and privacy-conscious open-source keyboard.
   - Updated on `Tue 09 June 2026 at 09:39 PM` | 5428⭐ | `GNU General Public License v3.0`
- [yairm210/Unciv](https://github.com/yairm210/Unciv) - Open-source Android/Desktop remake of Civ V.
   - Updated on `Tue 09 June 2026 at 11:18 AM` | 10490⭐ | `Mozilla Public License 2.0`
- [fork-maintainers/iceraven-browser](https://github.com/fork-maintainers/iceraven-browser) - Iceraven Browser.
   - Updated on `Fri 22 May 2026 at 01:25 PM` | 6195⭐ | `❌`
- [chenxiaolong/BCR](https://github.com/chenxiaolong/BCR) - A Basic Call Recorder for rooted Android devices.
   - Updated on `Tue 02 June 2026 at 11:11 PM` | 2772⭐ | `GNU General Public License v3.0`
- [NeoApplications/Neo-Backup](https://github.com/NeoApplications/Neo-Backup) - backup manager for android.
   - Updated on `Sun 03 May 2026 at 08:52 PM` | 3665⭐ | `GNU Affero General Public License v3.0`
- [element-hq/element-x-android](https://github.com/element-hq/element-x-android) - Android Matrix messenger application using the Matrix Rust Sdk and Jetpack Compose.
   - Updated on `Tue 09 June 2026 at 11:29 PM` | 2195⭐ | `GNU Affero General Public License v3.0`
- [ReVanced/revanced-manager](https://github.com/ReVanced/revanced-manager) - 💊 Application to use ReVanced on Android.
   - Updated on `Sun 07 June 2026 at 12:42 AM` | 28209⭐ | `GNU General Public License v3.0`
- [you-apps/TranslateYou](https://github.com/you-apps/TranslateYou) - Privacy focused translator app built with MD3.
   - Updated on `Thu 02 April 2026 at 08:48 PM` | 1497⭐ | `GNU General Public License v3.0`
- [ruslin-note/ruslin-android](https://github.com/ruslin-note/ruslin-android) - A simple notes application that supports syncing notes using a self-hosted Joplin server.
   - Updated on `Fri 09 May 2025 at 08:18 PM` | 183⭐ | `GNU General Public License v3.0`
- [tylerbwong/stack](https://github.com/tylerbwong/stack) - An Android app for browsing Stack Overflow and other Stack Exchange sites.
   - Updated on `Fri 04 April 2025 at 03:10 PM` | 575⭐ | `GNU General Public License v3.0`
- [OxygenCobalt/Auxio](https://github.com/OxygenCobalt/Auxio) - A simple, rational music player for android.
   - Updated on `Mon 01 June 2026 at 01:39 AM` | 3886⭐ | `GNU General Public License v3.0`
- [zyrouge/symphony](https://github.com/zyrouge/symphony) - 🎵 Lightweight, elegant music player for Android 9+.
   - Updated on `Sun 22 March 2026 at 04:59 PM` | 1673⭐ | `GNU Affero General Public License v3.0`
- [AlfaazPlus/QuranApp](https://github.com/AlfaazPlus/QuranApp) - An ad-free and privacy-focused app for exploring the Holy Qur’an.
   - Updated on `Sun 07 June 2026 at 08:40 PM` | 366⭐ | `GNU General Public License v3.0`
- [Schaka/rarrnomore](https://github.com/Schaka/rarrnomore) - Prevents Sonarr and Radarr from grabbing releases containing RARs.
   - Updated on `Mon 08 April 2024 at 07:27 PM` | 29⭐ | `❌`
- [BobbyESP/Spowlo](https://github.com/BobbyESP/Spowlo) - A Spotify songs downloader for Android made with Jetpack Compose, Material You and the spotDL Python library.
   - Updated on `Sun 16 March 2025 at 10:51 PM` | 1364⭐ | `GNU General Public License v3.0`
- [topjohnwu/Magisk](https://github.com/topjohnwu/Magisk) - The Magic Mask for Android.
   - Updated on `Sun 31 May 2026 at 06:47 AM` | 60869⭐ | `GNU General Public License v3.0`
- [Chooloo/koler](https://github.com/Chooloo/koler) - Just a phone app.
   - Updated on `Fri 27 December 2024 at 11:08 PM` | 1057⭐ | `Other`
- [TaylorKunZhang/SubTune](https://github.com/TaylorKunZhang/SubTune) - A music streaming app for Subsonic API compatible servers.
   - Updated on `Mon 31 July 2023 at 03:49 PM` | 163⭐ | `Apache License 2.0`
- [bitwarden/android](https://github.com/bitwarden/android) - Bitwarden mobile apps (Password Manager and Authenticator) for Android.
   - Updated on `Tue 09 June 2026 at 10:27 PM` | 8958⭐ | `GNU General Public License v3.0`
- [element-hq/element-android](https://github.com/element-hq/element-android) - A Matrix collaboration client for Android.
   - Updated on `Mon 08 June 2026 at 01:51 PM` | 3699⭐ | `GNU Affero General Public License v3.0`
- [lhns/restic-android](https://github.com/lhns/restic-android) - Restic Backup Android App  (Unofficial).
   - Updated on `Thu 26 February 2026 at 09:22 PM` | 218⭐ | `GNU General Public License v2.0`
- [Bartuzen/qBitController](https://github.com/Bartuzen/qBitController) - Control qBittorrent from any device.
   - Updated on `Tue 05 May 2026 at 02:17 PM` | 1209⭐ | `GNU General Public License v3.0`
- [Exodus-Privacy/exodus-android-app](https://github.com/Exodus-Privacy/exodus-android-app) - εxodus Android application.
   - Updated on `Thu 20 November 2025 at 06:38 PM` | 990⭐ | `GNU General Public License v3.0`
- [mudkipme/MoeMemosAndroid](https://github.com/mudkipme/MoeMemosAndroid) - An app to help you capture thoughts and ideas.
   - Updated on `Sun 31 May 2026 at 06:38 AM` | 1108⭐ | `GNU General Public License v3.0`
- [thunderbird/thunderbird-android](https://github.com/thunderbird/thunderbird-android) - Thunderbird for Android – Open Source Email App for Android (fka K-9 Mail).
   - Updated on `Tue 09 June 2026 at 04:49 PM` | 13579⭐ | `Apache License 2.0`
- [ouchadam/small-talk](https://github.com/ouchadam/small-talk) - Minimal Android messenger powered by Matrix.
   - Updated on `Fri 29 May 2026 at 03:08 PM` | 140⭐ | `Apache License 2.0`
- [binwiederhier/ntfy-android](https://github.com/binwiederhier/ntfy-android) - Android app for ntfy.sh.
   - Updated on `Fri 05 June 2026 at 05:18 PM` | 1006⭐ | `Apache License 2.0`
- [JunkFood02/Seal](https://github.com/JunkFood02/Seal) - 🦭 Video/Audio Downloader for Android, based on yt-dlp.
   - Updated on `Mon 25 May 2026 at 04:16 PM` | 26840⭐ | `GNU General Public License v3.0`
- [florisboard/florisboard](https://github.com/florisboard/florisboard) - An open-source keyboard for Android which respects your privacy. Currently in beta.
   - Updated on `Fri 24 April 2026 at 03:57 AM` | 8405⭐ | `Apache License 2.0`
- [Aliucord/Aliucord](https://github.com/Aliucord/Aliucord) - A mod for the Discord Android App.
   - Updated on `Mon 08 June 2026 at 02:31 AM` | 4748⭐ | `Open Software License 3.0`
- [jarnedemeulemeester/findroid](https://github.com/jarnedemeulemeester/findroid) - Third-party native Jellyfin Android app.
   - Updated on `Mon 08 June 2026 at 07:39 PM` | 4081⭐ | `GNU General Public License v3.0`
- [ReadYouApp/ReadYou](https://github.com/ReadYouApp/ReadYou) - An Android RSS reader presented in Material You style.
   - Updated on `Wed 03 June 2026 at 02:55 PM` | 7214⭐ | `GNU General Public License v3.0`
- [tasks/tasks](https://github.com/tasks/tasks) - Bringing Astrid Tasks back from the dead.
   - Updated on `Tue 09 June 2026 at 10:55 PM` | 5297⭐ | `GNU General Public License v3.0`
- [savvasdalkitsis/uhuruphotos-android](https://github.com/savvasdalkitsis/uhuruphotos-android) - 🖼️A modern media gallery, with features like backup/sync, semantic search, media map, face recognition, memories and much more built using the latest Android technologies.
   - Updated on `Mon 08 June 2026 at 06:10 PM` | 399⭐ | `Apache License 2.0`
- [mozilla-mobile/fenix](https://github.com/mozilla-mobile/fenix) - ⚠️ Fenix (Firefox for Android) moved to a new repository. It is now developed and maintained as part of: https://github.com/mozilla-mobile/firefox-android.
   - Updated on `Mon 20 February 2023 at 10:15 AM` | 6396⭐ | `Mozilla Public License 2.0`
- [signalapp/Signal-Android](https://github.com/signalapp/Signal-Android) - A private messenger for Android.
   - Updated on `Tue 09 June 2026 at 09:29 PM` | 28886⭐ | `GNU Affero General Public License v3.0`
- [NeoApplications/Neo-Store](https://github.com/NeoApplications/Neo-Store) - An F-Droid client with modern UI and an arsenal of extra features.
   - Updated on `Sat 25 April 2026 at 11:48 PM` | 4711⭐ | `GNU General Public License v3.0`
- [mollyim/mollyim-android](https://github.com/mollyim/mollyim-android) - Enhanced and security-focused fork of Signal.
   - Updated on `Sat 30 May 2026 at 12:59 AM` | 3438⭐ | `GNU Affero General Public License v3.0`
- [owntracks/android](https://github.com/owntracks/android) - OwnTracks Android App.
   - Updated on `Tue 09 June 2026 at 08:14 AM` | 1713⭐ | `Eclipse Public License 1.0`
- [chrisbanes/tivi](https://github.com/chrisbanes/tivi) - Tivi is a TV show tracking Android app, which connects to trakt.tv.
   - Updated on `Tue 12 November 2024 at 06:56 PM` | 6733⭐ | `Apache License 2.0`
 
**[`^        back to top        ^`](#)**

## Lua 

- [Y0URD34TH/Project-GLD](https://github.com/Y0URD34TH/Project-GLD) - Project GLD is a game library and download manager, which supports community made Lua scripts for downloading and searching games.
   - Updated on `Fri 05 June 2026 at 06:47 PM` | 470⭐ | `Apache License 2.0`
- [folke/trouble.nvim](https://github.com/folke/trouble.nvim) - 🚦 A pretty diagnostics, references, telescope results, quickfix and location list to help you solve all the trouble your code is causing.
   - Updated on `Fri 31 October 2025 at 02:02 PM` | 6813⭐ | `Apache License 2.0`
- [owntracks/docker-recorder](https://github.com/owntracks/docker-recorder) - Docker image for OwnTracks Recorder.
   - Updated on `Tue 12 August 2025 at 06:52 AM` | 225⭐ | `❌`
- [ntop/ntopng](https://github.com/ntop/ntopng) - Web-based Traffic and Security Network Traffic Monitoring.
   - Updated on `Tue 09 June 2026 at 05:12 PM` | 7857⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## MDX 

- [MarsX-dev/floatui](https://github.com/MarsX-dev/floatui) - Beautiful and responsive UI components and templates for React and Vue (soon) with Tailwind CSS.
   - Updated on `Sun 23 March 2025 at 01:28 PM` | 3573⭐ | `Other`
- [twbs/bootstrap](https://github.com/twbs/bootstrap) - The most popular HTML, CSS, and JavaScript framework for developing responsive, mobile first projects on the web.
   - Updated on `Tue 09 June 2026 at 10:20 PM` | 174295⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Makefile 

- [HaveAGitGat/Tdarr](https://github.com/HaveAGitGat/Tdarr) - Tdarr - Distributed transcode automation using FFmpeg/HandBrake + Audio/Video library analytics + video health checking (Windows, macOS, Linux & Docker).
   - Updated on `Fri 29 May 2026 at 04:34 PM` | 4157⭐ | `Other`
 
**[`^        back to top        ^`](#)**

## Markdown 

- [upptime/upptime](https://github.com/upptime/upptime) - ⬆️ GitHub Actions uptime monitor & status page by @AnandChowdhary.
   - Updated on `Wed 10 June 2026 at 12:04 AM` | 17050⭐ | `MIT License`
- [TRaSH-Guides/Guides](https://github.com/TRaSH-Guides/Guides) - TRaSH-Guides is a comprehensive collection of guides for Radarr, Sonarr, and related media management applications.
   - Updated on `Tue 09 June 2026 at 08:11 PM` | 3011⭐ | `MIT License`
- [tldr-pages/tldr](https://github.com/tldr-pages/tldr) - Collaborative cheatsheets for console commands 📚.
   - Updated on `Tue 09 June 2026 at 07:33 PM` | 62808⭐ | `Other`
- [codecrafters-io/build-your-own-x](https://github.com/codecrafters-io/build-your-own-x) - Master programming by recreating your favorite technologies from scratch.
   - Updated on `Sat 21 February 2026 at 09:34 AM` | 513644⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Nim 

- [zedeus/nitter](https://github.com/zedeus/nitter) - Alternative Twitter front-end.
   - Updated on `Mon 08 June 2026 at 10:01 PM` | 13080⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Objective-C 

- [opa334/TrollStore](https://github.com/opa334/TrollStore) - Jailed iOS app that can install IPAs permanently with arbitary entitlements and root helpers because it trolls Apple.
   - Updated on `Wed 01 April 2026 at 09:43 AM` | 21533⭐ | `Other`
- [Greg0109/ForwardNotifier](https://github.com/Greg0109/ForwardNotifier) - Forward your notifications from your iOS device to your PC/iOS device!.
   - Updated on `Sat 02 July 2022 at 01:24 AM` | 87⭐ | `GNU General Public License v3.0`
- [JeffResc/Dynamik](https://github.com/JeffResc/Dynamik) - Dynamic wallpapers of your choice.
   - Updated on `Tue 05 January 2021 at 08:36 PM` | 6⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Odin 

- [odin-lang/Odin](https://github.com/odin-lang/Odin) - Odin Programming Language.
   - Updated on `Tue 09 June 2026 at 04:23 PM` | 10709⭐ | `zlib License`
 
**[`^        back to top        ^`](#)**

## Others 

- [12britz/awesome-free-models](https://github.com/12britz/awesome-free-models) - A curated list of free AI models, APIs, and tools you can use without paying a cent.
   - Updated on `Tue 09 June 2026 at 06:47 PM` | 159⭐ | `Other`
- [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills) - A single CLAUDE.md file to improve Claude Code behavior, derived from Andrej Karpathy's observations on LLM coding pitfalls.
   - Updated on `Mon 20 April 2026 at 10:05 AM` | 172082⭐ | `❌`
- [mohgomaa-art/NotebookMG](https://github.com/mohgomaa-art/NotebookMG) - AI-powered Study Intelligence System that transforms passive learning into active mastery.
   - Updated on `Tue 24 February 2026 at 05:16 PM` | 4⭐ | `MIT License`
- [gaijin213/EasyLinuxScripts](https://github.com/gaijin213/EasyLinuxScripts) - This repo will be dedicated to make the new Linux users journey easier with a few automation scripts.
   - Updated on `Sun 28 December 2025 at 11:27 AM` | 1⭐ | `MIT License`
- [plausible/community-edition](https://github.com/plausible/community-edition) - Example Docker Compose setup for hosting Plausible Community Edition.
   - Updated on `Fri 15 May 2026 at 09:19 AM` | 2455⭐ | `MIT License`
- [Hack-with-Github/Awesome-Hacking](https://github.com/Hack-with-Github/Awesome-Hacking) - A collection of various awesome lists for hackers, pentesters and security researchers.
   - Updated on `Thu 07 May 2026 at 04:44 PM` | 114058⭐ | `Creative Commons Zero v1.0 Universal`
- [spa5k/tafsir_api](https://github.com/spa5k/tafsir_api) - Free Tafsir API Service with different languages and translations.
   - Updated on `Thu 04 June 2026 at 04:05 PM` | 174⭐ | `MIT License`
- [fawazahmed0/quran-api](https://github.com/fawazahmed0/quran-api) - Free Quran API Service with 90+ different languages and 400+ translations.
   - Updated on `Sun 15 March 2026 at 05:50 PM` | 1105⭐ | `The Unlicense`
- [fawazahmed0/hadith-api](https://github.com/fawazahmed0/hadith-api) - Free Hadith API Service with Multiple Languages and Multiple Grades.
   - Updated on `Wed 03 June 2026 at 04:16 AM` | 518⭐ | `The Unlicense`
- [lukasz-madon/awesome-remote-job](https://github.com/lukasz-madon/awesome-remote-job) - A curated list of awesome remote jobs and resources. Inspired by https://github.com/vinta/awesome-python.
   - Updated on `Fri 08 May 2026 at 10:48 PM` | 46272⭐ | `❌`
- [YimMenu/YimMenu](https://github.com/YimMenu/YimMenu) - YimMenu, a GTA V menu protecting against a wide ranges of the public crashes and improving the overall experience.
   - Updated on `Sun 06 October 2024 at 10:12 PM` | 1573⭐ | `GNU General Public License v2.0`
- [mozilla/geckodriver](https://github.com/mozilla/geckodriver) - WebDriver Classic proxy for automating Firefox through Marionette.
   - Updated on `Wed 03 June 2026 at 11:15 AM` | 7475⭐ | `Mozilla Public License 2.0`
- [digitalblossom/alternative-frontends](https://github.com/digitalblossom/alternative-frontends) - 🔐🌐 Privacy-respecting web frontends for popular services.
   - Updated on `Thu 21 March 2024 at 04:11 PM` | 2258⭐ | `GNU General Public License v3.0`
- [roshanlam/DarkFusion](https://github.com/roshanlam/DarkFusion) - A sleek VS Code theme blending Atom One Dark Pro, JetBrains, and Gruvbox.
   - Updated on `Fri 28 June 2024 at 05:21 AM` | 1⭐ | `MIT License`
- [yokoffing/filterlists](https://github.com/yokoffing/filterlists) - Collection of blocklists to fill in the gaps.
   - Updated on `Sun 31 May 2026 at 01:43 AM` | 3376⭐ | `❌`
- [KenneyNL/Adobe-Alternatives](https://github.com/KenneyNL/Adobe-Alternatives) - A list of alternatives for Adobe software.
   - Updated on `Mon 27 April 2026 at 07:27 PM` | 9057⭐ | `❌`
- [pantsufan/Magisk-Ad-Blocking-Module](https://github.com/pantsufan/Magisk-Ad-Blocking-Module) - Magisk module to block ads on android.
   - Updated on `Tue 09 June 2026 at 10:51 AM` | 564⭐ | `❌`
- [pantsufan/BlockAds](https://github.com/pantsufan/BlockAds) - Magisk module to block ads on android.
   - Updated on `Mon 01 June 2026 at 01:37 PM` | 210⭐ | `❌`
- [AndroidAudioMods/ViPER4Android](https://github.com/AndroidAudioMods/ViPER4Android) - ViPER4Android app configuration files.
   - Updated on `Mon 11 December 2023 at 06:59 PM` | 818⭐ | `❌`
- [Profluent-AI/OpenCRISPR](https://github.com/Profluent-AI/OpenCRISPR) - AI-generated gene editing systems.
   - Updated on `Fri 26 September 2025 at 07:21 PM` | 1195⭐ | `❌`
- [ai-boost/awesome-prompts](https://github.com/ai-boost/awesome-prompts) - Curated list of chatgpt prompts from the top-rated GPTs in the GPTs Store. Prompt Engineering, prompt attack & prompt protect. Advanced Prompt Engineering papers.
   - Updated on `Tue 09 June 2026 at 06:36 PM` | 8162⭐ | `GNU General Public License v3.0`
- [friuns2/BlackFriday-GPTs-Prompts](https://github.com/friuns2/BlackFriday-GPTs-Prompts) - List of free GPTs that doesn't require plus subscription.
   - Updated on `Wed 18 March 2026 at 02:09 AM` | 9501⭐ | `MIT License`
- [veggiemonk/awesome-docker](https://github.com/veggiemonk/awesome-docker) - :whale: A curated list of Docker resources and projects.
   - Updated on `Fri 05 June 2026 at 08:57 AM` | 36183⭐ | `Apache License 2.0`
- [MoonWalker440/Music-Megathread](https://github.com/MoonWalker440/Music-Megathread) - A Comprehensive List For All Music 🎶 Lovers. It's a treasure trove of music resources to enhance your listening experience and discover new music.
   - Updated on `Tue 24 June 2025 at 12:33 PM` | 2808⭐ | `❌`
- [runs-on/runs-on](https://github.com/runs-on/runs-on) - Self-hosted GitHub Actions runners made simple. For AWS. 10x cheaper, 40% faster, and unlimited caching. Best alternative to Actions Runner Controller.
   - Updated on `Tue 09 June 2026 at 08:32 AM` | 1197⭐ | `MIT License`
- [AndrewStetsenko/tech-jobs-with-relocation](https://github.com/AndrewStetsenko/tech-jobs-with-relocation) - All-in-one guide to getting a tech job abroad 🌎.
   - Updated on `Thu 19 March 2026 at 02:04 PM` | 4422⭐ | `Creative Commons Zero v1.0 Universal`
- [firstcontributions/first-contributions](https://github.com/firstcontributions/first-contributions) - 🚀✨ Help beginners to contribute to open source projects.
   - Updated on `Tue 09 June 2026 at 11:29 PM` | 54402⭐ | `MIT License`
- [alebcay/awesome-shell](https://github.com/alebcay/awesome-shell) - A curated list of awesome command-line frameworks, toolkits, guides and gizmos. Inspired by awesome-php.
   - Updated on `Thu 28 August 2025 at 02:40 PM` | 37062⭐ | `Creative Commons Zero v1.0 Universal`
- [dkhamsing/open-source-ios-apps](https://github.com/dkhamsing/open-source-ios-apps) - :iphone: Collaborative List of Open-Source iOS Apps.
   - Updated on `Tue 09 June 2026 at 12:50 PM` | 50646⭐ | `Creative Commons Zero v1.0 Universal`
- [serhii-londar/open-source-mac-os-apps](https://github.com/serhii-londar/open-source-mac-os-apps) - 🚀 Awesome list of open source applications for macOS. https://t.me/s/opensourcemacosapps.
   - Updated on `Wed 15 April 2026 at 10:47 PM` | 48983⭐ | `Creative Commons Zero v1.0 Universal`
- [wasabeef/awesome-android-ui](https://github.com/wasabeef/awesome-android-ui) - A curated list of awesome Android UI/UX libraries.
   - Updated on `Fri 05 June 2026 at 05:39 AM` | 56367⭐ | `MIT License`
- [sindresorhus/awesome-nodejs](https://github.com/sindresorhus/awesome-nodejs) - :zap: Delightful Node.js packages and resources [BECAUSE OF TOO MUCH SPAM AND LOW-QUALITY SUBMISSIONS, SUBMISSIONS ARE PAUSED UNTIL JULY].
   - Updated on `Sun 03 May 2026 at 08:24 AM` | 65914⭐ | `Creative Commons Zero v1.0 Universal`
- [tesseract-ocr/tessdata](https://github.com/tesseract-ocr/tessdata) - Trained models with fast variant of the "best" LSTM models + legacy models.
   - Updated on `Sat 09 March 2024 at 10:04 AM` | 7563⭐ | `Apache License 2.0`
- [sindresorhus/awesome](https://github.com/sindresorhus/awesome) - 😎 Awesome lists about all kinds of interesting topics.
   - Updated on `Tue 02 June 2026 at 12:50 PM` | 474333⭐ | `Creative Commons Zero v1.0 Universal`
- [nyas1/Material-You-app-list](https://github.com/nyas1/Material-You-app-list) - 🍦 A well organized & frequently updated collection of apps that supports material you design.
   - Updated on `Tue 09 June 2026 at 06:08 PM` | 4912⭐ | `❌`
- [cedrickchee/awesome-wireguard](https://github.com/cedrickchee/awesome-wireguard) - A curated list of WireGuard tools, projects, and resources.
   - Updated on `Wed 04 September 2024 at 03:07 AM` | 1459⭐ | `MIT License`
- [btw-so/open-source-alternatives](https://github.com/btw-so/open-source-alternatives) - List of open-source alternatives to everyday SaaS products.
   - Updated on `Fri 08 November 2024 at 10:26 AM` | 8495⭐ | `MIT License`
- [sindresorhus/awesome-whisper](https://github.com/sindresorhus/awesome-whisper) - 🔊 Awesome list for Whisper — an open-source AI-powered speech recognition system developed by OpenAI.
   - Updated on `Tue 17 March 2026 at 12:02 PM` | 2327⭐ | `Creative Commons Zero v1.0 Universal`
- [sindresorhus/awesome-chatgpt](https://github.com/sindresorhus/awesome-chatgpt) - 🤖 Awesome list for ChatGPT — an artificial intelligence chatbot developed by OpenAI.
   - Updated on `Sun 15 February 2026 at 06:05 PM` | 6287⭐ | `Creative Commons Zero v1.0 Universal`
- [mjhea0/awesome-flask](https://github.com/mjhea0/awesome-flask) - A curated list of awesome things related to Flask.
   - Updated on `Wed 13 May 2026 at 01:35 PM` | 1738⭐ | `Creative Commons Zero v1.0 Universal`
- [Entree3k/Jellyfin](https://github.com/Entree3k/Jellyfin) - Making better looking posters for Jellyfin's TV Network section.
   - Updated on `Mon 27 April 2026 at 04:32 AM` | 64⭐ | `❌`
- [ferritreader/ferrit](https://github.com/ferritreader/ferrit) - DISCONTINUED. Former fork of libreddit/libreddit.
   - Updated on `Mon 07 November 2022 at 05:59 AM` | 48⭐ | `GNU Affero General Public License v3.0`
- [bogachenkove/fuckfuckadblock](https://github.com/bogachenkove/fuckfuckadblock) - Filters for blocking mining, pop-ups and anti-adblock bypass.
   - Updated on `Sun 16 March 2025 at 06:09 PM` | 2184⭐ | `MIT License`
- [rfmoz/grafana-dashboards](https://github.com/rfmoz/grafana-dashboards) - Grafana dashboards.
   - Updated on `Wed 29 April 2026 at 09:45 PM` | 1689⭐ | `Apache License 2.0`
- [mendel5/alternative-front-ends](https://github.com/mendel5/alternative-front-ends) - Overview of alternative open source front-ends for popular internet platforms (e.g. YouTube, Twitter, etc.).
   - Updated on `Sun 18 August 2024 at 10:19 AM` | 8948⭐ | `GNU Affero General Public License v3.0`
- [christophercalm/if-im-gone](https://github.com/christophercalm/if-im-gone) - A cheat sheet for if I am somehow incapacitated.
   - Updated on `Fri 01 July 2022 at 11:10 AM` | 1049⭐ | `MIT License`
- [Psyhackological/AAA](https://github.com/Psyhackological/AAA) - :iphone: Curated list of THE BEST FOSS Android apps to maximize your freedom & privacy!.
   - Updated on `Thu 05 February 2026 at 07:20 PM` | 2868⭐ | `Creative Commons Attribution 4.0 International`
- [beeware/beeware](https://github.com/beeware/beeware) - A meta-package simplifying the installation of the BeeWare suite of tools.
   - Updated on `Sun 07 June 2026 at 10:57 PM` | 943⭐ | `BSD 3-Clause "New" or "Revised" License`
- [linuxserver/reverse-proxy-confs](https://github.com/linuxserver/reverse-proxy-confs) - These confs are pulled into our SWAG image: https://github.com/linuxserver/docker-swag.
   - Updated on `Wed 03 June 2026 at 07:07 AM` | 1619⭐ | `GNU General Public License v3.0`
- [tycrek/degoogle](https://github.com/tycrek/degoogle) - A huge list of alternatives to Google products. Privacy tips, tricks, and links.
   - Updated on `Sat 08 November 2025 at 04:16 AM` | 8508⭐ | `❌`
- [stoatchat/discussions](https://github.com/stoatchat/discussions) - Repository for miscellaneous repository management and discussions: https://github.com/revoltchat/revolt/discussions.
   - Updated on `Mon 16 February 2026 at 02:57 AM` | 2331⭐ | `❌`
- [yourduskquibbles/webannoyances](https://github.com/yourduskquibbles/webannoyances) - Fix and remove annoying web elements such as sticky headers, floating boxes, floating videos, dickbars, social share bars and other distracting elements.
   - Updated on `Fri 03 May 2024 at 01:48 PM` | 1345⭐ | `Creative Commons Attribution Share Alike 4.0 International`
- [awesome-selfhosted/awesome-selfhosted](https://github.com/awesome-selfhosted/awesome-selfhosted) - A list of Free Software network services and web applications which can be hosted on your own servers.
   - Updated on `Tue 09 June 2026 at 12:50 PM` | 298220⭐ | `Other`
- [dchakro/PiHoleOnMacOS](https://github.com/dchakro/PiHoleOnMacOS) - This is my workflow for installing and setiing up pihole inside a docker container on MacOS mojave and using it as a DNS server.
   - Updated on `Fri 01 January 2021 at 07:11 AM` | 24⭐ | `MIT License`
- [anderspitman/awesome-tunneling](https://github.com/anderspitman/awesome-tunneling) - List of ngrok, Cloudflare Tunnel, Tailscale, and ZeroTier alternatives and other tunneling software and services. Focus on self-hosting.
   - Updated on `Thu 26 February 2026 at 08:53 PM` | 21182⭐ | `❌`
- [AnthonyMusgrove/Emby-ExtendedNotifications](https://github.com/AnthonyMusgrove/Emby-ExtendedNotifications) - Extended Notifications Plugin for Emby - Post notifications from Emby to a configured Facebook Group / Profile Wall.
   - Updated on `Wed 08 April 2020 at 02:39 AM` | 2⭐ | `❌`
- [Mixaill/awesome-gog-galaxy](https://github.com/Mixaill/awesome-gog-galaxy) - A list of GOG Galaxy 2.0 integrations and upcoming features.
   - Updated on `Thu 23 November 2023 at 11:31 PM` | 1902⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## PHP 

- [matomo-org/matomo](https://github.com/matomo-org/matomo) - Empowering People Ethically 🚀 — Matomo is hiring! Join us → https://matomo.org/jobs Matomo is the leading open-source alternative to Google Analytics, giving you complete control and built-in privacy.
   - Updated on `Wed 10 June 2026 at 12:21 AM` | 21580⭐ | `GNU General Public License v3.0`
- [ellite/Wallos](https://github.com/ellite/Wallos) - Wallos: Open-source, self-hostable personal subscription tracker. Visualize your recurring expenses, manage your budget, and save money.
   - Updated on `Sat 06 June 2026 at 09:20 PM` | 8046⭐ | `GNU General Public License v3.0`
- [coollabsio/coolify](https://github.com/coollabsio/coolify) - An open-source, self-hostable PaaS alternative to Vercel, Heroku & Netlify that lets you easily deploy static sites, databases, full-stack applications and 280+ one-click services on your own servers.
   - Updated on `Tue 09 June 2026 at 08:28 PM` | 56715⭐ | `Apache License 2.0`
- [firefly-iii/firefly-iii](https://github.com/firefly-iii/firefly-iii) - Firefly III: a personal finances manager.
   - Updated on `Tue 09 June 2026 at 10:04 AM` | 23666⭐ | `GNU Affero General Public License v3.0`
- [nokonoko/Uguu](https://github.com/nokonoko/Uguu) - Uguu is a simple lightweight temporary file host with support for drop, paste, click and API uploading.
   - Updated on `Sat 06 December 2025 at 10:19 PM` | 1166⭐ | `GNU General Public License v3.0`
- [pablouser1/ProxiTok](https://github.com/pablouser1/ProxiTok) - Open source alternative frontend for TikTok made using PHP.
   - Updated on `Sat 31 May 2025 at 06:56 PM` | 2124⭐ | `GNU Affero General Public License v3.0`
- [aschmelyun/subvert](https://github.com/aschmelyun/subvert) - Generate subtitles, summaries, and chapters from videos in seconds.
   - Updated on `Fri 15 May 2026 at 07:52 AM` | 856⭐ | `MIT License`
- [nextcloud/server](https://github.com/nextcloud/server) - ☁️ Nextcloud server, a safe home for all your data.
   - Updated on `Wed 10 June 2026 at 12:32 AM` | 35714⭐ | `GNU Affero General Public License v3.0`
- [leepeuker/movary](https://github.com/leepeuker/movary) - Self hosted web app to track and rate your watched movies.
   - Updated on `Mon 25 May 2026 at 10:54 AM` | 738⭐ | `MIT License`
- [libretime/libretime](https://github.com/libretime/libretime) - LibreTime: Radio Broadcast & Automation Platform.
   - Updated on `Tue 02 June 2026 at 01:14 AM` | 932⭐ | `GNU Affero General Public License v3.0`
- [AzuraCast/AzuraCast](https://github.com/AzuraCast/AzuraCast) - A self-hosted web radio management suite, including turnkey installer tools for the full radio software stack and a modern, easy-to-use web app to manage your stations.
   - Updated on `Mon 08 June 2026 at 11:44 AM` | 3897⭐ | `GNU Affero General Public License v3.0`
- [DirectoryLister/DirectoryLister](https://github.com/DirectoryLister/DirectoryLister) - 📂 Directory Lister is the easiest way to expose the contents of any web-accessible folder for browsing and sharing.
   - Updated on `Tue 09 June 2026 at 03:17 AM` | 2520⭐ | `MIT License`
- [benjaminjonard/koillection](https://github.com/benjaminjonard/koillection) - Koillection is a self-hosted service allowing users to manage any kind of collections.
   - Updated on `Tue 09 June 2026 at 09:25 PM` | 1206⭐ | `MIT License`
- [shlinkio/shlink](https://github.com/shlinkio/shlink) - The definitive self-hosted URL shortener.
   - Updated on `Sun 07 June 2026 at 07:33 AM` | 5094⭐ | `MIT License`
- [pixelfed/pixelfed](https://github.com/pixelfed/pixelfed) - Photo Sharing. For Everyone.
   - Updated on `Mon 08 June 2026 at 08:02 AM` | 7001⭐ | `GNU Affero General Public License v3.0`
- [RSS-Bridge/rss-bridge](https://github.com/RSS-Bridge/rss-bridge) - The RSS feed for websites missing it.
   - Updated on `Fri 05 June 2026 at 04:24 PM` | 9002⭐ | `The Unlicense`
- [PrivateBin/PrivateBin](https://github.com/PrivateBin/PrivateBin) - A minimalist, open source online pastebin where the server has zero knowledge of pasted data. Data is encrypted/decrypted in the browser using 256 bits AES.
   - Updated on `Tue 09 June 2026 at 04:50 AM` | 8365⭐ | `Other`
- [arabcoders/watchstate](https://github.com/arabcoders/watchstate) - Self-hosted service to sync your plex, jellyfin and emby play state. without relying on 3rd-party external services.
   - Updated on `Sun 07 June 2026 at 07:51 PM` | 1444⭐ | `MIT License`
- [anonaddy/anonaddy](https://github.com/anonaddy/anonaddy) - Anonymous email forwarding.
   - Updated on `Thu 28 May 2026 at 08:55 AM` | 4681⭐ | `GNU Affero General Public License v3.0`
- [hakanersu/iwatched](https://github.com/hakanersu/iwatched) - Track movies or tv shows you watched.
   - Updated on `Sat 19 March 2022 at 11:29 AM` | 128⭐ | `❌`
- [FreshRSS/FreshRSS](https://github.com/FreshRSS/FreshRSS) - A free, self-hostable news aggregator….
   - Updated on `Fri 05 June 2026 at 09:12 PM` | 15264⭐ | `GNU Affero General Public License v3.0`
- [Bubka/2FAuth](https://github.com/Bubka/2FAuth) - A Web app to manage your Two-Factor Authentication (2FA) accounts and generate their security codes.
   - Updated on `Tue 09 June 2026 at 07:18 PM` | 3981⭐ | `GNU Affero General Public License v3.0`
- [diskoverdata/diskover-community](https://github.com/diskoverdata/diskover-community) - Diskover Community Edition - Open source file indexer, file search engine and data management and analytics powered by Elasticsearch.
   - Updated on `Wed 25 March 2026 at 11:17 PM` | 1791⭐ | `Apache License 2.0`
- [devfake/flox](https://github.com/devfake/flox) - Self Hosted Movie, Series and Anime Watch List.
   - Updated on `Tue 20 June 2023 at 09:47 PM` | 1345⭐ | `MIT License`
- [invoiceninja/invoiceninja](https://github.com/invoiceninja/invoiceninja) - A source-available invoice, quote, project and time-tracking app built with Laravel.
   - Updated on `Tue 09 June 2026 at 05:28 PM` | 9808⭐ | `Other`
- [sergix44/XBackBone](https://github.com/sergix44/XBackBone) - A lightweight file manager with full ShareX support and more.
   - Updated on `Wed 25 February 2026 at 07:55 PM` | 1138⭐ | `GNU Affero General Public License v3.0`
- [exposedev/expose](https://github.com/exposedev/expose) - A beautiful, fully open-source, tunneling service - written in pure PHP.
   - Updated on `Fri 27 March 2026 at 03:11 PM` | 4566⭐ | `MIT License`
- [causefx/Organizr](https://github.com/causefx/Organizr) - HTPC/Homelab Services Organizer - Written in PHP.
   - Updated on `Tue 19 May 2026 at 11:24 PM` | 5769⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Perl 

- [autodl-community/autodl-irssi](https://github.com/autodl-community/autodl-irssi) - A community-driven fork of autodl-irssi.
   - Updated on `Tue 11 May 2021 at 08:44 PM` | 377⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## PowerShell 

- [osfv/BraveDebloater](https://github.com/osfv/BraveDebloater) - Safety-first Windows PowerShell debloater for Brave Browser using enterprise policies, dry-run mode, backups, and restore.
   - Updated on `Thu 04 June 2026 at 07:29 PM` | 23⭐ | `MIT License`
- [zoicware/RemoveWindowsAI](https://github.com/zoicware/RemoveWindowsAI) - Force Remove Copilot, Recall and More in Windows 11.
   - Updated on `Sun 07 June 2026 at 11:34 PM` | 11986⭐ | `MIT License`
- [actions/runner-images](https://github.com/actions/runner-images) - GitHub Actions runner images.
   - Updated on `Tue 09 June 2026 at 06:36 PM` | 12822⭐ | `MIT License`
- [fscorrupt/posterizarr](https://github.com/fscorrupt/posterizarr) - 🖼️ Automated poster maker for Plex/Jellyfin/Emby.
   - Updated on `Tue 09 June 2026 at 12:57 PM` | 883⭐ | `GNU General Public License v3.0`
- [0x6d69636b/windows_hardening](https://github.com/0x6d69636b/windows_hardening) - HardeningKitty and Windows Hardening Settings.
   - Updated on `Mon 08 June 2026 at 04:21 PM` | 2626⭐ | `MIT License`
- [ThioJoe/Windows-Super-God-Mode](https://github.com/ThioJoe/Windows-Super-God-Mode) - Creates shortcuts to virtually every special location or action built into Windows.
   - Updated on `Thu 04 June 2026 at 07:35 PM` | 2062⭐ | `GNU General Public License v3.0`
- [Raphire/Win11Debloat](https://github.com/Raphire/Win11Debloat) - A simple, lightweight PowerShell script that allows you to remove pre-installed apps, disable telemetry, as well as perform various other changes to declutter and customize your Windows experience. Wi.
   - Updated on `Tue 09 June 2026 at 11:00 PM` | 47348⭐ | `MIT License`
- [ChrisTitusTech/winutil](https://github.com/ChrisTitusTech/winutil) - Chris Titus Tech's Windows Utility - Install Programs, Tweaks, Fixes, and Updates.
   - Updated on `Tue 09 June 2026 at 05:22 PM` | 55723⭐ | `MIT License`
- [farag2/Sophia-Script-for-Windows](https://github.com/farag2/Sophia-Script-for-Windows) - :zap: The most powerful open source tweaker on GitHub for fine-tuning Windows 10 & Windows 11.
   - Updated on `Sun 24 May 2026 at 06:50 PM` | 9356⭐ | `MIT License`
- [ntdevlabs/tiny11builder](https://github.com/ntdevlabs/tiny11builder) - Scripts to build a trimmed-down Windows 11 image.
   - Updated on `Fri 12 September 2025 at 07:23 AM` | 18865⭐ | `❌`
- [he3als/EdgeRemover](https://github.com/he3als/EdgeRemover) - A PowerShell script that correctly uninstalls or reinstalls Microsoft Edge on Windows 10 & 11.
   - Updated on `Fri 13 June 2025 at 01:02 PM` | 421⭐ | `The Unlicense`
- [SpotX-Official/SpotX](https://github.com/SpotX-Official/SpotX) - SpotX patcher used for patching the desktop version of Spotify.
   - Updated on `Sat 30 May 2026 at 07:39 PM` | 21224⭐ | `MIT License`
- [kenanajkunic/clevelandsteamer](https://github.com/kenanajkunic/clevelandsteamer) - Un-sh*t your Windows install.
   - Updated on `Tue 05 April 2022 at 08:50 AM` | 31⭐ | `MIT License`
- [ninthwalker/saverr](https://github.com/ninthwalker/saverr) - Download and save media from Plex Servers.
   - Updated on `Sun 09 June 2024 at 11:40 PM` | 146⭐ | `MIT License`
- [javinizer/Javinizer](https://github.com/javinizer/Javinizer) - (NSFW) Organize your local Japanese Adult Video (JAV) library.
   - Updated on `Fri 01 May 2026 at 03:44 AM` | 843⭐ | `MIT License`
- [Sycnex/Windows10Debloater](https://github.com/Sycnex/Windows10Debloater) - Script to remove Windows 10 bloatware.
   - Updated on `Fri 10 March 2023 at 04:15 AM` | 18816⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Python 

- [pewdiepie-archdaemon/odysseus](https://github.com/pewdiepie-archdaemon/odysseus) - Self-hosted AI workspace.
   - Updated on `Tue 09 June 2026 at 11:07 PM` | 65508⭐ | `GNU Affero General Public License v3.0`
- [turboderp-org/exllamav3](https://github.com/turboderp-org/exllamav3) - An optimized quantization and inference library for running LLMs locally on modern consumer-class GPUs.
   - Updated on `Tue 09 June 2026 at 08:15 PM` | 935⭐ | `MIT License`
- [Mailu/Mailu](https://github.com/Mailu/Mailu) - Insular email distribution - mail server as Docker images.
   - Updated on `Sun 31 May 2026 at 11:55 AM` | 7304⭐ | `Other`
- [anthropics/claude-code](https://github.com/anthropics/claude-code) - Claude Code is an agentic coding tool that lives in your terminal, understands your codebase, and helps you code faster by executing routine tasks, explaining complex code, and handling git workflows .
   - Updated on `Tue 09 June 2026 at 10:14 PM` | 131338⭐ | `❌`
- [st-tech/ppf-contact-solver](https://github.com/st-tech/ppf-contact-solver) - A contact solver for physics-based simulations involving 👚 shells, 🪵 solids and 🪢 rods.
   - Updated on `Tue 09 June 2026 at 12:12 AM` | 4015⭐ | `Apache License 2.0`
- [paperless-ngx/paperless-ngx](https://github.com/paperless-ngx/paperless-ngx) - A community-supported supercharged document management system: scan, index and archive all your documents.
   - Updated on `Tue 09 June 2026 at 06:14 PM` | 42012⭐ | `GNU General Public License v3.0`
- [cocoindex-io/cocoindex-code](https://github.com/cocoindex-io/cocoindex-code) - A super light-weight embedded code search engine CLI (AST based) that just works - saves 70% token and improves speed for coding agent  🌟 Star if you like it!.
   - Updated on `Tue 09 June 2026 at 07:30 AM` | 1827⭐ | `Apache License 2.0`
- [punitarani/fli](https://github.com/punitarani/fli) - Google Flights MCP, CLI and Python Library.
   - Updated on `Fri 29 May 2026 at 08:00 AM` | 2847⭐ | `MIT License`
- [CloakHQ/CloakBrowser](https://github.com/CloakHQ/CloakBrowser) - Stealth Chromium that passes every bot detection test. Drop-in Playwright replacement with source-level fingerprint patches. 30/30 tests passed.
   - Updated on `Tue 09 June 2026 at 03:38 PM` | 25173⭐ | `MIT License`
- [microsoft/markitdown](https://github.com/microsoft/markitdown) - Python tool for converting files and office documents to Markdown.
   - Updated on `Tue 26 May 2026 at 10:41 PM` | 149352⭐ | `MIT License`
- [MemPalace/mempalace](https://github.com/MemPalace/mempalace) - The best-benchmarked open-source AI memory system. And it's free.
   - Updated on `Mon 08 June 2026 at 11:03 PM` | 55191⭐ | `MIT License`
- [safishamsi/graphify](https://github.com/safishamsi/graphify) - AI coding assistant skill (Claude Code, Codex, OpenCode, Cursor, Gemini CLI, and more). Turn any folder of code, SQL schemas, R scripts, shell scripts, docs, papers, images, or videos into a queryable.
   - Updated on `Mon 08 June 2026 at 10:58 PM` | 64224⭐ | `MIT License`
- [elebumm/RedditVideoMakerBot](https://github.com/elebumm/RedditVideoMakerBot) - Create Reddit Videos with just✨ one command ✨.
   - Updated on `Mon 25 May 2026 at 03:50 PM` | 12421⭐ | `GNU General Public License v3.0`
- [langchain-ai/deepagents](https://github.com/langchain-ai/deepagents) - The batteries-included agent harness.
   - Updated on `Wed 10 June 2026 at 12:27 AM` | 24318⭐ | `MIT License`
- [D4Vinci/Scrapling](https://github.com/D4Vinci/Scrapling) - 🕷️ An adaptive Web Scraping framework that handles everything from a single request to a full-scale crawl!.
   - Updated on `Sun 07 June 2026 at 06:42 PM` | 62493⭐ | `BSD 3-Clause "New" or "Revised" License`
- [NevaMind-AI/memU](https://github.com/NevaMind-AI/memU) - The memory harness for proactive AI agents — structured storage, intent capture, 10x token reduction.
   - Updated on `Tue 09 June 2026 at 06:53 AM` | 13813⭐ | `Other`
- [PostHog/posthog](https://github.com/PostHog/posthog) - 🦔 PostHog is an all-in-one developer platform for building successful products. We offer product analytics, web analytics, session replay, error tracking, feature flags, experimentation, surveys, data.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 34933⭐ | `Other`
- [wolffcatskyy/crowdsec-blocklist-import](https://github.com/wolffcatskyy/crowdsec-blocklist-import) - 10-20x more blocks for your CrowdSec bouncers — 120k+ IPs from 36 free threat feeds.
   - Updated on `Sun 17 May 2026 at 04:21 AM` | 229⭐ | `MIT License`
- [google/langextract](https://github.com/google/langextract) - A Python library for extracting structured information from unstructured text using LLMs with precise source grounding and interactive visualization.
   - Updated on `Thu 21 May 2026 at 06:15 AM` | 36847⭐ | `Apache License 2.0`
- [3b1b/manim](https://github.com/3b1b/manim) - Animation engine for explanatory math videos.
   - Updated on `Sat 18 April 2026 at 01:59 PM` | 87480⭐ | `MIT License`
- [github/awesome-copilot](https://github.com/github/awesome-copilot) - Community-contributed instructions, agents, skills, and configurations to help you make the most of GitHub Copilot.
   - Updated on `Tue 09 June 2026 at 06:58 AM` | 34721⭐ | `MIT License`
- [modelcontextprotocol/python-sdk](https://github.com/modelcontextprotocol/python-sdk) - The official Python SDK for Model Context Protocol servers and clients.
   - Updated on `Tue 09 June 2026 at 03:16 PM` | 23275⭐ | `MIT License`
- [Gururagavendra/gmail-cleaner](https://github.com/Gururagavendra/gmail-cleaner) - web based GUI to cleanup gmail delete, mark as read, unsubsribe from uncessary things u dont like.
   - Updated on `Mon 29 December 2025 at 02:05 AM` | 1876⭐ | `MIT License`
- [clemcer/LoggiFly](https://github.com/clemcer/LoggiFly) - Get Alerts from your Docker Container Logs.
   - Updated on `Wed 03 June 2026 at 06:44 AM` | 1740⭐ | `MIT License`
- [coffeecms/goflask](https://github.com/coffeecms/goflask) - GoFlask is a high-performance web framework that provides 100% Flask-compatible API while delivering 5x better performance through Go's underlying implementation using the Fiber framework.
   - Updated on `Mon 28 July 2025 at 11:44 AM` | 1⭐ | `MIT License`
- [frappe/hrms](https://github.com/frappe/hrms) - Open Source HR and Payroll Software.
   - Updated on `Tue 09 June 2026 at 03:37 PM` | 8076⭐ | `GNU General Public License v3.0`
- [soxoj/maigret](https://github.com/soxoj/maigret) - 🕵️‍♂️ Collect a dossier on a person by username from 3000+ sites.
   - Updated on `Mon 08 June 2026 at 06:03 AM` | 31517⭐ | `MIT License`
- [PromtEngineer/localGPT](https://github.com/PromtEngineer/localGPT) - Chat with your documents on your local device using GPT models. No data leaves your device and 100% private.
   - Updated on `Tue 10 March 2026 at 09:32 PM` | 22215⭐ | `MIT License`
- [snap-stanford/Biomni](https://github.com/snap-stanford/Biomni) - Biomni: a general-purpose biomedical AI agent.
   - Updated on `Mon 08 June 2026 at 06:37 PM` | 3184⭐ | `Apache License 2.0`
- [marketcalls/openalgo](https://github.com/marketcalls/openalgo) - Open Source Algo Trading Platform for Everyone.
   - Updated on `Tue 09 June 2026 at 08:18 PM` | 2015⭐ | `GNU Affero General Public License v3.0`
- [jackkerouac/aphrodite](https://github.com/jackkerouac/aphrodite) - A Python-based application that adds badges and other visual elements to media posters. Aphrodite enhances your media library posters with quality indicators like resolution, audio codec, and review r.
   - Updated on `Sun 10 August 2025 at 08:27 PM` | 156⭐ | `MIT License`
- [jdepoix/youtube-transcript-api](https://github.com/jdepoix/youtube-transcript-api) - This is a python API which allows you to get the transcript/subtitles for a given YouTube video. It also works for automatically generated subtitles and it does not require an API key nor a headless b.
   - Updated on `Tue 19 May 2026 at 06:17 PM` | 7713⭐ | `MIT License`
- [itschasa/speedrr](https://github.com/itschasa/speedrr) - Dynamically manage speeds on torrent clients, with Plex/Jellyfin/Emby intergration.
   - Updated on `Thu 17 July 2025 at 03:22 PM` | 116⭐ | `GNU General Public License v3.0`
- [disposable-email-domains/disposable-email-domains](https://github.com/disposable-email-domains/disposable-email-domains) - a list of disposable email domains.
   - Updated on `Tue 09 June 2026 at 08:30 AM` | 5129⭐ | `Other`
- [XiaoYouChR/Ghost-Downloader-3](https://github.com/XiaoYouChR/Ghost-Downloader-3) - An AI-boost cross-platform multi-protocol fluent-design concurrent downloader built with Python & Qt.
   - Updated on `Tue 09 June 2026 at 03:23 PM` | 5354⭐ | `GNU General Public License v3.0`
- [sinaptik-ai/pandas-ai](https://github.com/sinaptik-ai/pandas-ai) - Chat with your database or your datalake (SQL, CSV, parquet). PandasAI makes data analysis conversational using LLMs and RAG.
   - Updated on `Tue 28 October 2025 at 10:02 AM` | 23579⭐ | `Other`
- [browser-use/browser-use](https://github.com/browser-use/browser-use) - 🌐 Make websites accessible for AI agents. Automate tasks online with ease.
   - Updated on `Tue 09 June 2026 at 11:46 PM` | 97980⭐ | `MIT License`
- [ocrmypdf/OCRmyPDF](https://github.com/ocrmypdf/OCRmyPDF) - OCRmyPDF adds an OCR text layer to scanned PDF files, allowing them to be searched.
   - Updated on `Tue 09 June 2026 at 08:13 AM` | 33833⭐ | `Mozilla Public License 2.0`
- [jftuga/deidentification](https://github.com/jftuga/deidentification) - Deidentify people's names and gender specific pronouns.
   - Updated on `Sat 03 May 2025 at 12:09 PM` | 53⭐ | `MIT License`
- [FuzzyGrim/Yamtrack](https://github.com/FuzzyGrim/Yamtrack) - A self hosted media tracker.
   - Updated on `Mon 08 June 2026 at 03:58 AM` | 2807⭐ | `GNU Affero General Public License v3.0`
- [DrewThomasson/ebook2audiobook](https://github.com/DrewThomasson/ebook2audiobook) - Generate audiobooks from e-books, voice cloning & 1158+ languages!.
   - Updated on `Tue 09 June 2026 at 06:22 PM` | 19222⭐ | `Apache License 2.0`
- [ManimCommunity/manim](https://github.com/ManimCommunity/manim) - A community-maintained Python framework for creating mathematical animations.
   - Updated on `Tue 09 June 2026 at 09:33 PM` | 38899⭐ | `MIT License`
- [QuivrHQ/MegaParse](https://github.com/QuivrHQ/MegaParse) - File Parser optimised for LLM Ingestion with no loss 🧠 Parse PDFs, Docx, PPTx in a format that is ideal for LLMs.
   - Updated on `Fri 21 February 2025 at 03:21 PM` | 7383⭐ | `Apache License 2.0`
- [sqlfluff/sqlfluff](https://github.com/sqlfluff/sqlfluff) - A modular SQL linter and auto-formatter with support for multiple dialects and templated code.
   - Updated on `Mon 08 June 2026 at 05:25 AM` | 9739⭐ | `MIT License`
- [andrewyng/aisuite](https://github.com/andrewyng/aisuite) - Simple, unified interface to multiple Generative AI providers.
   - Updated on `Mon 08 June 2026 at 11:18 PM` | 13806⭐ | `MIT License`
- [keephq/keep](https://github.com/keephq/keep) - The open-source AIOps and alert management platform.
   - Updated on `Mon 08 June 2026 at 09:16 PM` | 11909⭐ | `Other`
- [blacklanternsecurity/bbot](https://github.com/blacklanternsecurity/bbot) - The recursive internet scanner for hackers. 🧡.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 9874⭐ | `GNU Affero General Public License v3.0`
- [bryangerlach/Episodes](https://github.com/bryangerlach/Episodes) - Self Hosted TV show Episode tracker built using django, bootstrap5.
   - Updated on `Thu 02 April 2026 at 04:07 PM` | 16⭐ | `MIT License`
- [sarperavci/CloudflareBypassForScraping](https://github.com/sarperavci/CloudflareBypassForScraping) - A cloudflare verification bypass script for webscraping.
   - Updated on `Wed 27 May 2026 at 10:38 PM` | 2383⭐ | `MIT License`
- [nlevee/simple-cloudflare-solver](https://github.com/nlevee/simple-cloudflare-solver) - .
   - Updated on `Thu 05 March 2026 at 10:53 PM` | 85⭐ | `❌`
- [rommapp/romm](https://github.com/rommapp/romm) - A beautiful, powerful, self-hosted rom manager and player.
   - Updated on `Tue 09 June 2026 at 01:36 PM` | 8980⭐ | `GNU Affero General Public License v3.0`
- [docling-project/docling](https://github.com/docling-project/docling) - Get your documents ready for gen AI.
   - Updated on `Tue 09 June 2026 at 06:06 PM` | 61269⭐ | `MIT License`
- [open-mmlab/Amphion](https://github.com/open-mmlab/Amphion) - Amphion (/æmˈfaɪən/) is a toolkit for Audio, Music, and Speech Generation. Its purpose is to support reproducible research and help junior researchers and engineers get started in the field of audio, .
   - Updated on `Wed 25 March 2026 at 02:11 PM` | 9837⭐ | `MIT License`
- [soimort/you-get](https://github.com/soimort/you-get) - :arrow_double_down: Dumb downloader that scrapes the web.
   - Updated on `Thu 30 April 2026 at 05:24 AM` | 56839⭐ | `Other`
- [ranaroussi/yfinance](https://github.com/ranaroussi/yfinance) - Download market data from Yahoo! Finance's API.
   - Updated on `Tue 09 June 2026 at 05:41 PM` | 24165⭐ | `Apache License 2.0`
- [neonwatty/youtube-gif-maker](https://github.com/neonwatty/youtube-gif-maker) - Easily make and share gifs of your favorite YouTube moments.  Built to self host with Python, AI, and Docker.  Free and open source.
   - Updated on `Tue 03 December 2024 at 11:55 PM` | 17⭐ | `Apache License 2.0`
- [mandiant/capa](https://github.com/mandiant/capa) - The FLARE team's open-source tool to identify capabilities in executable files.
   - Updated on `Tue 09 June 2026 at 09:28 PM` | 6045⭐ | `Apache License 2.0`
- [mikf/gallery-dl](https://github.com/mikf/gallery-dl) - Command-line program to download image galleries and collections from several image hosting sites.
   - Updated on `Sat 06 June 2026 at 08:32 AM` | 18474⭐ | `GNU General Public License v2.0`
- [OpenBB-finance/OpenBB](https://github.com/OpenBB-finance/OpenBB) - Financial data platform for analysts, quants and AI agents.
   - Updated on `Tue 09 June 2026 at 05:27 PM` | 68837⭐ | `Other`
- [unclecode/crawl4ai](https://github.com/unclecode/crawl4ai) - 🚀🤖 Crawl4AI: Open-source LLM Friendly Web Crawler & Scraper. Don't be shy, join here: https://discord.gg/jP8KfhDhyN.
   - Updated on `Thu 04 June 2026 at 06:19 AM` | 68155⭐ | `Apache License 2.0`
- [requests-cache/aiohttp-client-cache](https://github.com/requests-cache/aiohttp-client-cache) - An async persistent cache for aiohttp requests.
   - Updated on `Mon 08 June 2026 at 10:00 PM` | 151⭐ | `MIT License`
- [aio-libs/aiocache](https://github.com/aio-libs/aiocache) - Asyncio cache manager for redis, memcached and memory.
   - Updated on `Tue 26 May 2026 at 08:52 PM` | 1431⭐ | `BSD 3-Clause "New" or "Revised" License`
- [cupy/cupy](https://github.com/cupy/cupy) - NumPy & SciPy for GPU.
   - Updated on `Tue 09 June 2026 at 02:52 PM` | 10992⭐ | `MIT License`
- [cheahjs/free-llm-api-resources](https://github.com/cheahjs/free-llm-api-resources) - A list of free LLM inference resources accessible via API.
   - Updated on `Tue 09 June 2026 at 12:50 AM` | 23197⭐ | `❌`
- [squidfunk/mkdocs-material](https://github.com/squidfunk/mkdocs-material) - Documentation that simply works.
   - Updated on `Wed 13 May 2026 at 05:46 PM` | 26892⭐ | `MIT License`
- [sissbruecker/linkding](https://github.com/sissbruecker/linkding) - Self-hosted bookmark manager that is designed be to be minimal, fast, and easy to set up using Docker.
   - Updated on `Tue 07 April 2026 at 04:30 AM` | 10700⭐ | `MIT License`
- [Recol/DLSS-Updater](https://github.com/Recol/DLSS-Updater) - What if you could update all the games that are detected on your system?.
   - Updated on `Mon 08 June 2026 at 11:53 PM` | 940⭐ | `GNU Affero General Public License v3.0`
- [Cinnamon/kotaemon](https://github.com/Cinnamon/kotaemon) - An open-source RAG-based tool for chatting with your documents.
   - Updated on `Tue 09 June 2026 at 09:55 AM` | 25448⭐ | `Apache License 2.0`
- [LaurenceRawlings/savify](https://github.com/LaurenceRawlings/savify) - Download Spotify songs to mp3 with full metadata and cover art!.
   - Updated on `Thu 22 June 2023 at 09:46 AM` | 1362⭐ | `MIT License`
- [coleifer/huey](https://github.com/coleifer/huey) - a little task queue for python.
   - Updated on `Thu 14 May 2026 at 03:38 PM` | 5971⭐ | `MIT License`
- [opendatalab/MinerU](https://github.com/opendatalab/MinerU) - Transforms complex documents like PDFs and Office docs into LLM-ready markdown/JSON for your Agentic workflows.
   - Updated on `Mon 08 June 2026 at 03:16 PM` | 67019⭐ | `Other`
- [SWE-agent/SWE-agent](https://github.com/SWE-agent/SWE-agent) - SWE-agent takes a GitHub issue and tries to automatically fix it, using your LM of choice. It can also be employed for offensive cybersecurity or competitive coding challenges. [NeurIPS 2024].
   - Updated on `Mon 08 June 2026 at 09:35 PM` | 19466⭐ | `MIT License`
- [hacksider/Deep-Live-Cam](https://github.com/hacksider/Deep-Live-Cam) - real time face swap and one-click video deepfake with only a single image.
   - Updated on `Sun 31 May 2026 at 04:50 PM` | 93687⭐ | `GNU Affero General Public License v3.0`
- [BerriAI/litellm](https://github.com/BerriAI/litellm) - Python SDK, Proxy Server (AI Gateway) to call 100+ LLM APIs in OpenAI (or native) format, with cost tracking, guardrails, loadbalancing and logging. [Bedrock, Azure, OpenAI, VertexAI, Cohere, Anthropi.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 49819⭐ | `Other`
- [AgentOps-AI/tokencost](https://github.com/AgentOps-AI/tokencost) - Easy token price estimates for 400+ LLMs. TokenOps.
   - Updated on `Fri 05 September 2025 at 08:10 AM` | 1983⭐ | `MIT License`
- [ranahaani/GNews](https://github.com/ranahaani/GNews) - A Happy and lightweight Python Package that Provides an API to search for articles on Google News and returns a JSON response.
   - Updated on `Fri 16 January 2026 at 12:39 PM` | 971⭐ | `MIT License`
- [IceWreck/tagpatch](https://github.com/IceWreck/tagpatch) - CLI tool which applies common patches to music tags.
   - Updated on `Thu 01 January 2026 at 02:23 PM` | 21⭐ | `MIT License`
- [mem0ai/mem0](https://github.com/mem0ai/mem0) - Universal memory layer for AI Agents.
   - Updated on `Tue 09 June 2026 at 07:34 PM` | 58201⭐ | `Apache License 2.0`
- [entropy-research/Devon](https://github.com/entropy-research/Devon) - Devon: An open-source pair programmer.
   - Updated on `Mon 26 May 2025 at 05:47 AM` | 3448⭐ | `GNU Affero General Public License v3.0`
- [apify/crawlee-python](https://github.com/apify/crawlee-python) - Crawlee—A web scraping and browser automation library for Python to build reliable crawlers. Extract data for AI, LLMs, RAG, or GPTs. Download HTML, PDF, JPG, PNG, and other files from websites. Works.
   - Updated on `Tue 09 June 2026 at 02:20 PM` | 9151⭐ | `Apache License 2.0`
- [Aider-AI/aider](https://github.com/Aider-AI/aider) - aider is AI pair programming in your terminal.
   - Updated on `Fri 22 May 2026 at 02:02 PM` | 45940⭐ | `Apache License 2.0`
- [need4swede/Portall](https://github.com/need4swede/Portall) - Port Management Interface.
   - Updated on `Sat 21 June 2025 at 05:17 AM` | 838⭐ | `MIT License`
- [litestar-org/litestar](https://github.com/litestar-org/litestar) - Light, flexible and extensible ASGI framework | Built to scale.
   - Updated on `Tue 09 June 2026 at 09:51 PM` | 8273⭐ | `MIT License`
- [fabriziosalmi/UglyFeed](https://github.com/fabriziosalmi/UglyFeed) - Retrieve, aggregate, filter, evaluate, rewrite and serve RSS feeds using Large Language Models for fun, research and learning purposes.
   - Updated on `Sun 26 April 2026 at 09:38 PM` | 312⭐ | `GNU Affero General Public License v3.0`
- [ScrapeGraphAI/Scrapegraph-ai](https://github.com/ScrapeGraphAI/Scrapegraph-ai) - Python scraper based on AI.
   - Updated on `Tue 09 June 2026 at 07:28 AM` | 26990⭐ | `MIT License`
- [datalab-to/marker](https://github.com/datalab-to/marker) - Convert PDF to markdown + JSON quickly with high accuracy.
   - Updated on `Sat 06 June 2026 at 09:20 PM` | 35935⭐ | `GNU General Public License v3.0`
- [datalab-to/surya](https://github.com/datalab-to/surya) - OCR, layout analysis, reading order, table recognition in 90+ languages.
   - Updated on `Tue 02 June 2026 at 04:50 PM` | 20746⭐ | `Apache License 2.0`
- [khoj-ai/khoj](https://github.com/khoj-ai/khoj) - Your AI second brain. Self-hostable. Get answers from the web or your docs. Build custom agents, schedule automations, do deep research. Turn any online or local LLM into your personal, autonomous AI .
   - Updated on `Thu 26 March 2026 at 03:35 AM` | 35036⭐ | `GNU Affero General Public License v3.0`
- [bleachbit/bleachbit](https://github.com/bleachbit/bleachbit) - BleachBit system cleaner for Windows and Linux.
   - Updated on `Tue 09 June 2026 at 11:01 PM` | 5858⭐ | `GNU General Public License v3.0`
- [bottlesdevs/Bottles](https://github.com/bottlesdevs/Bottles) - Run Windows software and games on Linux.
   - Updated on `Tue 09 June 2026 at 08:22 AM` | 8493⭐ | `GNU General Public License v3.0`
- [kivy/kivy](https://github.com/kivy/kivy) - Open source UI framework written in Python, running on Windows, Linux, macOS, Android and iOS.
   - Updated on `Sun 07 June 2026 at 09:07 PM` | 18954⭐ | `MIT License`
- [JPCERTCC/LogonTracer](https://github.com/JPCERTCC/LogonTracer) - Investigate malicious Windows logon by visualizing and analyzing Windows event log.
   - Updated on `Wed 22 April 2026 at 03:13 PM` | 3179⭐ | `Other`
- [McGill-NLP/webllama](https://github.com/McGill-NLP/webllama) - Llama-3 agents that can browse the web by following instructions and talking to you.
   - Updated on `Tue 10 December 2024 at 04:44 PM` | 1402⭐ | `MIT License`
- [bhimrazy/receipt-ocr](https://github.com/bhimrazy/receipt-ocr) - An efficient OCR engine for receipt image processing.
   - Updated on `Sun 07 June 2026 at 07:42 PM` | 343⭐ | `MIT License`
- [hiyouga/LlamaFactory](https://github.com/hiyouga/LlamaFactory) - Unified Efficient Fine-Tuning of 100+ LLMs & VLMs (ACL 2024).
   - Updated on `Mon 08 June 2026 at 05:01 PM` | 72032⭐ | `Apache License 2.0`
- [element-hq/synapse](https://github.com/element-hq/synapse) - Synapse: Matrix homeserver written in Python/Twisted + Rust.
   - Updated on `Tue 09 June 2026 at 10:52 PM` | 4278⭐ | `GNU Affero General Public License v3.0`
- [Lightning-AI/litgpt](https://github.com/Lightning-AI/litgpt) - 20+ high-performance LLMs with recipes to pretrain, finetune and deploy at scale.
   - Updated on `Tue 09 June 2026 at 03:04 PM` | 13416⭐ | `Apache License 2.0`
- [nashsu/FreeAskInternet](https://github.com/nashsu/FreeAskInternet) - FreeAskInternet is a completely free, PRIVATE and LOCALLY running search aggregator & answer generate using MULTI LLMs, without GPU needed. The user can ask a question and the system will  make a mult.
   - Updated on `Thu 18 April 2024 at 06:46 AM` | 8731⭐ | `Apache License 2.0`
- [sympy/sympy](https://github.com/sympy/sympy) - A computer algebra system written in pure Python.
   - Updated on `Tue 09 June 2026 at 04:03 PM` | 14673⭐ | `Other`
- [stitionai/devika](https://github.com/stitionai/devika) - Devika is the first open-source implementation of an Agentic Software Engineer. Initially started as an open-source alternative to Devin.
   - Updated on `Thu 25 September 2025 at 08:47 AM` | 19510⭐ | `MIT License`
- [OpenHands/OpenHands](https://github.com/OpenHands/OpenHands) - 🙌 OpenHands: AI-Driven Development.
   - Updated on `Tue 09 June 2026 at 10:50 PM` | 76331⭐ | `Other`
- [hpcaitech/Open-Sora](https://github.com/hpcaitech/Open-Sora) - Open-Sora: Democratizing Efficient Video Production for All.
   - Updated on `Thu 09 April 2026 at 01:52 AM` | 29082⭐ | `Apache License 2.0`
- [bunkerity/bunkerweb](https://github.com/bunkerity/bunkerweb) - 🛡️ Open-source and cloud-native Web Application Firewall (WAF).
   - Updated on `Tue 09 June 2026 at 07:03 AM` | 10596⭐ | `GNU Affero General Public License v3.0`
- [mitmproxy/mitmproxy](https://github.com/mitmproxy/mitmproxy) - An interactive TLS-capable intercepting HTTP proxy for penetration testers and software developers.
   - Updated on `Fri 05 June 2026 at 02:02 PM` | 43849⭐ | `MIT License`
- [pyinstaller/pyinstaller](https://github.com/pyinstaller/pyinstaller) - Freeze (package) Python programs into stand-alone executables.
   - Updated on `Sun 07 June 2026 at 08:23 PM` | 13011⭐ | `Other`
- [arrow-py/arrow](https://github.com/arrow-py/arrow) - 🏹 Better dates & times for Python.
   - Updated on `Mon 08 June 2026 at 02:13 AM` | 9045⭐ | `Apache License 2.0`
- [pallets-eco/flask-admin](https://github.com/pallets-eco/flask-admin) - Simple and extensible administrative interface framework for Flask.
   - Updated on `Mon 01 June 2026 at 10:43 PM` | 6071⭐ | `BSD 3-Clause "New" or "Revised" License`
- [sherlock-project/sherlock](https://github.com/sherlock-project/sherlock) - Hunt down social media accounts by username across social networks.
   - Updated on `Tue 09 June 2026 at 06:28 AM` | 84787⭐ | `MIT License`
- [pallets-eco/flask-sqlalchemy](https://github.com/pallets-eco/flask-sqlalchemy) - Adds SQLAlchemy support to Flask.
   - Updated on `Mon 18 May 2026 at 11:27 PM` | 4311⭐ | `BSD 3-Clause "New" or "Revised" License`
- [agno-agi/agno](https://github.com/agno-agi/agno) - Build, run, and manage agent platforms.
   - Updated on `Tue 09 June 2026 at 11:22 PM` | 40608⭐ | `Apache License 2.0`
- [Unmanic/unmanic](https://github.com/Unmanic/unmanic) - Unmanic - Library Optimiser.
   - Updated on `Tue 09 June 2026 at 05:47 AM` | 2313⭐ | `GNU General Public License v3.0`
- [FujiwaraChoki/MoneyPrinter](https://github.com/FujiwaraChoki/MoneyPrinter) - Automate Creation of YouTube Shorts using MoviePy.
   - Updated on `Thu 26 March 2026 at 10:17 PM` | 13386⭐ | `MIT License`
- [pyveci/verlib2](https://github.com/pyveci/verlib2) - Standalone variant of "distutils.version" and "packaging.version", without anything else.
   - Updated on `Mon 08 June 2026 at 08:42 PM` | 2⭐ | `BSD 2-Clause "Simplified" License`
- [granawkins/rawdog](https://github.com/granawkins/rawdog) - Generate and auto-execute Python scripts in the cli.
   - Updated on `Mon 18 August 2025 at 05:31 AM` | 1805⭐ | `Apache License 2.0`
- [Kludex/uvicorn](https://github.com/Kludex/uvicorn) - An ASGI web server, for Python. 🦄.
   - Updated on `Tue 09 June 2026 at 09:56 PM` | 10747⭐ | `BSD 3-Clause "New" or "Revised" License`
- [benoitc/gunicorn](https://github.com/benoitc/gunicorn) - gunicorn 'Green Unicorn' is a WSGI HTTP Server for UNIX, fast clients and sleepy applications.
   - Updated on `Tue 05 May 2026 at 09:30 AM` | 10597⭐ | `Other`
- [abersheeran/zibai](https://github.com/abersheeran/zibai) - A modern high-performance pure-Python WSGI server.
   - Updated on `Fri 24 April 2026 at 12:54 PM` | 63⭐ | `Apache License 2.0`
- [instantX-research/InstantID](https://github.com/instantX-research/InstantID) - InstantID: Zero-shot Identity-Preserving Generation in Seconds 🔥.
   - Updated on `Thu 18 July 2024 at 04:12 AM` | 11957⭐ | `Apache License 2.0`
- [ArchiveBox/ArchiveBox](https://github.com/ArchiveBox/ArchiveBox) - 🗃 Open source self-hosted web archiving. Takes URLs/browser history/bookmarks/Pocket/Pinboard/etc., saves HTML, JS, PDFs, media, and more...
   - Updated on `Tue 09 June 2026 at 06:47 AM` | 27670⭐ | `MIT License`
- [Tecnativa/docker-socket-proxy](https://github.com/Tecnativa/docker-socket-proxy) - Proxy over your Docker socket to restrict which requests it accepts.
   - Updated on `Wed 25 February 2026 at 08:15 AM` | 2553⭐ | `Apache License 2.0`
- [fastapi/fastapi](https://github.com/fastapi/fastapi) - FastAPI framework, high performance, easy to learn, fast to code, ready for production.
   - Updated on `Tue 09 June 2026 at 10:07 PM` | 99030⭐ | `MIT License`
- [mljar/mercury](https://github.com/mljar/mercury) - Create web apps from Python notebooks.
   - Updated on `Tue 09 June 2026 at 02:40 PM` | 4330⭐ | `Apache License 2.0`
- [jzhang38/TinyLlama](https://github.com/jzhang38/TinyLlama) - The TinyLlama project is an open endeavor to pretrain a 1.1B Llama model on 3 trillion tokens.
   - Updated on `Fri 03 May 2024 at 08:21 PM` | 8983⭐ | `Apache License 2.0`
- [rohankishore/Aura-Text](https://github.com/rohankishore/Aura-Text) - Aura Text is a versatile and powerful text editor powered by QScintilla that provides all the necessary tools for developers. It is build using PyQt6 and Python.
   - Updated on `Tue 09 June 2026 at 01:57 PM` | 347⭐ | `GNU General Public License v3.0`
- [zai-org/CodeGeeX](https://github.com/zai-org/CodeGeeX) - CodeGeeX: An Open Multilingual Code Generation Model (KDD 2023).
   - Updated on `Tue 13 August 2024 at 05:59 AM` | 8791⭐ | `Apache License 2.0`
- [JoeanAmier/TikTokDownloader](https://github.com/JoeanAmier/TikTokDownloader) - TikTok 发布/喜欢/合辑/直播/视频/图集/音乐；抖音发布/喜欢/收藏/收藏夹/视频/图集/实况/直播/音乐/合集/评论/账号/搜索/热榜数据采集工具/下载工具.
   - Updated on `Sun 07 June 2026 at 04:26 AM` | 14729⭐ | `GNU General Public License v3.0`
- [pypdfium2-team/pypdfium2](https://github.com/pypdfium2-team/pypdfium2) - Python bindings to PDFium, reasonably cross-platform.
   - Updated on `Tue 09 June 2026 at 10:51 PM` | 778⭐ | `❌`
- [open-webui/open-webui](https://github.com/open-webui/open-webui) - User-friendly AI Interface (Supports Ollama, OpenAI API, ...).
   - Updated on `Mon 08 June 2026 at 09:52 PM` | 140858⭐ | `Other`
- [google-gemini/deprecated-generative-ai-python](https://github.com/google-gemini/deprecated-generative-ai-python) - This SDK is now deprecated, use the new unified Google GenAI SDK.
   - Updated on `Tue 16 December 2025 at 05:45 PM` | 2311⭐ | `Apache License 2.0`
- [coqui-ai/TTS](https://github.com/coqui-ai/TTS) - 🐸💬 - a deep learning toolkit for Text-to-Speech, battle-tested in research and production.
   - Updated on `Fri 16 August 2024 at 12:07 PM` | 45518⭐ | `Mozilla Public License 2.0`
- [unslothai/unsloth](https://github.com/unslothai/unsloth) - Unsloth Studio is a web UI for training and running open models like Gemma 4, Qwen3.6, DeepSeek, gpt-oss locally.
   - Updated on `Tue 09 June 2026 at 08:43 PM` | 66142⭐ | `Apache License 2.0`
- [OthersideAI/self-operating-computer](https://github.com/OthersideAI/self-operating-computer) - A framework to enable a multimodal model to operate a computer.
   - Updated on `Fri 19 September 2025 at 07:31 PM` | 10247⭐ | `MIT License`
- [run-llama/rags](https://github.com/run-llama/rags) - Build ChatGPT over your data, all with natural language.
   - Updated on `Fri 05 April 2024 at 05:36 AM` | 6544⭐ | `MIT License`
- [QuivrHQ/quivr](https://github.com/QuivrHQ/quivr) - Opiniated RAG for integrating GenAI in your apps 🧠   Focus on your product rather than the RAG. Easy integration in existing products with customisation!  Any LLM: GPT4, Groq, Llama. Any Vectorstore: .
   - Updated on `Wed 09 July 2025 at 12:55 PM` | 39152⭐ | `Other`
- [owasp-dep-scan/dep-scan](https://github.com/owasp-dep-scan/dep-scan) - OWASP dep-scan is a next-generation security and risk audit tool based on known vulnerabilities, advisories, and license limitations for project dependencies. Both local repositories and container ima.
   - Updated on `Wed 27 May 2026 at 02:53 PM` | 1244⭐ | `MIT License`
- [instaloader/instaloader](https://github.com/instaloader/instaloader) - Download pictures (or videos) along with their captions and other metadata from Instagram.
   - Updated on `Wed 15 April 2026 at 07:26 PM` | 12535⭐ | `MIT License`
- [deepseek-ai/DeepSeek-Coder](https://github.com/deepseek-ai/DeepSeek-Coder) - DeepSeek Coder: Let the Code Write Itself.
   - Updated on `Tue 11 November 2025 at 06:44 AM` | 23654⭐ | `MIT License`
- [twitchtv/igdb-api-python](https://github.com/twitchtv/igdb-api-python) - Python wrapper for IGDB API version 4.
   - Updated on `Wed 17 July 2024 at 08:32 AM` | 104⭐ | `MIT License`
- [pgjones/hypercorn](https://github.com/pgjones/hypercorn) - Hypercorn is an ASGI and WSGI Server based on Hyper libraries and inspired by Gunicorn.
   - Updated on `Sat 08 November 2025 at 01:49 PM` | 1581⭐ | `MIT License`
- [microsoft/playwright-python](https://github.com/microsoft/playwright-python) - Python version of the Playwright testing and automation library.
   - Updated on `Tue 09 June 2026 at 12:53 PM` | 14737⭐ | `Apache License 2.0`
- [Datenlord1510/PYxREL](https://github.com/Datenlord1510/PYxREL) - A Python API wrapper for the official xREL API.
   - Updated on `Thu 16 May 2024 at 12:01 PM` | 2⭐ | `MIT License`
- [snowby666/poe-api-wrapper](https://github.com/snowby666/poe-api-wrapper) - 👾 A Python API wrapper for Poe.com. With this, you will have free access to GPT-4, Claude, Llama, Gemini, Mistral and more! 🚀.
   - Updated on `Sun 31 August 2025 at 05:29 AM` | 1106⭐ | `GNU General Public License v3.0`
- [ageitgey/face_recognition](https://github.com/ageitgey/face_recognition) - The world's simplest facial recognition api for Python and the command line.
   - Updated on `Wed 21 August 2024 at 06:22 AM` | 56473⭐ | `MIT License`
- [reflex-dev/reflex](https://github.com/reflex-dev/reflex) - 🕸️ Web apps in pure Python 🐍.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 28463⭐ | `Apache License 2.0`
- [openinterpreter/openinterpreter](https://github.com/openinterpreter/openinterpreter) - A natural language interface for computers.
   - Updated on `Tue 09 June 2026 at 10:32 PM` | 63851⭐ | `GNU Affero General Public License v3.0`
- [pyodide/pyodide](https://github.com/pyodide/pyodide) - Pyodide is a Python distribution for the browser and Node.js based on WebAssembly.
   - Updated on `Tue 09 June 2026 at 12:49 PM` | 14639⭐ | `Mozilla Public License 2.0`
- [huggingface/huggingface_hub](https://github.com/huggingface/huggingface_hub) - The official Python client for the Hugging Face Hub.
   - Updated on `Tue 09 June 2026 at 03:02 PM` | 3663⭐ | `Apache License 2.0`
- [omnilib/aiosqlite](https://github.com/omnilib/aiosqlite) - asyncio bridge to the standard sqlite3 module.
   - Updated on `Sun 01 March 2026 at 03:02 PM` | 1557⭐ | `MIT License`
- [h2oai/h2ogpt](https://github.com/h2oai/h2ogpt) - Private chat with local GPT with document, images, video, etc. 100% private, Apache 2.0. Supports oLLaMa, Mixtral, llama.cpp, and more. Demo: https://gpt.h2o.ai/ https://gpt-docs.h2o.ai/.
   - Updated on `Thu 09 October 2025 at 11:30 PM` | 11988⭐ | `Apache License 2.0`
- [bigscience-workshop/petals](https://github.com/bigscience-workshop/petals) - 🌸 Run LLMs at home, BitTorrent-style. Fine-tuning and inference up to 10x faster than offloading.
   - Updated on `Sat 07 September 2024 at 11:54 AM` | 10182⭐ | `MIT License`
- [Soulter/hugging-chat-api](https://github.com/Soulter/hugging-chat-api) - HuggingChat Python API🤗.
   - Updated on `Thu 12 June 2025 at 01:00 AM` | 941⭐ | `GNU Affero General Public License v3.0`
- [lm-sys/FastChat](https://github.com/lm-sys/FastChat) - An open platform for training, serving, and evaluating large language models. Release repo for Vicuna and Chatbot Arena.
   - Updated on `Fri 01 May 2026 at 12:25 AM` | 39469⭐ | `Apache License 2.0`
- [langchain-ai/langchain](https://github.com/langchain-ai/langchain) - The agent engineering platform.
   - Updated on `Wed 10 June 2026 at 12:29 AM` | 138900⭐ | `MIT License`
- [aio-libs/aiohttp](https://github.com/aio-libs/aiohttp) - Asynchronous HTTP client/server framework for asyncio and Python.
   - Updated on `Tue 09 June 2026 at 11:30 PM` | 16441⭐ | `Apache License 2.0`
- [openai/whisper](https://github.com/openai/whisper) - Robust Speech Recognition via Large-Scale Weak Supervision.
   - Updated on `Wed 15 April 2026 at 04:32 PM` | 102321⭐ | `MIT License`
- [oobabooga/textgen](https://github.com/oobabooga/textgen) - Open-source desktop app for local LLMs. Text, vision, tool-calling, OpenAI/Anthropic-compatible API. 100% private.
   - Updated on `Tue 02 June 2026 at 03:10 AM` | 47286⭐ | `GNU Affero General Public License v3.0`
- [meta-llama/llama](https://github.com/meta-llama/llama) - Inference code for Llama models.
   - Updated on `Sun 26 January 2025 at 09:42 PM` | 59452⭐ | `Other`
- [danielgross/localpilot](https://github.com/danielgross/localpilot) - .
   - Updated on `Sun 25 February 2024 at 06:19 PM` | 3361⭐ | `MIT License`
- [PabloLec/RecoverPy](https://github.com/PabloLec/RecoverPy) - Interactively find and recover deleted or :point_right: overwritten :point_left: files from your terminal.
   - Updated on `Wed 20 May 2026 at 12:03 AM` | 1765⭐ | `GNU General Public License v3.0`
- [saleguas/slushie](https://github.com/saleguas/slushie) - Relative paths according to the CURRENT FILE, not the current shell location.
   - Updated on `Thu 11 April 2024 at 06:29 AM` | 23⭐ | `MIT License`
- [TencentARC/GFPGAN](https://github.com/TencentARC/GFPGAN) - GFPGAN aims at developing Practical Algorithms for Real-world Face Restoration.
   - Updated on `Fri 26 July 2024 at 06:44 PM` | 37468⭐ | `Other`
- [loganmarchione/homelab-svg-assets](https://github.com/loganmarchione/homelab-svg-assets) - Full-color SVG icons of homelab-related software, products, and brands.
   - Updated on `Thu 04 June 2026 at 04:36 AM` | 1222⭐ | `MIT License`
- [automatic-ripping-machine/automatic-ripping-machine](https://github.com/automatic-ripping-machine/automatic-ripping-machine) - Automatic Ripping Machine (ARM) Scripts.
   - Updated on `Fri 22 May 2026 at 09:33 PM` | 4851⭐ | `MIT License`
- [miguelgrinberg/Flask-SocketIO](https://github.com/miguelgrinberg/Flask-SocketIO) - Socket.IO integration for Flask applications.
   - Updated on `Thu 14 May 2026 at 10:21 PM` | 5512⭐ | `MIT License`
- [pallets/quart](https://github.com/pallets/quart) - An async Python micro framework for building web applications.
   - Updated on `Mon 01 September 2025 at 06:49 PM` | 3638⭐ | `MIT License`
- [xtekky/gpt4free](https://github.com/xtekky/gpt4free) - The official gpt4free repository | various collection of powerful language models | opus 4.6 gpt 5.3 kimi 2.5 deepseek v3.2 gemini 3.
   - Updated on `Tue 09 June 2026 at 09:39 PM` | 66306⭐ | `GNU General Public License v3.0`
- [arc53/DocsGPT](https://github.com/arc53/DocsGPT) - Private AI platform for agents, assistants and enterprise search. Built-in Agent Builder, Deep research, Document analysis, Multi-model support, and API connectivity for agents.
   - Updated on `Tue 09 June 2026 at 11:57 PM` | 17923⭐ | `MIT License`
- [vanessamcf/login_system](https://github.com/vanessamcf/login_system) - Login system, the user is able to Sign up and Sign in with email, Reset password or login with Github, Google, or Facebook. Integration of flask-login, flask-dance and flask-mail.
   - Updated on `Fri 05 January 2024 at 03:10 AM` | 50⭐ | `MIT License`
- [maxcountryman/flask-login](https://github.com/maxcountryman/flask-login) - Flask user session management.
   - Updated on `Wed 27 August 2025 at 03:00 AM` | 3677⭐ | `MIT License`
- [sns-sdks/python-youtube](https://github.com/sns-sdks/python-youtube) - A simple Python wrapper for YouTube Data API :sparkles: :cake: :sparkles: .
   - Updated on `Fri 17 April 2026 at 09:33 AM` | 352⭐ | `MIT License`
- [CorentinJ/Real-Time-Voice-Cloning](https://github.com/CorentinJ/Real-Time-Voice-Cloning) - Clone a voice in 5 seconds to generate arbitrary speech in real-time.
   - Updated on `Mon 09 March 2026 at 10:31 AM` | 59904⭐ | `Other`
- [krateng/maloja](https://github.com/krateng/maloja) - Self-hosted music scrobble database to create personal listening statistics and charts.
   - Updated on `Sun 18 May 2025 at 12:08 PM` | 1737⭐ | `GNU General Public License v3.0`
- [ZacharyHampton/HomeHarvest](https://github.com/ZacharyHampton/HomeHarvest) - Python package for scraping real estate property data.
   - Updated on `Fri 26 December 2025 at 07:33 AM` | 697⭐ | `MIT License`
- [commaai/openpilot](https://github.com/commaai/openpilot) - openpilot is an operating system for robotics. Currently, it upgrades the driver assistance system on 300+ supported cars.
   - Updated on `Tue 09 June 2026 at 11:56 PM` | 61326⭐ | `MIT License`
- [JaidedAI/EasyOCR](https://github.com/JaidedAI/EasyOCR) - Ready-to-use OCR with 80+ supported languages and all popular writing scripts including Latin, Chinese, Arabic, Devanagari, Cyrillic and etc.
   - Updated on `Fri 05 December 2025 at 12:23 PM` | 29586⭐ | `Apache License 2.0`
- [sirfz/tesserocr](https://github.com/sirfz/tesserocr) - A Python wrapper for the tesseract-ocr API.
   - Updated on `Mon 16 March 2026 at 02:37 PM` | 2168⭐ | `MIT License`
- [GBMiro/Back-Up-Trakt-for-Me](https://github.com/GBMiro/Back-Up-Trakt-for-Me) - Python application to back up your Trakt data and check your watched history.
   - Updated on `Fri 07 February 2025 at 06:13 PM` | 8⭐ | `MIT License`
- [vsakkas/sydney.py](https://github.com/vsakkas/sydney.py) - Python Client for Copilot (formerly named Bing Chat), also known as Sydney.
   - Updated on `Fri 17 January 2025 at 09:20 PM` | 293⭐ | `MIT License`
- [dsdanielpark/Bard-API](https://github.com/dsdanielpark/Bard-API) - The unofficial python package that returns response of Google Bard through cookie value.
   - Updated on `Wed 24 April 2024 at 10:38 AM` | 5196⭐ | `MIT License`
- [Pythagora-io/gpt-pilot](https://github.com/Pythagora-io/gpt-pilot) - The first real AI developer.
   - Updated on `Mon 08 June 2026 at 11:13 AM` | 33743⭐ | `Other`
- [db0/pythorhead](https://github.com/db0/pythorhead) - A python library for interacting with Lemmy.
   - Updated on `Thu 16 October 2025 at 12:37 PM` | 91⭐ | `GNU Affero General Public License v3.0`
- [maxmind/GeoIP2-python](https://github.com/maxmind/GeoIP2-python) - Python code for GeoIP2 webservice client and database reader.
   - Updated on `Tue 09 June 2026 at 03:45 PM` | 1213⭐ | `Apache License 2.0`
- [brandon-wallace/simpleweatherreport](https://github.com/brandon-wallace/simpleweatherreport) - Get the Weather Report with Python and Flask using Geolocation.
   - Updated on `Thu 16 February 2023 at 01:30 AM` | 7⭐ | `❌`
- [geopy/geopy](https://github.com/geopy/geopy) - Geocoding library for Python.
   - Updated on `Tue 27 January 2026 at 07:22 PM` | 4820⭐ | `MIT License`
- [facefusion/facefusion](https://github.com/facefusion/facefusion) - Industry leading face manipulation platform.
   - Updated on `Tue 09 June 2026 at 08:30 PM` | 28745⭐ | `Other`
- [microsoft/Bringing-Old-Photos-Back-to-Life](https://github.com/microsoft/Bringing-Old-Photos-Back-to-Life) - Bringing Old Photo Back to Life (CVPR 2020 oral).
   - Updated on `Thu 26 October 2023 at 08:06 AM` | 15686⭐ | `MIT License`
- [joshuaboniface/remote-faster-whisper](https://github.com/joshuaboniface/remote-faster-whisper) - A basic HTTP API for handling Faster Whisper audio transcriptions over the network.
   - Updated on `Tue 29 July 2025 at 04:44 PM` | 34⭐ | `GNU General Public License v3.0`
- [AUTOMATIC1111/stable-diffusion-webui](https://github.com/AUTOMATIC1111/stable-diffusion-webui) - Stable Diffusion web UI.
   - Updated on `Mon 02 March 2026 at 07:00 AM` | 163584⭐ | `GNU Affero General Public License v3.0`
- [AntonOsika/gpt-engineer](https://github.com/AntonOsika/gpt-engineer) - CLI platform to experiment with codegen. Precursor to: https://lovable.dev.
   - Updated on `Wed 14 May 2025 at 10:15 AM` | 55199⭐ | `MIT License`
- [princeton-vl/infinigen](https://github.com/princeton-vl/infinigen) - Infinite Photorealistic Worlds using Procedural Generation.
   - Updated on `Tue 19 May 2026 at 12:50 PM` | 7002⭐ | `BSD 3-Clause "New" or "Revised" License`
- [reactive-python/reactpy](https://github.com/reactive-python/reactpy) - It's React, but in Python.
   - Updated on `Sat 18 April 2026 at 11:42 PM` | 8138⭐ | `MIT License`
- [w-okada/voice-changer](https://github.com/w-okada/voice-changer) - リアルタイムボイスチェンジャー Realtime Voice Changer.
   - Updated on `Sat 21 March 2026 at 03:52 AM` | 20387⭐ | `Other`
- [Casvt/Kapowarr](https://github.com/Casvt/Kapowarr) - Kapowarr is a software to build and manage a comic book library, fitting in the *arr suite of software.
   - Updated on `Tue 26 May 2026 at 11:42 PM` | 990⭐ | `GNU General Public License v3.0`
- [maschere/pyjop](https://github.com/maschere/pyjop) - Official JOY OF PROGRAMMING Python Interface. https://store.steampowered.com/app/2216770.
   - Updated on `Sun 19 January 2025 at 12:03 PM` | 81⭐ | `MIT License`
- [MustardChef/WSABuilds](https://github.com/MustardChef/WSABuilds) - Run Windows Subsystem For Android on your Windows 10 and Windows 11 PC using prebuilt binaries with Google Play Store (MindTheGapps) and/or Magisk or KernelSU (root solutions) built in.
   - Updated on `Mon 01 June 2026 at 12:08 PM` | 17556⭐ | `GNU Affero General Public License v3.0`
- [opennaslab/kubespider](https://github.com/opennaslab/kubespider) - A global resource download orchestration system, build your home download center.
   - Updated on `Tue 14 April 2026 at 12:33 AM` | 2189⭐ | `Apache License 2.0`
- [zylon-ai/private-gpt](https://github.com/zylon-ai/private-gpt) - Interact with your documents using the power of GPT, 100% privately, no data leaks.
   - Updated on `Tue 09 June 2026 at 03:55 PM` | 57214⭐ | `Apache License 2.0`
- [GreyDGL/PentestGPT](https://github.com/GreyDGL/PentestGPT) - Automated Penetration Testing Agentic Framework Powered by Large Language Models.
   - Updated on `Sun 07 June 2026 at 07:25 AM` | 13620⭐ | `MIT License`
- [gaomingqi/Track-Anything](https://github.com/gaomingqi/Track-Anything) - Track-Anything is a flexible and interactive tool for video object tracking and segmentation, based on Segment Anything, XMem, and E2FGVI.
   - Updated on `Sat 13 December 2025 at 11:02 AM` | 6962⭐ | `MIT License`
- [lissy93/portainer-templates](https://github.com/lissy93/portainer-templates) - 🚢 500+ 1-click Portainer app templates.
   - Updated on `Sun 10 May 2026 at 08:06 PM` | 2847⭐ | `MIT License`
- [gjcarneiro/yacron](https://github.com/gjcarneiro/yacron) - A modern Cron replacement that is Docker-friendly.
   - Updated on `Mon 20 October 2025 at 09:43 AM` | 470⭐ | `MIT License`
- [LAION-AI/Open-Assistant](https://github.com/LAION-AI/Open-Assistant) - OpenAssistant is a chat-based assistant that understands tasks, can interact with third-party systems, and retrieve information dynamically to do so.
   - Updated on `Sat 17 August 2024 at 01:55 AM` | 37400⭐ | `Apache License 2.0`
- [Significant-Gravitas/AutoGPT](https://github.com/Significant-Gravitas/AutoGPT) - AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to provide the tools, so that you can focus on what matters.
   - Updated on `Tue 09 June 2026 at 11:53 PM` | 184860⭐ | `Other`
- [anasty17/mirror-leech-telegram-bot](https://github.com/anasty17/mirror-leech-telegram-bot) - Official Repository: Telegram bot which can download direct links, torrents, nzb, google drive, telegram document, any file/folder from rclone supported clouds, all yt-dlp supported sites and jdownloa.
   - Updated on `Sun 07 June 2026 at 10:04 PM` | 4189⭐ | `GNU General Public License v3.0`
- [offa/android-foss](https://github.com/offa/android-foss) - A list of Free and Open Source Software (FOSS) for Android – saving Freedom and Privacy.
   - Updated on `Mon 08 June 2026 at 07:20 PM` | 10314⭐ | `GNU General Public License v3.0`
- [mdhiggins/PlexAutoSkip](https://github.com/mdhiggins/PlexAutoSkip) - Automatically skip content in Plex.
   - Updated on `Tue 30 September 2025 at 11:05 PM` | 221⭐ | `MIT License`
- [beetbox/beets](https://github.com/beetbox/beets) - music library manager and MusicBrainz tagger.
   - Updated on `Tue 09 June 2026 at 08:24 AM` | 15245⭐ | `MIT License`
- [rtts/djhtml](https://github.com/rtts/djhtml) - Django/Jinja template indenter.
   - Updated on `Fri 17 April 2026 at 02:26 PM` | 641⭐ | `GNU General Public License v3.0`
- [ipinfo/python](https://github.com/ipinfo/python) - Official Python Library for IPinfo API (IP geolocation and other types of IP data).
   - Updated on `Wed 03 June 2026 at 11:33 PM` | 560⭐ | `Apache License 2.0`
- [christopherpickering/flask-session2](https://github.com/christopherpickering/flask-session2) - Server side session extension for Flask.
   - Updated on `Tue 12 May 2026 at 09:48 AM` | 32⭐ | `Other`
- [AzeemIdrisi/PhoneSploit-Pro](https://github.com/AzeemIdrisi/PhoneSploit-Pro) - An all-in-one hacking tool to remotely exploit Android devices using ADB and Metasploit-Framework to get a Meterpreter session.
   - Updated on `Sun 31 May 2026 at 02:05 PM` | 5908⭐ | `GNU General Public License v3.0`
- [viniciuschiele/flask-apscheduler](https://github.com/viniciuschiele/flask-apscheduler) - Adds APScheduler support to Flask.
   - Updated on `Tue 07 November 2023 at 12:46 PM` | 1140⭐ | `Apache License 2.0`
- [CollinHeist/TitleCardMaker](https://github.com/CollinHeist/TitleCardMaker) - Automated title card maker for Plex, Jellyfin, and Emby.
   - Updated on `Sun 07 June 2026 at 03:23 AM` | 823⭐ | `GNU General Public License v3.0`
- [dchevell/flask-executor](https://github.com/dchevell/flask-executor) - Adds concurrent.futures support to Flask.
   - Updated on `Sat 14 January 2023 at 10:01 PM` | 179⭐ | `MIT License`
- [pallets-eco/flask-caching](https://github.com/pallets-eco/flask-caching) - A caching extension for Flask.
   - Updated on `Sun 31 May 2026 at 08:12 PM` | 935⭐ | `Other`
- [dwolfhub/zxcvbn-python](https://github.com/dwolfhub/zxcvbn-python) - Python implementation of Dropbox's realistic password strength estimator.
   - Updated on `Mon 13 April 2026 at 04:52 PM` | 715⭐ | `MIT License`
- [flask-api/flask-api](https://github.com/flask-api/flask-api) - Browsable web APIs for Flask.
   - Updated on `Tue 13 May 2025 at 06:12 AM` | 1472⭐ | `Other`
- [metabrainz/picard](https://github.com/metabrainz/picard) - Picard is a cross-platform music tagger powered by the MusicBrainz database.
   - Updated on `Tue 09 June 2026 at 01:12 PM` | 4933⭐ | `GNU General Public License v2.0`
- [mikeabrahamsen/Flask-Meld](https://github.com/mikeabrahamsen/Flask-Meld) - Flask-Meld is a library to provide server rendered templates over websockets for Flask applications to build reactive components without Javascript.
   - Updated on `Sat 22 January 2022 at 08:57 PM` | 320⭐ | `MIT License`
- [alisaifee/flask-limiter](https://github.com/alisaifee/flask-limiter) - Rate Limiting extension for Flask.
   - Updated on `Sun 10 May 2026 at 11:07 PM` | 1202⭐ | `MIT License`
- [pallets-eco/flask-wtf](https://github.com/pallets-eco/flask-wtf) - Simple integration of Flask and WTForms, including CSRF, file upload and Recaptcha integration.
   - Updated on `Wed 03 June 2026 at 06:57 AM` | 1507⭐ | `BSD 3-Clause "New" or "Revised" License`
- [python-restx/flask-restx](https://github.com/python-restx/flask-restx) - Fork of Flask-RESTPlus: Fully featured framework for fast, easy and documented API development with Flask.
   - Updated on `Tue 14 April 2026 at 03:43 PM` | 2233⭐ | `Other`
- [pallets/flask](https://github.com/pallets/flask) - The Python micro framework for building web applications.
   - Updated on `Sun 31 May 2026 at 02:42 PM` | 71629⭐ | `BSD 3-Clause "New" or "Revised" License`
- [waynerv/flask-mailman](https://github.com/waynerv/flask-mailman) - Porting Django's email implementation to your Flask applications.
   - Updated on `Wed 04 September 2024 at 12:14 AM` | 128⭐ | `BSD 3-Clause "New" or "Revised" License`
- [corydolphin/flask-cors](https://github.com/corydolphin/flask-cors) - Cross Origin Resource Sharing ( CORS ) support for Flask.
   - Updated on `Tue 09 June 2026 at 01:52 AM` | 932⭐ | `MIT License`
- [singingwolfboy/flask-dance](https://github.com/singingwolfboy/flask-dance) - Doing the OAuth dance with style using Flask, requests, and oauthlib.
   - Updated on `Fri 07 June 2024 at 07:10 AM` | 1016⭐ | `MIT License`
- [pyauth/pyotp](https://github.com/pyauth/pyotp) - Python One-Time Password Library.
   - Updated on `Sat 03 January 2026 at 12:20 AM` | 3311⭐ | `Other`
- [samuelcolvin/watchfiles](https://github.com/samuelcolvin/watchfiles) - Simple, modern and fast file watching and code reload for Python, written in Rust.
   - Updated on `Mon 18 May 2026 at 04:32 AM` | 2502⭐ | `MIT License`
- [maubot/maubot](https://github.com/maubot/maubot) - A plugin-based Matrix bot system.
   - Updated on `Sun 10 May 2026 at 11:38 AM` | 883⭐ | `GNU Affero General Public License v3.0`
- [vaparr/pyoverseerr](https://github.com/vaparr/pyoverseerr) - Python interface to Overseerr.
   - Updated on `Sun 06 August 2023 at 01:28 PM` | 8⭐ | `MIT License`
- [imbev/simplematrixbotlib](https://github.com/imbev/simplematrixbotlib) - Development continued at  https://codeberg.org/imbev/simplematrixbotlib.
   - Updated on `Wed 22 May 2024 at 09:14 PM` | 77⭐ | `MIT License`
- [roflcoopter/viseron](https://github.com/roflcoopter/viseron) - Self-hosted, local only NVR and AI Computer Vision software.  With features such as object detection, motion detection, face recognition and more, it gives you the power to keep an eye on your home, o.
   - Updated on `Mon 08 June 2026 at 03:19 PM` | 3211⭐ | `MIT License`
- [netalertx/NetAlertX](https://github.com/netalertx/NetAlertX) - Centralized network visibility and continuous asset discovery.  Monitor devices, detect change, and stay aware across distributed networks.
   - Updated on `Thu 04 June 2026 at 03:58 AM` | 6529⭐ | `GNU General Public License v3.0`
- [sentriz/betanin](https://github.com/sentriz/betanin) - beets based mitm of your torrent client and music player.
   - Updated on `Mon 08 June 2026 at 02:49 PM` | 447⭐ | `GNU General Public License v3.0`
- [jellyfin/jellyfin-mpv-shim](https://github.com/jellyfin/jellyfin-mpv-shim) - MPV Cast Client for Jellyfin.
   - Updated on `Tue 09 June 2026 at 12:27 PM` | 2222⭐ | `Other`
- [TomSchimansky/CustomTkinter](https://github.com/TomSchimansky/CustomTkinter) - A modern and customizable python UI-library based on Tkinter.
   - Updated on `Tue 24 February 2026 at 09:08 PM` | 13393⭐ | `MIT License`
- [lennart-rth/Live-Earth-Wallpapers](https://github.com/lennart-rth/Live-Earth-Wallpapers) - A collection of all earth related space Images in one script to set as your Desktop background.
   - Updated on `Thu 03 July 2025 at 07:39 AM` | 326⭐ | `GNU General Public License v3.0`
- [lemon24/reader](https://github.com/lemon24/reader) - A Python feed reader library.
   - Updated on `Tue 26 May 2026 at 03:23 PM` | 540⭐ | `BSD 3-Clause "New" or "Revised" License`
- [msgspec/msgspec](https://github.com/msgspec/msgspec) - A fast serialization and validation library, with builtin support for JSON, MessagePack, YAML, and TOML.
   - Updated on `Sat 06 June 2026 at 07:15 PM` | 3796⭐ | `BSD 3-Clause "New" or "Revised" License`
- [Coder-128/AutoMSRewards](https://github.com/Coder-128/AutoMSRewards) - A python program which automatically searches for points on Microsoft Edge.
   - Updated on `Sun 09 May 2021 at 08:31 PM` | 22⭐ | `❌`
- [Miksus/rocketry](https://github.com/Miksus/rocketry) - Modern scheduling library for Python.
   - Updated on `Wed 15 November 2023 at 01:05 PM` | 3368⭐ | `MIT License`
- [wizarrrr/wizarr](https://github.com/wizarrrr/wizarr) - Wizarr is an advanced user invitation and management system for Jellyfin, Plex, Emby etc.
   - Updated on `Tue 09 June 2026 at 09:06 AM` | 2900⭐ | `MIT License`
- [behind24proxies/PixelHub](https://github.com/behind24proxies/PixelHub) - PixelHub, a place where you click on black boxes.
   - Updated on `Sun 18 September 2022 at 07:03 PM` | 184⭐ | `MIT License`
- [uysalserkan/Logges](https://github.com/uysalserkan/Logges) - An alternative logging tool working on Python, also have different functions.
   - Updated on `Fri 30 January 2026 at 04:20 PM` | 21⭐ | `MIT License`
- [wkentaro/gdown](https://github.com/wkentaro/gdown) - Google Drive public file downloader when curl/wget fails.
   - Updated on `Sat 30 May 2026 at 11:55 AM` | 5297⭐ | `MIT License`
- [spotDL/spotify-downloader](https://github.com/spotDL/spotify-downloader) - Download your Spotify playlists and songs along with album art and metadata (from YouTube if a match is found).
   - Updated on `Tue 19 May 2026 at 09:11 PM` | 25086⭐ | `MIT License`
- [RemiRigal/Plex-Auto-Languages](https://github.com/RemiRigal/Plex-Auto-Languages) - Automated language selection for Plex TV Shows !.
   - Updated on `Sun 30 November 2025 at 03:02 PM` | 752⭐ | `MIT License`
- [dbader/schedule](https://github.com/dbader/schedule) - Python job scheduling for humans.
   - Updated on `Sat 25 May 2024 at 06:41 PM` | 12247⭐ | `MIT License`
- [vinta/awesome-python](https://github.com/vinta/awesome-python) - An opinionated list of Python frameworks, libraries, tools, and resources.
   - Updated on `Tue 09 June 2026 at 05:31 AM` | 302125⭐ | `Other`
- [bugy/script-server](https://github.com/bugy/script-server) - Web UI for your scripts with execution management.
   - Updated on `Tue 24 March 2026 at 01:25 PM` | 1865⭐ | `Other`
- [MycroftAI/mimic3](https://github.com/MycroftAI/mimic3) - A fast local neural text to speech engine for Mycroft.
   - Updated on `Tue 25 March 2025 at 12:29 PM` | 1262⭐ | `GNU Affero General Public License v3.0`
- [Granitosaurus/sauth](https://github.com/Granitosaurus/sauth) - Simple http server with authentication.
   - Updated on `Fri 04 March 2022 at 12:27 PM` | 32⭐ | `Other`
- [Casvt/Plex-scripts](https://github.com/Casvt/Plex-scripts) - Plex, the arr's and tautulli scripts coming from user requests.
   - Updated on `Fri 12 September 2025 at 02:50 PM` | 450⭐ | `GNU General Public License v3.0`
- [searxng/searxng](https://github.com/searxng/searxng) - SearXNG is a free internet metasearch engine which aggregates results from various search services and databases. Users are neither tracked nor profiled.
   - Updated on `Mon 08 June 2026 at 11:37 AM` | 31782⭐ | `GNU Affero General Public License v3.0`
- [iperov/DeepFaceLive](https://github.com/iperov/DeepFaceLive) - Real-time face swap for PC streaming or video calls.
   - Updated on `Fri 08 November 2024 at 03:07 AM` | 30913⭐ | `GNU General Public License v3.0`
- [faif/python-patterns](https://github.com/faif/python-patterns) - A collection of design patterns/idioms in Python.
   - Updated on `Fri 13 March 2026 at 05:04 PM` | 42797⭐ | `❌`
- [maguowei/starred](https://github.com/maguowei/starred) - Create and maintain your own Awesome-style list from GitHub stars!.
   - Updated on `Mon 08 June 2026 at 02:46 PM` | 1913⭐ | `MIT License`
- [luigi311/JellyPlex-Watched](https://github.com/luigi311/JellyPlex-Watched) - Sync watched status between jellyfin, plex and emby locally.
   - Updated on `Thu 04 June 2026 at 04:41 PM` | 1013⭐ | `GNU General Public License v3.0`
- [andrewrabert/jellysub](https://github.com/andrewrabert/jellysub) - Use Subsonic clients with Jellyfin.
   - Updated on `Fri 14 October 2022 at 05:27 PM` | 61⭐ | `MIT License`
- [pyscript/pyscript](https://github.com/pyscript/pyscript) - An open source platform for Python in the browser. https://pyscript.net Docs: https://docs.pyscript.net/ Try it: https://pyscript.com/ Community: https://discord.gg/HxvBtukrg2.
   - Updated on `Tue 09 June 2026 at 01:45 PM` | 18690⭐ | `Apache License 2.0`
- [life4/deal](https://github.com/life4/deal) - 🤝 Design by contract for Python. Write bug-free code. Add a few decorators, get static analysis and tests for free.
   - Updated on `Sun 30 November 2025 at 10:55 AM` | 895⭐ | `MIT License`
- [jmbannon/ytdl-sub](https://github.com/jmbannon/ytdl-sub) - Lightweight tool to automate downloading and metadata generation with yt-dlp.
   - Updated on `Thu 04 June 2026 at 08:59 PM` | 2843⭐ | `GNU General Public License v3.0`
- [robertoszek/pleroma-bot](https://github.com/robertoszek/pleroma-bot) - Bot for mirroring one or multiple Twitter accounts in Pleroma/Mastodon/Misskey.
   - Updated on `Sun 29 October 2023 at 11:46 PM` | 110⭐ | `MIT License`
- [Horhik/Instagram2Fedi](https://github.com/Horhik/Instagram2Fedi) - Python script for crossposting from Instagram to Mastodon or Pixelfed.
   - Updated on `Fri 20 October 2023 at 10:57 AM` | 106⭐ | `GNU General Public License v3.0`
- [sepandhaghighi/art](https://github.com/sepandhaghighi/art) - 🎨 ASCII art library for Python.
   - Updated on `Fri 15 May 2026 at 06:47 AM` | 2476⭐ | `MIT License`
- [uraid/qbittorrent_throttle](https://github.com/uraid/qbittorrent_throttle) - Throttle qBittorrent on Plex stream Start/Stop.
   - Updated on `Sun 31 December 2023 at 06:49 AM` | 20⭐ | `❌`
- [LibreTranslate/LibreTranslate](https://github.com/LibreTranslate/LibreTranslate) - Free and Open Source Machine Translation API. Self-hosted, offline capable and easy to setup.
   - Updated on `Fri 05 June 2026 at 01:03 PM` | 14521⭐ | `GNU Affero General Public License v3.0`
- [python-poetry/poetry](https://github.com/python-poetry/poetry) - Python packaging and dependency management made easy.
   - Updated on `Mon 08 June 2026 at 09:08 PM` | 34275⭐ | `MIT License`
- [LibrePhotos/librephotos](https://github.com/LibrePhotos/librephotos) - A self-hosted open source photo management service.
   - Updated on `Tue 09 June 2026 at 07:48 PM` | 7990⭐ | `MIT License`
- [etesync/server](https://github.com/etesync/server) - The Etebase server (so you can run your own).
   - Updated on `Fri 12 July 2024 at 12:44 PM` | 1756⭐ | `GNU Affero General Public License v3.0`
- [home-assistant/core](https://github.com/home-assistant/core) - :house_with_garden: Open source home automation that puts local control and privacy first.
   - Updated on `Tue 09 June 2026 at 09:17 PM` | 87635⭐ | `Apache License 2.0`
- [matthuisman/gdrivedl](https://github.com/matthuisman/gdrivedl) - Google Drive Download Python Script.
   - Updated on `Thu 01 May 2025 at 08:20 AM` | 224⭐ | `GNU General Public License v3.0`
- [liiight/notifiers](https://github.com/liiight/notifiers) - The easy way to send notifications.
   - Updated on `Mon 08 June 2026 at 05:32 PM` | 2735⭐ | `MIT License`
- [praw-dev/asyncpraw](https://github.com/praw-dev/asyncpraw) - Async PRAW, an abbreviation for "Asynchronous Python Reddit API Wrapper", is a python package that allows for simple access to Reddit's API.
   - Updated on `Tue 09 June 2026 at 05:35 PM` | 148⭐ | `BSD 2-Clause "Simplified" License`
- [praw-dev/praw](https://github.com/praw-dev/praw) - PRAW, an acronym for "Python Reddit API Wrapper", is a python package that allows for simple access to Reddit's API.
   - Updated on `Tue 09 June 2026 at 09:22 PM` | 4144⭐ | `BSD 2-Clause "Simplified" License`
- [Delgan/loguru](https://github.com/Delgan/loguru) - Python logging made (stupidly) simple.
   - Updated on `Sun 24 May 2026 at 01:07 PM` | 23946⭐ | `MIT License`
- [searx/searx](https://github.com/searx/searx) - Privacy-respecting metasearch engine.
   - Updated on `Thu 14 May 2026 at 10:02 AM` | 13508⭐ | `GNU Affero General Public License v3.0`
- [yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp) - A feature-rich command-line audio/video downloader.
   - Updated on `Tue 09 June 2026 at 11:26 PM` | 169504⭐ | `The Unlicense`
- [JVT038/MetaTube](https://github.com/JVT038/MetaTube) - Python web ui to download videos from YouTube and add metadata on top of it.
   - Updated on `Mon 16 March 2026 at 10:48 AM` | 368⭐ | `GNU General Public License v3.0`
- [tiararosebiezetta/HerokuDynoSwitcher](https://github.com/tiararosebiezetta/HerokuDynoSwitcher) - A little script and trick to make your heroku app run forever without being concerned about dyno hours.
   - Updated on `Thu 14 April 2022 at 07:33 AM` | 142⭐ | `❌`
- [milesmcc/shynet](https://github.com/milesmcc/shynet) - Modern, privacy-friendly, and detailed web analytics that works without cookies or JS.
   - Updated on `Sun 15 March 2026 at 11:00 PM` | 3141⭐ | `Apache License 2.0`
- [alexta69/metube](https://github.com/alexta69/metube) - Self-hosted video downloader for YouTube and other sites (web UI for youtube-dl / yt-dlp).
   - Updated on `Sat 06 June 2026 at 06:52 AM` | 13808⭐ | `GNU Affero General Public License v3.0`
- [benbusby/whoogle-search](https://github.com/benbusby/whoogle-search) - A self-hosted, ad-free, privacy-respecting metasearch engine.
   - Updated on `Tue 19 May 2026 at 07:42 PM` | 11544⭐ | `MIT License`
- [meeb/tubesync](https://github.com/meeb/tubesync) - Syncs YouTube channels and playlists to a locally hosted media server.
   - Updated on `Thu 04 June 2026 at 04:46 PM` | 2736⭐ | `GNU Affero General Public License v3.0`
- [megadose/holehe](https://github.com/megadose/holehe) - holehe allows you to check if the mail is used on different sites like twitter, instagram and will retrieve information on sites with the forgotten password function.
   - Updated on `Tue 10 September 2024 at 08:24 PM` | 11220⭐ | `GNU General Public License v3.0`
- [StuffAnThings/qbit_manage](https://github.com/StuffAnThings/qbit_manage) - This tool will help manage tedious tasks in qBittorrent and automate them. Tag, categorize, remove Orphaned data, remove unregistered torrents and much much more.
   - Updated on `Mon 08 June 2026 at 07:34 PM` | 1544⭐ | `MIT License`
- [Kometa-Team/Kometa](https://github.com/Kometa-Team/Kometa) - Python script to update metadata information for items in plex as well as automatically build collections and playlists. The Wiki Documentation is linked below.
   - Updated on `Tue 09 June 2026 at 08:38 PM` | 3319⭐ | `MIT License`
- [ZeroQI/Absolute-Series-Scanner](https://github.com/ZeroQI/Absolute-Series-Scanner) - Seasons, absolute mode, Subfolders...
   - Updated on `Thu 21 May 2026 at 09:10 PM` | 1105⭐ | `❌`
- [Flexget/Flexget](https://github.com/Flexget/Flexget) - The official FlexGet repository.
   - Updated on `Mon 01 June 2026 at 07:55 PM` | 1949⭐ | `MIT License`
- [Diaoul/subliminal](https://github.com/Diaoul/subliminal) - Subtitles, faster than your thoughts.
   - Updated on `Sat 23 May 2026 at 09:31 PM` | 2638⭐ | `MIT License`
- [smacke/ffsubsync](https://github.com/smacke/ffsubsync) - Automagically synchronize subtitles with video.
   - Updated on `Sun 07 June 2026 at 07:34 PM` | 7732⭐ | `MIT License`
- [lardbit/nefarious](https://github.com/lardbit/nefarious) - Web application for automatically downloading TV & Movies (w/ VPN).
   - Updated on `Tue 09 June 2026 at 02:31 PM` | 1250⭐ | `GNU General Public License v3.0`
- [SickGear/SickGear](https://github.com/SickGear/SickGear) - SickGear has proven the most reliable stable TV fork of the great Sick-Beard to fully automate TV enjoyment with innovation.
   - Updated on `Mon 08 June 2026 at 09:39 PM` | 775⭐ | `GNU General Public License v3.0`
- [Shubham0Rajput/Rclone-Telegram-Bot](https://github.com/Shubham0Rajput/Rclone-Telegram-Bot) - This is Rclone Bot in Telegram made for your ease.
   - Updated on `Sat 12 September 2020 at 05:38 PM` | 81⭐ | `MIT License`
- [davidfoerster/aptsources-cleanup](https://github.com/davidfoerster/aptsources-cleanup) - Detects and interactively deactivates duplicate Apt source entries and deletes sources list files without valid enabled source entries (as requested in https://askubuntu.com/a/762815/175814).
   - Updated on `Fri 16 August 2024 at 01:40 AM` | 684⭐ | `MIT License`
- [a4k-openproject/a4kSubtitles](https://github.com/a4k-openproject/a4kSubtitles) - a4kSubtitles (Multi-source subtitles addon for KODI).
   - Updated on `Thu 07 May 2026 at 09:29 AM` | 336⭐ | `MIT License`
- [sgrontflix/simplehttpserverwithupload](https://github.com/sgrontflix/simplehttpserverwithupload) - Simple HTTP server with upload functionality written in Python.
   - Updated on `Tue 12 October 2021 at 07:00 AM` | 8⭐ | `GNU General Public License v3.0`
- [tubearchivist/tubearchivist](https://github.com/tubearchivist/tubearchivist) - Your self hosted YouTube media server.
   - Updated on `Sat 23 May 2026 at 09:04 AM` | 8070⭐ | `GNU General Public License v3.0`
- [huashengdun/webssh](https://github.com/huashengdun/webssh) - :seedling: Web based ssh client.
   - Updated on `Sun 27 July 2025 at 01:48 PM` | 5110⭐ | `MIT License`
- [vincreator/eunhamirror](https://github.com/vincreator/eunhamirror) - Aria,Qbittorent & Leech Telegram Mirror Bot.
   - Updated on `Sat 18 February 2023 at 04:12 PM` | 172⭐ | `GNU General Public License v3.0`
- [croneter/PlexKodiConnect](https://github.com/croneter/PlexKodiConnect) - Plex integration in Kodi done right.
   - Updated on `Wed 01 April 2026 at 11:41 AM` | 1332⭐ | `GNU General Public License v2.0`
- [psf/black](https://github.com/psf/black) - The uncompromising Python code formatter.
   - Updated on `Tue 09 June 2026 at 06:32 AM` | 41560⭐ | `MIT License`
- [bellyjay1005/pydroneapi](https://github.com/bellyjay1005/pydroneapi) - Python client for Drone CI API.  Manages authentication against Drone CI and performs common Drone CI API operations using a Python wrapper.
   - Updated on `Thu 08 June 2023 at 04:08 PM` | 1⭐ | `MIT License`
- [tinvaan/drone-python](https://github.com/tinvaan/drone-python) - Python client for the drone.io public API.
   - Updated on `Wed 21 August 2024 at 08:06 AM` | 17⭐ | `Apache License 2.0`
- [matrix-org/synapse](https://github.com/matrix-org/synapse) - Synapse: Matrix homeserver written in Python/Twisted.
   - Updated on `Fri 26 April 2024 at 03:47 PM` | 12095⭐ | `Apache License 2.0`
- [healthchecks/healthchecks](https://github.com/healthchecks/healthchecks) - Open-source cron job and background task monitoring service, written in Python & Django.
   - Updated on `Tue 09 June 2026 at 01:52 PM` | 10084⭐ | `BSD 3-Clause "New" or "Revised" License`
- [goauthentik/authentik](https://github.com/goauthentik/authentik) - The authentication glue you need.
   - Updated on `Tue 09 June 2026 at 10:43 PM` | 21884⭐ | `Other`
- [jkwill87/mnamer](https://github.com/jkwill87/mnamer) - media file renaming and organizing tool.
   - Updated on `Fri 05 June 2026 at 02:14 PM` | 1110⭐ | `MIT License`
- [mvt-project/mvt](https://github.com/mvt-project/mvt) - MVT (Mobile Verification Toolkit) helps with conducting forensics of mobile devices in order to find signs of a potential compromise.
   - Updated on `Sat 06 June 2026 at 03:31 PM` | 12471⭐ | `Other`
- [public-apis/public-apis](https://github.com/public-apis/public-apis) - A collective list of free APIs.
   - Updated on `Sun 07 June 2026 at 07:41 AM` | 440504⭐ | `MIT License`
- [PrivOci/ReTube](https://github.com/PrivOci/ReTube) - ReImagine Tubing.
   - Updated on `Fri 27 January 2023 at 08:47 AM` | 23⭐ | `❌`
- [dgtlmoon/changedetection.io](https://github.com/dgtlmoon/changedetection.io) - Best and simplest tool for website change detection, web page monitoring, and website change alerts. Perfect for tracking content changes, price drops, restock alerts, and website defacement monitorin.
   - Updated on `Wed 03 June 2026 at 11:26 AM` | 31932⭐ | `Apache License 2.0`
- [simple-login/app](https://github.com/simple-login/app) - The SimpleLogin back-end and web app.
   - Updated on `Tue 09 June 2026 at 10:09 AM` | 6718⭐ | `GNU Affero General Public License v3.0`
- [Taxel/PlexTraktSync](https://github.com/Taxel/PlexTraktSync) - A python script that syncs the movies, shows and ratings between trakt and Plex (without needing a PlexPass or Trakt VIP subscription).
   - Updated on `Mon 08 June 2026 at 12:42 PM` | 2095⭐ | `MIT License`
- [chrisdewa/dpytools](https://github.com/chrisdewa/dpytools) - Collection of easy to use, beginner friendly but powerful, orthogonal tools to speed up discord bots development (discord.py).
   - Updated on `Wed 18 August 2021 at 05:48 PM` | 20⭐ | `MIT License`
- [Absolute-Tinkerer/CLAPI](https://github.com/Absolute-Tinkerer/CLAPI) - A basic API to scrape Craigslist.
   - Updated on `Sun 02 May 2021 at 04:24 PM` | 44⭐ | `GNU General Public License v3.0`
- [lushan88a/google_trans_new](https://github.com/lushan88a/google_trans_new) - A free and unlimited python API for google translate.
   - Updated on `Thu 06 October 2022 at 01:02 PM` | 396⭐ | `MIT License`
- [psf/requests](https://github.com/psf/requests) - A simple, yet elegant, HTTP library.
   - Updated on `Tue 09 June 2026 at 03:46 PM` | 54038⭐ | `Apache License 2.0`
- [Tautulli/Tautulli](https://github.com/Tautulli/Tautulli) - A Python based monitoring and tracking tool for Plex Media Server.
   - Updated on `Mon 08 June 2026 at 05:02 PM` | 6511⭐ | `GNU General Public License v3.0`
- [gfjaru/reddit-watcher](https://github.com/gfjaru/reddit-watcher) - 👀 Grab latest post from subreddit.
   - Updated on `Tue 15 September 2020 at 10:21 AM` | 1⭐ | `❌`
- [thp/urlwatch](https://github.com/thp/urlwatch) - Watch (parts of) webpages and get notified when something changes via e-mail, on your phone or via other means. Highly configurable.
   - Updated on `Sat 23 May 2026 at 05:07 AM` | 3102⭐ | `Other`
- [torpyorg/torpy](https://github.com/torpyorg/torpy) - Pure python Tor client implementation.
   - Updated on `Fri 21 April 2023 at 06:45 PM` | 437⭐ | `Apache License 2.0`
- [yaronzz/Tidal-Media-Downloader](https://github.com/yaronzz/Tidal-Media-Downloader) - Download 'TIDAL' Music On Windows/Linux/MacOs (PYTHON/C#).
   - Updated on `Fri 17 October 2025 at 03:41 AM` | 4250⭐ | `Apache License 2.0`
- [FlareSolverr/FlareSolverr](https://github.com/FlareSolverr/FlareSolverr) - Proxy server to bypass Cloudflare protection.
   - Updated on `Fri 05 June 2026 at 04:50 PM` | 14238⭐ | `MIT License`
- [mza921/Plex-Auto-Collections](https://github.com/mza921/Plex-Auto-Collections) - Python 3 script to automatically update Plex collections based off a configuration file.
   - Updated on `Tue 15 August 2023 at 05:44 PM` | 316⭐ | `❌`
- [pierre-delecto/stash_theporndb_scraper](https://github.com/pierre-delecto/stash_theporndb_scraper) - A python script to scrape Stash data from thePornDB.
   - Updated on `Thu 08 December 2022 at 03:49 AM` | 57⭐ | `MIT License`
- [l3uddz/plex_dupefinder](https://github.com/l3uddz/plex_dupefinder) - Find and delete duplicate files in Plex.
   - Updated on `Wed 21 February 2024 at 04:01 PM` | 341⭐ | `GNU General Public License v3.0`
- [anthony-foulfoin/trakt-tv-duplicates-removal](https://github.com/anthony-foulfoin/trakt-tv-duplicates-removal) - This script allows you to remove all the duplicate or additionnal plays for your trakt.tv movies or episodes.
   - Updated on `Sat 18 April 2026 at 07:08 PM` | 80⭐ | `❌`
- [toddrob99/searcharr](https://github.com/toddrob99/searcharr) - Sonarr & Radarr & Readarr Telegram Bot.
   - Updated on `Wed 08 April 2026 at 12:03 AM` | 298⭐ | `MIT License`
- [aapatre/Automatic-Udemy-Course-Enroller-GET-PAID-UDEMY-COURSES-for-FREE](https://github.com/aapatre/Automatic-Udemy-Course-Enroller-GET-PAID-UDEMY-COURSES-for-FREE) - Do you want to LEARN NEW STUFF for FREE? Don't worry, with the power of web-scraping and automation, this script will find the necessary Udemy coupons & enroll you for PAID UDEMY COURSES, ABSOLUTELY F.
   - Updated on `Tue 28 April 2026 at 11:49 AM` | 3299⭐ | `GNU General Public License v3.0`
- [svp318/MSFS2020-Real-Time-Weather](https://github.com/svp318/MSFS2020-Real-Time-Weather) - A work in progress to update a custom Microsoft Flight Simulator 2020 weather preset with the current conditions of a given airport.
   - Updated on `Wed 21 April 2021 at 02:12 AM` | 24⭐ | `GNU General Public License v3.0`
- [iamkroot/trakt-scrobbler](https://github.com/iamkroot/trakt-scrobbler) - Scrobbler for trakt.tv that supports VLC, Plex, MPC-HC, and MPV.
   - Updated on `Thu 28 May 2026 at 05:55 AM` | 550⭐ | `GNU General Public License v2.0`
- [Al-Azif/ps4-exploit-host](https://github.com/Al-Azif/ps4-exploit-host) - Easy Exploit Hosting.
   - Updated on `Mon 08 January 2024 at 07:40 PM` | 1199⭐ | `MIT License`
- [caronc/apprise](https://github.com/caronc/apprise) - Apprise - Push Notifications that work with just about every platform!.
   - Updated on `Tue 09 June 2026 at 03:43 AM` | 16705⭐ | `BSD 2-Clause "Simplified" License`
- [Waterboy1602/Addarr](https://github.com/Waterboy1602/Addarr) - Telegram bot for adding series/movies to Sonarr/Radarr or for changing the download speed of Transmission/Sabnzbd.
   - Updated on `Sun 15 June 2025 at 07:38 PM` | 278⭐ | `MIT License`
- [NickBrecht/ArrJanitor](https://github.com/NickBrecht/ArrJanitor) - A python script designed to clean up Radarr/Sonarr downloads in Deluge.
   - Updated on `Fri 12 March 2021 at 11:35 AM` | 15⭐ | `❌`
- [CouchPotato/CouchPotatoServer](https://github.com/CouchPotato/CouchPotatoServer) - Automatic Movie Downloading via NZBs & Torrents.
   - Updated on `Wed 04 August 2021 at 06:39 PM` | 3875⭐ | `GNU General Public License v3.0`
- [syncarr/syncarr](https://github.com/syncarr/syncarr) - one/two way sync of Radarr, Sonarr, or Lidarr instances.
   - Updated on `Fri 15 March 2024 at 08:19 AM` | 269⭐ | `MIT License`
- [l3uddz/traktarr](https://github.com/l3uddz/traktarr) - Script to add new series & movies to Sonarr/Radarr based on Trakt lists.
   - Updated on `Tue 14 March 2023 at 06:34 PM` | 622⭐ | `GNU General Public License v3.0`
- [r0oth3x49/udemy-dl](https://github.com/r0oth3x49/udemy-dl) - A cross-platform python based utility to download courses from udemy for personal offline use.
   - Updated on `Sun 06 June 2021 at 10:16 AM` | 4948⭐ | `MIT License`
- [stewartmcgown/uds](https://github.com/stewartmcgown/uds) - 📀 Unlimited Google Drive Storage by splitting binary files into base64.
   - Updated on `Tue 02 November 2021 at 07:51 AM` | 4373⭐ | `GNU Affero General Public License v3.0`
- [FriendsOfGalaxy/galaxy-integration-epic](https://github.com/FriendsOfGalaxy/galaxy-integration-epic) - Epic integration for GOG Galaxy 2.0. Deprecated since GOG has official integration with EpicGames.
   - Updated on `Thu 02 July 2020 at 12:34 PM` | 31⭐ | `MIT License`
- [FriendsOfGalaxy/galaxy-integration-blizzard](https://github.com/FriendsOfGalaxy/galaxy-integration-blizzard) - In case of any issues please refer to the original repository:.
   - Updated on `Tue 09 November 2021 at 07:03 PM` | 72⭐ | `MIT License`
- [FriendsOfGalaxy/galaxy-integration-origin](https://github.com/FriendsOfGalaxy/galaxy-integration-origin) - origin integration for galaxy.
   - Updated on `Tue 24 October 2023 at 03:06 AM` | 82⭐ | `❌`
- [FriendsOfGalaxy/galaxy-integration-humble](https://github.com/FriendsOfGalaxy/galaxy-integration-humble) - In case of any issues please refer to the original repository:.
   - Updated on `Fri 08 October 2021 at 07:23 PM` | 23⭐ | `GNU General Public License v3.0`
- [FriendsOfGalaxy/galaxy-integration-psn](https://github.com/FriendsOfGalaxy/galaxy-integration-psn) - psn integration for galaxy.
   - Updated on `Wed 15 December 2021 at 02:45 PM` | 77⭐ | `❌`
- [FriendsOfGalaxy/galaxy-integration-rockstar](https://github.com/FriendsOfGalaxy/galaxy-integration-rockstar) - In case of any issues please refer to the original repository:.
   - Updated on `Tue 22 March 2022 at 12:22 AM` | 27⭐ | `MIT License`
- [FriendsOfGalaxy/galaxy-integration-paradox](https://github.com/FriendsOfGalaxy/galaxy-integration-paradox) - .
   - Updated on `Thu 20 July 2023 at 03:09 PM` | 20⭐ | `MIT License`
- [FriendsOfGalaxy/galaxy-integration-wargaming](https://github.com/FriendsOfGalaxy/galaxy-integration-wargaming) - In case of any issues please refer to the original repository:.
   - Updated on `Tue 26 October 2021 at 10:50 AM` | 7⭐ | `MIT License`
- [FriendsOfGalaxy/galaxy-integration-uplay](https://github.com/FriendsOfGalaxy/galaxy-integration-uplay) - uplay integration with g2.0.
   - Updated on `Mon 09 October 2023 at 05:38 PM` | 124⭐ | `MIT License`
- [mnsardone/Reddit-to-Telegram-Python-Bot](https://github.com/mnsardone/Reddit-to-Telegram-Python-Bot) - Simple bot using PRAW and python-telegram-bot. Minimal knowledge of python needed.
   - Updated on `Mon 06 January 2020 at 08:45 PM` | 1⭐ | `❌`
- [xyou365/AutoRclone](https://github.com/xyou365/AutoRclone) - AutoRclone: rclone copy/move/sync (automatically) with thousands of service accounts.
   - Updated on `Sun 11 December 2022 at 07:12 AM` | 1383⭐ | `❌`
- [StevenBlack/hosts](https://github.com/StevenBlack/hosts) - 🔒 Consolidating and extending hosts files from several well-curated sources. Optionally pick extensions for porn, social media, and other categories.
   - Updated on `Sun 07 June 2026 at 06:19 PM` | 30510⭐ | `MIT License`
- [ddragosd/python-rclone](https://github.com/ddragosd/python-rclone) - Python module for rclone.
   - Updated on `Thu 30 December 2021 at 08:51 PM` | 80⭐ | `Apache License 2.0`
- [anime-dl/anime-downloader](https://github.com/anime-dl/anime-downloader) - A simple but powerful anime downloader and streamer.
   - Updated on `Thu 08 December 2022 at 03:13 AM` | 1988⭐ | `The Unlicense`
- [Slashbunny/gog-galaxy-plugin-downloader](https://github.com/Slashbunny/gog-galaxy-plugin-downloader) - Downloads and Updates GOG Galaxy 2.0 Plugins/Integrations.
   - Updated on `Tue 20 August 2024 at 12:10 PM` | 282⭐ | `GNU General Public License v3.0`
- [pawamoy/aria2p](https://github.com/pawamoy/aria2p) - Command-line tool and library to interact with an aria2c daemon process with JSON-RPC.
   - Updated on `Wed 25 March 2026 at 12:13 PM` | 564⭐ | `ISC License`
- [arsenlosenko/transfer.sh_client](https://github.com/arsenlosenko/transfer.sh_client) - Python client for uploading files to transfer.sh (https://transfer.sh/).
   - Updated on `Mon 22 May 2023 at 10:27 PM` | 13⭐ | `MIT License`
- [googlearchive/PyDrive](https://github.com/googlearchive/PyDrive) - Google Drive API Python wrapper library.
   - Updated on `Thu 24 June 2021 at 05:24 AM` | 1314⭐ | `Other`
- [ahmedsajid/aria2-dl](https://github.com/ahmedsajid/aria2-dl) - youtube-dl + aria2.
   - Updated on `Thu 20 November 2025 at 06:38 PM` | 9⭐ | `MIT License`
- [pyload/pyload](https://github.com/pyload/pyload) - The free and open-source Download Manager written in pure Python.
   - Updated on `Sat 06 June 2026 at 11:57 PM` | 3788⭐ | `Other`
- [eternnoir/pyTelegramBotAPI](https://github.com/eternnoir/pyTelegramBotAPI) - Python Telegram bot api.
   - Updated on `Wed 03 June 2026 at 11:41 PM` | 8752⭐ | `GNU General Public License v2.0`
- [glumia/openload_dl](https://github.com/glumia/openload_dl) - A python library and CLI tool to download files from openload.co.
   - Updated on `Tue 04 January 2022 at 08:35 PM` | 36⭐ | `MIT License`
- [SpEcHiDe/AnyDLBot](https://github.com/SpEcHiDe/AnyDLBot) - An Open Source GPLv3 All-In-One Telegram Bot.
   - Updated on `Tue 30 November 2021 at 08:12 AM` | 395⭐ | `GNU General Public License v3.0`
- [get-pytube/pytube3](https://github.com/get-pytube/pytube3) - A lightweight, dependency-free Python 3 library (and command-line utility) for downloading YouTube Videos.
   - Updated on `Wed 10 July 2024 at 08:11 PM` | 184⭐ | `Other`
- [shivamdev-lgtm/python-aria-mirror-bot](https://github.com/shivamdev-lgtm/python-aria-mirror-bot) - A telegram bot for all your mirror needs | OG Repo.
   - Updated on `Wed 14 September 2022 at 03:35 PM` | 782⭐ | `GNU General Public License v3.0`
- [MrS0m30n3/youtube-dl-gui](https://github.com/MrS0m30n3/youtube-dl-gui) - A cross platform front-end GUI of the popular youtube-dl written in wxPython.
   - Updated on `Tue 06 December 2022 at 05:12 PM` | 9418⭐ | `The Unlicense`
- [aryanvikash/Google-Drive-Uploader](https://github.com/aryanvikash/Google-Drive-Uploader) - Telegram bot To upload Files To Google Drive Remotely @gdriveupme_bot (outdated).
   - Updated on `Thu 16 September 2021 at 08:42 AM` | 308⭐ | `❌`
- [python-telegram-bot/python-telegram-bot](https://github.com/python-telegram-bot/python-telegram-bot) - We have made you a wrapper you can't refuse.
   - Updated on `Tue 09 June 2026 at 08:23 PM` | 29213⭐ | `GNU General Public License v3.0`
- [ytdl-org/youtube-dl](https://github.com/ytdl-org/youtube-dl) - Command-line program to download videos from YouTube.com and other video sites.
   - Updated on `Thu 19 February 2026 at 04:45 PM` | 140469⭐ | `The Unlicense`
- [jonathanTIE/googledrive-copy-downloader](https://github.com/jonathanTIE/googledrive-copy-downloader) - Python script to download google drive files even if the daily limit of download has excedeed.
   - Updated on `Thu 08 December 2022 at 03:22 AM` | 206⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## Ruby 

- [Freika/dawarich](https://github.com/Freika/dawarich) - Your favorite self-hostable alternative to Google Timeline (Google Location History).
   - Updated on `Tue 09 June 2026 at 06:50 PM` | 9270⭐ | `GNU Affero General Public License v3.0`
- [ubicloud/ubicloud](https://github.com/ubicloud/ubicloud) - Open source alternative to AWS. Elastic compute, block storage (non replicated), firewall and load balancer, managed Postgres, K8s, AI inference, and IAM services.
   - Updated on `Wed 10 June 2026 at 12:11 AM` | 12090⭐ | `GNU Affero General Public License v3.0`
- [postalserver/postal](https://github.com/postalserver/postal) - 📮 A fully featured open source mail delivery platform for incoming & outgoing e-mail.
   - Updated on `Tue 09 June 2026 at 06:50 PM` | 16595⭐ | `MIT License`
- [maybe-finance/maybe](https://github.com/maybe-finance/maybe) - The personal finance app for everyone.
   - Updated on `Thu 24 July 2025 at 10:20 PM` | 54149⭐ | `GNU Affero General Public License v3.0`
- [fastlane/fastlane](https://github.com/fastlane/fastlane) - 🚀 The easiest way to automate building and releasing your iOS and Android apps.
   - Updated on `Tue 09 June 2026 at 01:05 PM` | 41632⭐ | `MIT License`
- [chatwoot/chatwoot](https://github.com/chatwoot/chatwoot) - Open-source live-chat, email support, omni-channel desk. An alternative to Intercom, Zendesk, Salesforce Service Cloud etc. 🔥💬.
   - Updated on `Tue 09 June 2026 at 07:39 PM` | 30062⭐ | `Other`
- [siteinspector/siteinspector](https://github.com/siteinspector/siteinspector) - A tool for catching spelling errors, grammatical errors, broken links, and other errors on websites.
   - Updated on `Sun 12 January 2025 at 10:15 AM` | 410⭐ | `GNU Affero General Public License v3.0`
- [FortAwesome/font-awesome-sass](https://github.com/FortAwesome/font-awesome-sass) - Font-Awesome Sass gem for use in Ruby/Rails projects.
   - Updated on `Thu 13 February 2025 at 09:11 PM` | 923⭐ | `MIT License`
- [snibox/snibox](https://github.com/snibox/snibox) - Self-hosted snippet manager.
   - Updated on `Mon 12 December 2022 at 02:10 PM` | 1811⭐ | `❌`
- [mastodon/mastodon](https://github.com/mastodon/mastodon) - Your self-hosted, globally interconnected microblogging community.
   - Updated on `Tue 09 June 2026 at 10:48 PM` | 50013⭐ | `GNU Affero General Public License v3.0`
- [huginn/huginn](https://github.com/huginn/huginn) - Create agents that monitor and act on your behalf.  Your agents are standing by!.
   - Updated on `Tue 09 June 2026 at 05:07 PM` | 49447⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Rust 

- [microsoft/coreutils](https://github.com/microsoft/coreutils) - Coreutils for Windows: Installer & Packaging.
   - Updated on `Tue 09 June 2026 at 08:00 PM` | 3757⭐ | `MIT License`
- [uutils/coreutils](https://github.com/uutils/coreutils) - Cross-platform Rust rewrite of the GNU coreutils.
   - Updated on `Tue 09 June 2026 at 08:17 PM` | 23564⭐ | `MIT License`
- [run-llama/liteparse](https://github.com/run-llama/liteparse) - A fast, helpful, and open-source document parser.
   - Updated on `Tue 09 June 2026 at 10:53 PM` | 9723⭐ | `Apache License 2.0`
- [rubix-studios-pty-ltd/ntfy-app](https://github.com/rubix-studios-pty-ltd/ntfy-app) - A desktop application for ntfy that provides instant native notifications and real-time messaging directly from your desktop. It supports both ntfy.sh and self-hosted ntfy instances through a lightwei.
   - Updated on `Sun 07 June 2026 at 07:13 AM` | 13⭐ | `MIT License`
- [ruvnet/RuView](https://github.com/ruvnet/RuView) - π RuView turns commodity WiFi signals into real-time spatial intelligence, vital sign monitoring, and presence detection — all without a single pixel of video.
   - Updated on `Tue 09 June 2026 at 09:27 PM` | 72435⭐ | `MIT License`
- [1jehuang/jcode](https://github.com/1jehuang/jcode) - Coding Agent Harness.
   - Updated on `Wed 10 June 2026 at 12:10 AM` | 6964⭐ | `MIT License`
- [tinyhumansai/openhuman](https://github.com/tinyhumansai/openhuman) - Your Personal AI super intelligence. Private, Simple and extremely powerful.
   - Updated on `Tue 09 June 2026 at 08:31 PM` | 31253⭐ | `GNU General Public License v3.0`
- [warpdotdev/warp](https://github.com/warpdotdev/warp) - Warp is an agentic development environment, born out of the terminal.
   - Updated on `Wed 10 June 2026 at 12:03 AM` | 61355⭐ | `GNU Affero General Public License v3.0`
- [cjpais/Handy](https://github.com/cjpais/Handy) - A free, open source, and extensible speech-to-text application that works completely offline.
   - Updated on `Fri 05 June 2026 at 01:22 AM` | 23449⭐ | `MIT License`
- [LLukas22/Jellyswarrm](https://github.com/LLukas22/Jellyswarrm) - Bring all your Jellyfin servers together.
   - Updated on `Sun 07 June 2026 at 04:08 PM` | 758⭐ | `GNU General Public License v2.0`
- [meilisearch/meilisearch](https://github.com/meilisearch/meilisearch) - A lightning-fast search engine API bringing AI-powered hybrid search to your sites and applications.
   - Updated on `Tue 09 June 2026 at 04:02 PM` | 58028⭐ | `Other`
- [euzu/tuliprox](https://github.com/euzu/tuliprox) - Flexible IPTV playlist processor & proxy in Rust 🦀 - supports M3U, Xtream Codes, HDHomeRun, Plex, Emby & Jellyfin. Filtering, merging, scheduling & web delivery.
   - Updated on `Tue 09 June 2026 at 03:59 PM` | 474⭐ | `MIT License`
- [helix-editor/helix](https://github.com/helix-editor/helix) - A post-modern modal text editor.
   - Updated on `Tue 09 June 2026 at 11:24 PM` | 44872⭐ | `Mozilla Public License 2.0`
- [mtkennerly/ludusavi](https://github.com/mtkennerly/ludusavi) - Backup tool for PC game saves.
   - Updated on `Fri 17 April 2026 at 11:23 PM` | 5707⭐ | `MIT License`
- [aaif-goose/goose](https://github.com/aaif-goose/goose) - an open source, extensible AI agent that goes beyond code suggestions - install, execute, edit, and test with any LLM.
   - Updated on `Wed 10 June 2026 at 12:20 AM` | 48492⭐ | `Apache License 2.0`
- [louis-e/arnis](https://github.com/louis-e/arnis) - Generate any location from the real world in Minecraft with a high level of detail.
   - Updated on `Mon 08 June 2026 at 10:33 PM` | 15997⭐ | `Apache License 2.0`
- [mozilla-services/autopush-rs](https://github.com/mozilla-services/autopush-rs) - Push Server in Rust.
   - Updated on `Mon 01 June 2026 at 06:14 PM` | 316⭐ | `Mozilla Public License 2.0`
- [jtroo/kanata](https://github.com/jtroo/kanata) - Improve keyboard comfort and usability with advanced customization.
   - Updated on `Sun 07 June 2026 at 01:06 AM` | 7464⭐ | `GNU Lesser General Public License v3.0`
- [denoland/deno](https://github.com/denoland/deno) - A modern runtime for JavaScript and TypeScript.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 107008⭐ | `MIT License`
- [screenpipe/screenpipe](https://github.com/screenpipe/screenpipe) - YC (S26) | AI that knows what you've seen, said, or heard. Records everything you do, say, hear 24/7, local, private, secure.
   - Updated on `Wed 10 June 2026 at 12:28 AM` | 19230⭐ | `Other`
- [RustPython/RustPython](https://github.com/RustPython/RustPython) - A Python Interpreter written in Rust.
   - Updated on `Tue 09 June 2026 at 01:12 AM` | 22104⭐ | `MIT License`
- [achristmascarl/rainfrog](https://github.com/achristmascarl/rainfrog) - 🐸 a database tool for the terminal.
   - Updated on `Fri 05 June 2026 at 09:54 PM` | 5104⭐ | `MIT License`
- [astral-sh/uv](https://github.com/astral-sh/uv) - An extremely fast Python package and project manager, written in Rust.
   - Updated on `Tue 09 June 2026 at 11:36 PM` | 86201⭐ | `Apache License 2.0`
- [vladkens/ghstats](https://github.com/vladkens/ghstats) - 🤩📈 Self-hosted GitHub traffic analytics with long-term history, SQLite storage, and a tiny Docker image.
   - Updated on `Mon 01 June 2026 at 06:36 PM` | 180⭐ | `MIT License`
- [mollyim/mollysocket](https://github.com/mollyim/mollysocket) - MollySocket allows getting Signal notifications via UnifiedPush.
   - Updated on `Wed 25 March 2026 at 09:02 AM` | 354⭐ | `GNU Affero General Public License v3.0`
- [moghtech/komodo](https://github.com/moghtech/komodo) - 🦎 a tool to build and deploy software on many servers 🦎.
   - Updated on `Tue 12 May 2026 at 01:45 AM` | 11352⭐ | `GNU General Public License v3.0`
- [j-hc/zygisk-detach](https://github.com/j-hc/zygisk-detach) - Zygisk module to detach installed apps from Play Store, hooking binder.
   - Updated on `Sun 24 May 2026 at 10:02 AM` | 2014⭐ | `Apache License 2.0`
- [gitbutlerapp/gitbutler](https://github.com/gitbutlerapp/gitbutler) - The GitButler version control client, backed by Git, powered by Tauri/Rust/Svelte.
   - Updated on `Tue 09 June 2026 at 09:33 PM` | 20970⭐ | `Other`
- [gitui-org/gitui](https://github.com/gitui-org/gitui) - Blazing 💥 fast terminal-ui for git written in rust 🦀.
   - Updated on `Wed 27 May 2026 at 04:48 AM` | 22091⭐ | `MIT License`
- [GyulyVGC/sniffnet](https://github.com/GyulyVGC/sniffnet) - Comfortably monitor your Internet traffic 🕵️‍♂️.
   - Updated on `Mon 08 June 2026 at 10:16 AM` | 38897⭐ | `Apache License 2.0`
- [ginger51011/pandoras_pot](https://github.com/ginger51011/pandoras_pot) - HTTP honeypot to punish and educate unruly web crawlers, written in Rust (🚀).
   - Updated on `Tue 12 May 2026 at 01:19 AM` | 137⭐ | `GNU Affero General Public License v3.0`
- [GraphiteEditor/Graphite](https://github.com/GraphiteEditor/Graphite) - Community-built comprehensive 2D content creation appplication for graphic design, digital art, and interactive real-time motion graphics powered by a node-based procedural graphics engine.
   - Updated on `Tue 09 June 2026 at 06:14 PM` | 26231⭐ | `Apache License 2.0`
- [oven-sh/bun](https://github.com/oven-sh/bun) - Incredibly fast JavaScript runtime, bundler, test runner, and package manager – all in one.
   - Updated on `Wed 10 June 2026 at 12:32 AM` | 93007⭐ | `Other`
- [rustic-rs/rustic](https://github.com/rustic-rs/rustic) - rustic - fast, encrypted, and deduplicated backups powered by Rust.
   - Updated on `Mon 08 June 2026 at 11:57 AM` | 3091⭐ | `Apache License 2.0`
- [Universal-Debloater-Alliance/universal-android-debloater-next-generation](https://github.com/Universal-Debloater-Alliance/universal-android-debloater-next-generation) - Cross-platform GUI written in Rust using ADB to debloat non-rooted Android devices. Improve your privacy, the security and battery life of your device.
   - Updated on `Sun 07 June 2026 at 10:15 PM` | 6771⭐ | `GNU General Public License v3.0`
- [tw93/Pake](https://github.com/tw93/Pake) - 🤱🏻 Turn any webpage into a desktop app with one command.
   - Updated on `Sun 07 June 2026 at 02:57 PM` | 50240⭐ | `MIT License`
- [ajeetdsouza/zoxide](https://github.com/ajeetdsouza/zoxide) - A smarter cd command. Supports all major shells.
   - Updated on `Thu 21 May 2026 at 06:43 PM` | 37391⭐ | `MIT License`
- [Orange-OpenSource/hurl](https://github.com/Orange-OpenSource/hurl) - Hurl, run and test HTTP requests with plain text.
   - Updated on `Tue 09 June 2026 at 04:22 PM` | 18997⭐ | `Apache License 2.0`
- [astral-sh/rye](https://github.com/astral-sh/rye) - a Hassle-Free Python Experience.
   - Updated on `Thu 05 February 2026 at 12:39 AM` | 14217⭐ | `MIT License`
- [fish-shell/fish-shell](https://github.com/fish-shell/fish-shell) - The user-friendly command line shell.
   - Updated on `Tue 09 June 2026 at 03:45 PM` | 33636⭐ | `Other`
- [neondatabase/neon](https://github.com/neondatabase/neon) - Neon: Serverless Postgres. We separated storage and compute to offer autoscaling, code-like database branching, and scale to zero.
   - Updated on `Mon 25 May 2026 at 01:07 PM` | 22184⭐ | `Apache License 2.0`
- [rust-lang/rust](https://github.com/rust-lang/rust) - Empowering everyone to build reliable and efficient software.
   - Updated on `Tue 09 June 2026 at 09:51 PM` | 113732⭐ | `Apache License 2.0`
- [ekzhang/sshx](https://github.com/ekzhang/sshx) - Fast, collaborative live terminal sharing over the web.
   - Updated on `Thu 19 June 2025 at 02:09 PM` | 7502⭐ | `MIT License`
- [lapce/lapce](https://github.com/lapce/lapce) - Lightning-fast and Powerful Code Editor written in Rust.
   - Updated on `Tue 09 June 2026 at 01:06 AM` | 38621⭐ | `Apache License 2.0`
- [rust-unofficial/awesome-rust](https://github.com/rust-unofficial/awesome-rust) - A curated list of Rust code and resources.
   - Updated on `Tue 09 June 2026 at 07:18 AM` | 57780⭐ | `Creative Commons Zero v1.0 Universal`
- [juspay/hyperswitch](https://github.com/juspay/hyperswitch) - Open source, composable payments platform | PCI compliant | SaaS and Self-host options | Enables connectivity to multiple payment, payout, fraud, vault and tokenization providers | Uplifts authorizati.
   - Updated on `Tue 09 June 2026 at 09:43 PM` | 42839⭐ | `Apache License 2.0`
- [LemmyNet/lemmy](https://github.com/LemmyNet/lemmy) - 🐀 A decentralised discussion platform for communities.
   - Updated on `Tue 09 June 2026 at 03:38 PM` | 14428⭐ | `GNU Affero General Public License v3.0`
- [lidotcircle/portForwarder](https://github.com/lidotcircle/portForwarder) - a small tool for forwarding TCP or UDP traffic based on first packet sent by client.
   - Updated on `Fri 27 March 2026 at 12:03 AM` | 84⭐ | `MIT License`
- [mCaptcha/mCaptcha](https://github.com/mCaptcha/mCaptcha) - A no-nonsense CAPTCHA system with seamless UX | Backend component.
   - Updated on `Tue 07 October 2025 at 08:47 AM` | 2460⭐ | `GNU Affero General Public License v3.0`
- [TabbyML/tabby](https://github.com/TabbyML/tabby) - Self-hosted AI coding assistant.
   - Updated on `Mon 02 March 2026 at 08:08 PM` | 33579⭐ | `Other`
- [astral-sh/ruff](https://github.com/astral-sh/ruff) - An extremely fast Python linter and code formatter, written in Rust.
   - Updated on `Tue 09 June 2026 at 11:43 PM` | 47890⭐ | `MIT License`
- [Marekkon5/onetagger](https://github.com/Marekkon5/onetagger) - Music tagger for Windows, MacOS and Linux with Beatport, Discogs, Musicbrainz, Spotify, Traxsource and many other platforms support.
   - Updated on `Fri 13 February 2026 at 09:39 PM` | 1004⭐ | `GNU General Public License v3.0`
- [mcronce/oci-registry](https://github.com/mcronce/oci-registry) - .
   - Updated on `Mon 24 June 2024 at 10:02 PM` | 193⭐ | `MIT License`
- [lldap/lldap](https://github.com/lldap/lldap) - Light LDAP implementation.
   - Updated on `Tue 26 May 2026 at 10:06 PM` | 6293⭐ | `GNU General Public License v3.0`
- [ekzhang/bore](https://github.com/ekzhang/bore) - 🕳 bore is a simple CLI tool for making tunnels to localhost.
   - Updated on `Wed 04 February 2026 at 06:00 PM` | 11219⭐ | `MIT License`
- [szabodanika/microbin](https://github.com/szabodanika/microbin) - A secure, configurable file-sharing and URL shortening web app written in Rust.
   - Updated on `Thu 26 March 2026 at 10:13 AM` | 4389⭐ | `BSD 3-Clause "New" or "Revised" License`
- [spacedriveapp/spacedrive](https://github.com/spacedriveapp/spacedrive) - Spacedrive is an open source cross-platform file explorer, powered by a virtual distributed filesystem written in Rust.
   - Updated on `Sun 19 April 2026 at 08:27 AM` | 38250⭐ | `Other`
- [rustdesk/rustdesk](https://github.com/rustdesk/rustdesk) - An open-source remote desktop application designed for self-hosting, as an alternative to TeamViewer.
   - Updated on `Tue 09 June 2026 at 11:42 PM` | 115917⭐ | `GNU Affero General Public License v3.0`
- [0x192/universal-android-debloater](https://github.com/0x192/universal-android-debloater) - Cross-platform GUI written in Rust using ADB to debloat non-rooted android devices. Improve your privacy, the security and battery life of your device.
   - Updated on `Fri 02 August 2024 at 04:16 PM` | 19452⭐ | `GNU General Public License v3.0`
- [x89/Solana-Arbitrage-Bot](https://github.com/x89/Solana-Arbitrage-Bot) - https://t.me/githubx89 Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Arbitrage Bot Solana Ar.
   - Updated on `Fri 08 May 2026 at 08:27 AM` | 1167⭐ | `MIT License`
- [dani-garcia/vaultwarden](https://github.com/dani-garcia/vaultwarden) - Unofficial Bitwarden compatible server written in Rust, formerly known as bitwarden_rs.
   - Updated on `Fri 05 June 2026 at 07:52 PM` | 62127⭐ | `GNU Affero General Public License v3.0`
- [vv9k/dockeye](https://github.com/vv9k/dockeye) - GUI app for managing Docker/Podman.
   - Updated on `Thu 07 July 2022 at 06:22 PM` | 411⭐ | `Other`
- [Dusk-Labs/dim](https://github.com/Dusk-Labs/dim) - Dim, a media manager fueled by dark forces.
   - Updated on `Sun 21 September 2025 at 11:13 PM` | 4081⭐ | `GNU Affero General Public License v3.0`
- [libreddit/libreddit](https://github.com/libreddit/libreddit) - Private front-end for Reddit.
   - Updated on `Sat 15 February 2025 at 07:11 PM` | 5193⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## SCSS 

- [zkreations/tooltips](https://github.com/zkreations/tooltips) - A lightweight, powerful solution that doesn’t rely on JavaScript.
   - Updated on `Tue 28 April 2026 at 12:23 AM` | 32⭐ | `MIT License`
- [helloflask/bootstrap-flask](https://github.com/helloflask/bootstrap-flask) - Bootstrap 4 & 5 helper for your Flask projects.
   - Updated on `Fri 20 February 2026 at 11:26 PM` | 1196⭐ | `Other`
- [prayag17/JellySkin](https://github.com/prayag17/JellySkin) - Vibrante/minimal Jellyfin CSS using custom Icons and more!!, created for Jellyfin web :film_strip:, can be used by just one line.
   - Updated on `Mon 01 July 2024 at 08:05 AM` | 1006⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Shell 

- [rtvkiz/minimal](https://github.com/rtvkiz/minimal) - Minimal CVE Hardened container image collection.
   - Updated on `Tue 09 June 2026 at 11:59 PM` | 489⭐ | `Other`
- [pgautoupgrade/docker-pgautoupgrade](https://github.com/pgautoupgrade/docker-pgautoupgrade) - A PostgreSQL Docker container that automatically upgrades your database.
   - Updated on `Tue 26 May 2026 at 11:55 AM` | 1146⭐ | `MIT License`
- [awesome-jellyfin/awesome-jellyfin](https://github.com/awesome-jellyfin/awesome-jellyfin) - A Collection of Awesome Jellyfin Plugins, Themes, Guides and More!.
   - Updated on `Sat 06 June 2026 at 11:18 PM` | 8131⭐ | `Creative Commons Zero v1.0 Universal`
- [jdlawrie/simplelogin-docker](https://github.com/jdlawrie/simplelogin-docker) - .
   - Updated on `Mon 07 October 2024 at 11:22 AM` | 6⭐ | `GNU General Public License v3.0`
- [j-hc/revanced-magisk-module](https://github.com/j-hc/revanced-magisk-module) - Extensive ReVanced builder. Builds both modules and APKs. Updated daily.
   - Updated on `Fri 29 May 2026 at 08:19 PM` | 7307⭐ | `GNU General Public License v3.0`
- [anasfanani/magisk-autoboot](https://github.com/anasfanani/magisk-autoboot) - a Magisk module to enable automatic booting/for turning on of your Android device when it's connected to a charger or USB.
   - Updated on `Sat 08 June 2024 at 09:32 AM` | 237⭐ | `Other`
- [dockur/windows](https://github.com/dockur/windows) - Windows inside a Docker container.
   - Updated on `Sat 06 June 2026 at 07:38 AM` | 51649⭐ | `MIT License`
- [GloriousEggroll/wine-ge-custom](https://github.com/GloriousEggroll/wine-ge-custom) - My custom build of wine, made to use with lutris. Built with lutris's buildbot.
   - Updated on `Fri 11 July 2025 at 11:36 PM` | 2174⭐ | `❌`
- [go-nv/goenv](https://github.com/go-nv/goenv) - :blue_car: Like pyenv and rbenv, but for Go.
   - Updated on `Tue 09 June 2026 at 06:36 PM` | 2513⭐ | `MIT License`
- [nvm-sh/nvm](https://github.com/nvm-sh/nvm) - Node Version Manager - POSIX-compliant bash script to manage multiple active node.js versions.
   - Updated on `Thu 04 June 2026 at 10:24 PM` | 93731⭐ | `MIT License`
- [nodesource/distributions](https://github.com/nodesource/distributions) - NodeSource Node.js Binary Distributions.
   - Updated on `Tue 05 May 2026 at 04:29 PM` | 13832⭐ | `MIT License`
- [just-containers/s6-overlay](https://github.com/just-containers/s6-overlay) - s6 overlay for containers (includes execline, s6-linux-utils & a custom init).
   - Updated on `Tue 02 June 2026 at 11:11 PM` | 4501⭐ | `Other`
- [cisagov/LME](https://github.com/cisagov/LME) - Logging Made Easy (LME) is a no cost, open source platform that centralizes log collection, enhances threat detection, and enables real-time alerting, helping small to medium-sized organizations secur.
   - Updated on `Fri 22 May 2026 at 05:05 PM` | 1435⭐ | `Other`
- [pyenv/pyenv](https://github.com/pyenv/pyenv) - Simple Python version management.
   - Updated on `Thu 04 June 2026 at 07:36 PM` | 44877⭐ | `MIT License`
- [framps/raspiBackup](https://github.com/framps/raspiBackup) - Create and keep multiple backup versions of your running Raspberries.
   - Updated on `Tue 09 June 2026 at 04:28 PM` | 1001⭐ | `GNU General Public License v3.0`
- [VSCodium/vscodium](https://github.com/VSCodium/vscodium) - binary releases of VS Code without MS branding/telemetry/licensing.
   - Updated on `Thu 04 June 2026 at 01:37 AM` | 31856⭐ | `MIT License`
- [uazo/bromite-buildtools](https://github.com/uazo/bromite-buildtools) - my build machine for bromite development.
   - Updated on `Thu 20 July 2023 at 01:14 PM` | 311⭐ | `GNU General Public License v3.0`
- [alexbelgium/hassio-addons](https://github.com/alexbelgium/hassio-addons) - My homeassistant addons.
   - Updated on `Mon 08 June 2026 at 06:15 PM` | 2501⭐ | `MIT License`
- [masonr/yet-another-bench-script](https://github.com/masonr/yet-another-bench-script) - YABS - a simple bash script to estimate Linux server performance using fio, iperf3, & Geekbench.
   - Updated on `Tue 12 May 2026 at 03:40 AM` | 6475⭐ | `Do What The F*ck You Want To Public License`
- [tianon/docker-postgres-upgrade](https://github.com/tianon/docker-postgres-upgrade) - a PoC for using "pg_upgrade" inside Docker -- learn from it, adapt it for your needs; don't expect it to work as-is!.
   - Updated on `Sat 06 June 2026 at 05:16 AM` | 1364⭐ | `MIT License`
- [pi-hole/docker-pi-hole](https://github.com/pi-hole/docker-pi-hole) - The official Pi-hole Docker image from pi-hole.net.
   - Updated on `Sat 06 June 2026 at 10:13 AM` | 11184⭐ | `Other`
- [RandomNinjaAtk/docker-amd](https://github.com/RandomNinjaAtk/docker-amd) - AMD is a Lidarr companion script to automatically download music for Lidarr.
   - Updated on `Sat 16 July 2022 at 01:32 PM` | 220⭐ | `GNU General Public License v3.0`
- [swizzin/swizzin](https://github.com/swizzin/swizzin) - A simple, modular seedbox solution.
   - Updated on `Mon 18 May 2026 at 09:09 PM` | 2364⭐ | `GNU General Public License v3.0`
- [pi-hole/PADD](https://github.com/pi-hole/PADD) - PADD (formerly Chronometer2) is a more expansive version of the original chronometer.sh that is included with Pi-Hole. PADD provides in-depth information about your Pi-hole.
   - Updated on `Sat 06 June 2026 at 10:04 AM` | 1539⭐ | `❌`
- [GhostWriters/DockSTARTer](https://github.com/GhostWriters/DockSTARTer) - DockSTARTer helps you get started with running apps in Docker.
   - Updated on `Wed 03 June 2026 at 07:39 PM` | 2559⭐ | `MIT License`
- [pi-hole/pi-hole](https://github.com/pi-hole/pi-hole) - A black hole for Internet advertisements.
   - Updated on `Mon 08 June 2026 at 08:27 PM` | 59202⭐ | `Other`
- [elmerfds/OrganizrInstaller](https://github.com/elmerfds/OrganizrInstaller) - Automated install script for Organizr (https://github.com/causefx/Organizr) for Ubuntu/Debian/Raspbian/CentOS and Windows.
   - Updated on `Mon 13 May 2024 at 01:21 PM` | 132⭐ | `GNU General Public License v3.0`
- [dnschneid/crouton](https://github.com/dnschneid/crouton) - Chromium OS Universal Chroot Environment (EOL).
   - Updated on `Sun 30 March 2025 at 01:56 AM` | 8583⭐ | `BSD 3-Clause "New" or "Revised" License`
 
**[`^        back to top        ^`](#)**

## Svelte 

- [seanmorley15/AdventureLog](https://github.com/seanmorley15/AdventureLog) - Self-hostable travel tracker and trip planner.
   - Updated on `Mon 08 June 2026 at 01:39 AM` | 3301⭐ | `Other`
- [johanohly/AirTrail](https://github.com/johanohly/AirTrail) - A modern, open-source personal flight tracking system.
   - Updated on `Sat 06 June 2026 at 10:27 PM` | 1123⭐ | `GNU General Public License v3.0`
- [Finsys/dockhand](https://github.com/Finsys/dockhand) - Dockhand - Docker management you will like.
   - Updated on `Mon 08 June 2026 at 04:08 PM` | 4782⭐ | `Other`
- [saadeghi/daisyui](https://github.com/saadeghi/daisyui) - 🌼 🌼 🌼 🌼 🌼  The most popular, free and open-source Tailwind CSS component library.
   - Updated on `Tue 09 June 2026 at 10:56 PM` | 41095⭐ | `MIT License`
- [WardPearce/paaster](https://github.com/WardPearce/paaster) - Paaster is a secure and user-friendly pastebin application that prioritizes privacy and simplicity. With end-to-end encryption and paste history, Paaster ensures that your pasted code remains confiden.
   - Updated on `Mon 08 June 2026 at 01:21 PM` | 545⭐ | `GNU Affero General Public License v3.0`
- [open-wanderer/wanderer](https://github.com/open-wanderer/wanderer) - wanderer is a self-hosted trail database. Save your adventures!.
   - Updated on `Tue 09 June 2026 at 08:09 PM` | 3650⭐ | `GNU Affero General Public License v3.0`
- [serge-chat/serge](https://github.com/serge-chat/serge) - A web interface for chatting with Alpaca through llama.cpp. Fully dockerized, with an easy to use API.
   - Updated on `Fri 21 November 2025 at 08:07 AM` | 5725⭐ | `Apache License 2.0`
- [beromir/Servas](https://github.com/beromir/Servas) - A self-hosted bookmark management tool.
   - Updated on `Wed 15 April 2026 at 08:09 PM` | 815⭐ | `GNU General Public License v3.0`
- [proofrock/pupcloud](https://github.com/proofrock/pupcloud) - [SUSPENDED] A portable web file manager and gallery.
   - Updated on `Tue 31 January 2023 at 09:44 AM` | 133⭐ | `GNU General Public License v3.0`
- [machengim/oasis](https://github.com/machengim/oasis) - Self-hosted file server.
   - Updated on `Sat 28 January 2023 at 01:22 AM` | 387⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Swift 

- [rileytestut/Delta](https://github.com/rileytestut/Delta) - Delta is an all-in-one classic video game emulator for non-jailbroken iOS devices.
   - Updated on `Wed 03 June 2026 at 07:34 PM` | 5980⭐ | `GNU Affero General Public License v3.0`
- [Whisky-App/Whisky](https://github.com/Whisky-App/Whisky) - A modern Wine wrapper for macOS built with SwiftUI.
   - Updated on `Sun 11 May 2025 at 05:50 PM` | 15089⭐ | `GNU General Public License v3.0`
- [open-meteo/open-meteo](https://github.com/open-meteo/open-meteo) - Free Weather Forecast API for non-commercial use.
   - Updated on `Tue 09 June 2026 at 08:58 PM` | 5641⭐ | `GNU Affero General Public License v3.0`
- [ZonD80/duppy](https://github.com/ZonD80/duppy) - iOS/iPadOS App duplicator for jailbroken devices.
   - Updated on `Tue 14 April 2026 at 05:04 PM` | 85⭐ | `GNU General Public License v3.0`
- [zbrateam/Zebra](https://github.com/zbrateam/Zebra) - 🦓 A Useful Package Manager for iOS.
   - Updated on `Wed 01 April 2026 at 01:42 PM` | 1260⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Text 

- [hagezi/dns-blocklists](https://github.com/hagezi/dns-blocklists) - DNS-Blocklists: For a better internet - keep the internet clean!.
   - Updated on `Tue 09 June 2026 at 07:16 PM` | 23586⭐ | `GNU General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## TypeScript 

- [skillsgate/skillsgate](https://github.com/skillsgate/skillsgate) - .
   - Updated on `Mon 08 June 2026 at 03:38 AM` | 758⭐ | `MIT License`
- [CarrotRub/Fit-Launcher](https://github.com/CarrotRub/Fit-Launcher) - A better version of my previous Fitgirl's Repacks Launcher. Made using Tauri with SolidJS.
   - Updated on `Fri 05 June 2026 at 05:20 AM` | 1133⭐ | `GNU General Public License v3.0`
- [MrGray17/opentoken](https://github.com/MrGray17/opentoken) - Token-saving companion for OpenCode — 42 compression layers, zero risk, no caveman speak.
   - Updated on `Fri 29 May 2026 at 11:09 PM` | 137⭐ | `MIT License`
- [logabell/GameVault](https://github.com/logabell/GameVault) - Local-first desktop app and browser extension for tracking your PC game library, source updates, and SteamDB patch history.
   - Updated on `Sun 24 May 2026 at 02:39 AM` | 9⭐ | `GNU General Public License v3.0`
- [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) - Pre-indexed code knowledge graph for Claude Code, Codex, Gemini, Cursor, OpenCode, AntiGravity, Kiro, and Hermes Agent — fewer tokens, fewer tool calls, 100% local.
   - Updated on `Tue 09 June 2026 at 04:53 PM` | 45895⭐ | `MIT License`
- [grojeda/opencode-config](https://github.com/grojeda/opencode-config) - .
   - Updated on `Sun 07 June 2026 at 09:48 AM` | 23⭐ | `❌`
- [proxysoul/soulforge](https://github.com/proxysoul/soulforge) - AI coding agent that edits symbols, not strings. AST surgery, full LSP, and a live code graph wired to memory that resurfaces by file, co-change, and semantics.
   - Updated on `Tue 09 June 2026 at 09:42 PM` | 768⭐ | `Other`
- [millionco/react-doctor](https://github.com/millionco/react-doctor) - Your agent writes bad React. This catches it.
   - Updated on `Wed 10 June 2026 at 12:25 AM` | 12515⭐ | `MIT License`
- [playcanvas/supersplat](https://github.com/playcanvas/supersplat) - 3D Gaussian Splat Editor.
   - Updated on `Thu 04 June 2026 at 02:38 PM` | 9085⭐ | `MIT License`
- [Lazztech/Libre-Closet](https://github.com/Lazztech/Libre-Closet) - Free, open-source, self-hosted wardrobe organizer. Catalog clothes, build outfits, PWA. Docker one-liner deploy.
   - Updated on `Tue 09 June 2026 at 11:23 PM` | 231⭐ | `GNU Affero General Public License v3.0`
- [OpenCloudGaming/OpenNOW](https://github.com/OpenCloudGaming/OpenNOW) - Custom GeForce Now Client Named OpenNOW.
   - Updated on `Mon 08 June 2026 at 09:01 PM` | 2068⭐ | `MIT License`
- [mauriceboe/TREK](https://github.com/mauriceboe/TREK) - A self-hosted travel/trip planner with real-time collaboration, interactive maps, PWA support, SSO, budgets, packing lists, and more.
   - Updated on `Tue 09 June 2026 at 02:03 PM` | 5559⭐ | `GNU Affero General Public License v3.0`
- [arpanghosh8453/open-dronelog](https://github.com/arpanghosh8453/open-dronelog) - Drone Log analyzer: A high-performance universal dashboard application for organizing and analyzing DJI/Litchi flight logs privately in one place. Supports plugin for custom flight log formats. Built .
   - Updated on `Tue 12 May 2026 at 06:12 AM` | 1416⭐ | `GNU Affero General Public License v3.0`
- [Crosstalk-Solutions/project-nomad](https://github.com/Crosstalk-Solutions/project-nomad) - Project N.O.M.A.D, is a self-contained, offline survival computer packed with critical tools, knowledge, and AI to keep you informed and empowered—anytime, anywhere.
   - Updated on `Tue 09 June 2026 at 09:21 PM` | 30287⭐ | `Apache License 2.0`
- [horlesq/gamelog](https://github.com/horlesq/gamelog) - A full-stack web application designed to track your video-game library.
   - Updated on `Sat 21 March 2026 at 06:52 AM` | 3⭐ | `MIT License`
- [altcha-org/altcha](https://github.com/altcha-org/altcha) - GDPR, WCAG 2.2 AA, and EAA compliant, self-hosted CAPTCHA alternative with PoW mechanism.
   - Updated on `Sun 07 June 2026 at 11:18 AM` | 2333⭐ | `MIT License`
- [connorgallopo/Tracearr](https://github.com/connorgallopo/Tracearr) - Real-time monitoring for Plex, Jellyfin, and Emby servers. Track streams, analyze playback, and detect account sharing from a single dashboard.
   - Updated on `Mon 08 June 2026 at 05:53 PM` | 1992⭐ | `Other`
- [Namo2/InPlayerEpisodePreview](https://github.com/Namo2/InPlayerEpisodePreview) - A Jellyfin plugin for the web and client version, which displayes an episode list inside the player.
   - Updated on `Fri 29 May 2026 at 04:53 PM` | 372⭐ | `MIT License`
- [anomalyco/opencode](https://github.com/anomalyco/opencode) - The open source coding agent.
   - Updated on `Wed 10 June 2026 at 12:31 AM` | 172149⭐ | `MIT License`
- [smashah/receipthero-ng](https://github.com/smashah/receipthero-ng) - Organize all your receipts with AI (with Paperless NGX support).
   - Updated on `Mon 13 April 2026 at 03:13 PM` | 69⭐ | `❌`
- [spotbye/SpotiFLAC](https://github.com/spotbye/SpotiFLAC) - Get Spotify tracks in true FLAC from Tidal, Qobuz & Amazon Music — no account required.
   - Updated on `Mon 08 June 2026 at 11:29 PM` | 9107⭐ | `MIT License`
- [puckeditor/puck](https://github.com/puckeditor/puck) - The visual editor for React.
   - Updated on `Tue 09 June 2026 at 06:52 AM` | 12782⭐ | `MIT License`
- [C4illin/ConvertX](https://github.com/C4illin/ConvertX) - 💾 Self-hosted online file converter. Supports 1000+ formats ⚙️.
   - Updated on `Mon 27 April 2026 at 07:43 PM` | 16981⭐ | `GNU Affero General Public License v3.0`
- [rowboatlabs/rowboat](https://github.com/rowboatlabs/rowboat) - Open-source AI coworker, with memory.
   - Updated on `Tue 09 June 2026 at 06:47 PM` | 14940⭐ | `Apache License 2.0`
- [bknd-io/bknd](https://github.com/bknd-io/bknd) - Lightweight Firebase/Supabase alternative built to run anywhere — incl. Next.js, React Router, Astro, Cloudflare, Bun, Node, AWS Lambda & more.
   - Updated on `Mon 06 April 2026 at 12:59 PM` | 3727⭐ | `Apache License 2.0`
- [srbhr/Resume-Matcher](https://github.com/srbhr/Resume-Matcher) - Improve your resumes with Resume Matcher. Get insights, keyword suggestions and tune your resumes to job descriptions.
   - Updated on `Thu 04 June 2026 at 12:31 PM` | 27315⭐ | `Apache License 2.0`
- [better-auth/better-auth](https://github.com/better-auth/better-auth) - The most comprehensive authentication framework.
   - Updated on `Wed 10 June 2026 at 12:32 AM` | 28653⭐ | `MIT License`
- [browserbase/stagehand](https://github.com/browserbase/stagehand) - The SDK For Browser Agents.
   - Updated on `Tue 09 June 2026 at 11:37 PM` | 23049⭐ | `MIT License`
- [iib0011/omni-tools](https://github.com/iib0011/omni-tools) - Self-hosted collection of powerful web-based tools for everyday tasks. No ads, no tracking, just fast, accessible utilities right from your browser!.
   - Updated on `Mon 04 May 2026 at 08:25 PM` | 9618⭐ | `MIT License`
- [Infisical/infisical](https://github.com/Infisical/infisical) - Infisical is the open-source platform for secrets, certificates, and privileged access management.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 27280⭐ | `Other`
- [NginxProxyManager/nginx-proxy-manager](https://github.com/NginxProxyManager/nginx-proxy-manager) - Docker container for managing Nginx proxy hosts with a simple, powerful interface.
   - Updated on `Wed 10 June 2026 at 12:21 AM` | 33197⭐ | `MIT License`
- [onlook-dev/onlook](https://github.com/onlook-dev/onlook) - The Cursor for Designers • An Open-Source AI-First Design tool • Visually build, style, and edit your React App with AI.
   - Updated on `Tue 09 June 2026 at 11:18 PM` | 25896⭐ | `Apache License 2.0`
- [visgl/deck.gl](https://github.com/visgl/deck.gl) - WebGL2 powered visualization framework.
   - Updated on `Tue 09 June 2026 at 11:43 PM` | 14236⭐ | `MIT License`
- [vitejs/vite](https://github.com/vitejs/vite) - Next generation frontend tooling. It's fast!.
   - Updated on `Tue 09 June 2026 at 07:53 PM` | 81357⭐ | `MIT License`
- [voideditor/void](https://github.com/voideditor/void) - .
   - Updated on `Tue 02 June 2026 at 10:03 PM` | 28812⭐ | `Apache License 2.0`
- [linshenkx/prompt-optimizer](https://github.com/linshenkx/prompt-optimizer) - An AI prompt optimizer for writing better prompts and getting better AI results.
   - Updated on `Tue 09 June 2026 at 07:34 AM` | 30652⭐ | `Other`
- [GoodRxOSS/lifecycle](https://github.com/GoodRxOSS/lifecycle) - Lifecycle is a tool that transforms pull requests into ephemeral development environments that seamlessly connect to required dependencies while remaining isolated from unrelated changes.
   - Updated on `Mon 08 June 2026 at 09:40 PM` | 78⭐ | `Apache License 2.0`
- [streamyfin/streamyfin](https://github.com/streamyfin/streamyfin) - A modern Jellyfin client built with Expo.
   - Updated on `Tue 09 June 2026 at 11:22 PM` | 4912⭐ | `Mozilla Public License 2.0`
- [fredrikburmester/streamystats](https://github.com/fredrikburmester/streamystats) - Streamystats is a statistics service for Jellyfin, providing analytics and data visualization.
   - Updated on `Sun 31 May 2026 at 05:24 AM` | 742⭐ | `❌`
- [tj-actions/changed-files](https://github.com/tj-actions/changed-files) - :octocat: Github action to retrieve all (added, copied, modified, deleted, renamed, type changed, unmerged, unknown) files and directories.
   - Updated on `Mon 08 June 2026 at 01:33 AM` | 2701⭐ | `MIT License`
- [TupleType/greenholts.com](https://github.com/TupleType/greenholts.com) - My Portfolio website featuring a blog, built with Next.js and React.
   - Updated on `Tue 19 May 2026 at 03:15 PM` | 4⭐ | `MIT License`
- [Floorp-Projects/Floorp](https://github.com/Floorp-Projects/Floorp) - All of source code of Floorp 12, the most Advanced and Fastest Firefox derivative 🦊.
   - Updated on `Tue 09 June 2026 at 09:35 AM` | 8198⭐ | `Mozilla Public License 2.0`
- [fosrl/pangolin](https://github.com/fosrl/pangolin) - Identity-aware VPN and tunneled reverse proxy for remote access based on WireGuard®.
   - Updated on `Tue 09 June 2026 at 11:57 PM` | 21082⭐ | `Other`
- [activepieces/activepieces](https://github.com/activepieces/activepieces) - AI Agents & MCPs & AI Workflow Automation • (~400 MCP servers for AI agents) • AI Automation / AI Agent with MCPs • AI Workflows & AI Agents • MCPs for AI Agents.
   - Updated on `Wed 10 June 2026 at 12:26 AM` | 22670⭐ | `Other`
- [bluewave-labs/Checkmate](https://github.com/bluewave-labs/Checkmate) - Checkmate is an open-source, self-hosted tool designed to track and monitor server hardware, uptime, response times, and incidents in real-time with beautiful visualizations. Don't be shy, join here: .
   - Updated on `Tue 09 June 2026 at 07:40 PM` | 9969⭐ | `GNU Affero General Public License v3.0`
- [al1abb/invoify](https://github.com/al1abb/invoify) - An invoice generator app built using Next.js, Typescript, and Shadcn.
   - Updated on `Thu 21 May 2026 at 09:43 PM` | 6302⭐ | `MIT License`
- [tim-smart/receipts](https://github.com/tim-smart/receipts) - .
   - Updated on `Sat 02 May 2026 at 07:38 AM` | 43⭐ | `❌`
- [didi/xiaoju-survey](https://github.com/didi/xiaoju-survey) - XIAOJUSURVEY is an enterprises form builder and analytics platform that allows users to create questionnaires, exams, polls, quizzes, and analyze data online.
   - Updated on `Thu 02 April 2026 at 03:22 AM` | 3764⭐ | `Apache License 2.0`
- [4gray/iptvnator](https://github.com/4gray/iptvnator) - :tv: Cross-platform IPTV player application with multiple features, such as support of m3u and m3u8 playlists, favorites, TV guide, TV archive/catchup and more.
   - Updated on `Tue 09 June 2026 at 06:58 AM` | 6152⭐ | `MIT License`
- [yamadashy/repomix](https://github.com/yamadashy/repomix) - 📦 Repomix is a powerful tool that packs your entire repository into a single, AI-friendly file. Perfect for when you need to feed your codebase to Large Language Models (LLMs) or other AI tools like C.
   - Updated on `Tue 09 June 2026 at 01:28 PM` | 26122⭐ | `MIT License`
- [microsoft/genaiscript](https://github.com/microsoft/genaiscript) - Automatable GenAI Scripting.
   - Updated on `Thu 21 May 2026 at 01:15 AM` | 2897⭐ | `MIT License`
- [getmaxun/maxun](https://github.com/getmaxun/maxun) - 🔥 The open-source no-code platform for web scraping, crawling, search and AI data extraction • Turn websites into structured APIs in minutes 🔥.
   - Updated on `Mon 08 June 2026 at 01:21 PM` | 15828⭐ | `GNU Affero General Public License v3.0`
- [omnivore-app/omnivore](https://github.com/omnivore-app/omnivore) - Omnivore is a complete, open source read-it-later solution for people who like reading.
   - Updated on `Thu 26 March 2026 at 02:22 PM` | 16095⭐ | `GNU Affero General Public License v3.0`
- [sbondCo/Casterr](https://github.com/sbondCo/Casterr) - Screen recorder, editor and video browser with the main goal of easing the process of recording and clipping your best moments in-game.
   - Updated on `Tue 16 April 2024 at 08:51 AM` | 29⭐ | `GNU General Public License v3.0`
- [labring/FastGPT](https://github.com/labring/FastGPT) - FastGPT is a knowledge-based platform built on the LLMs, offers a comprehensive suite of out-of-the-box capabilities such as data processing, RAG retrieval, and visual AI workflow orchestration, letti.
   - Updated on `Tue 09 June 2026 at 08:03 AM` | 28327⭐ | `Other`
- [fingerprintjs/fingerprintjs](https://github.com/fingerprintjs/fingerprintjs) - The most advanced free and open-source browser fingerprinting library.
   - Updated on `Wed 27 May 2026 at 08:15 PM` | 27271⭐ | `MIT License`
- [unkeyed/unkey](https://github.com/unkeyed/unkey) - The Developer Platform for Modern APIs.
   - Updated on `Tue 09 June 2026 at 11:08 PM` | 5333⭐ | `Other`
- [cline/cline](https://github.com/cline/cline) - Autonomous coding agent as an SDK, IDE extension, or CLI assistant.
   - Updated on `Wed 10 June 2026 at 12:32 AM` | 62991⭐ | `Apache License 2.0`
- [papermark/papermark](https://github.com/papermark/papermark) - Papermark is the open-source DocSend alternative with built-in analytics and custom domains.
   - Updated on `Tue 09 June 2026 at 08:04 PM` | 8461⭐ | `Other`
- [AykutSarac/jsoncrack.com](https://github.com/AykutSarac/jsoncrack.com) - ✨ Innovative and open-source visualization application that transforms various data formats, such as JSON, YAML, XML and CSV into interactive graphs.
   - Updated on `Fri 15 May 2026 at 08:43 PM` | 44104⭐ | `Apache License 2.0`
- [Litlyx/litlyx](https://github.com/Litlyx/litlyx) - Powerful Analytics Solution. Setup in 30 seconds. Display all your data on a Simple, AI-powered dashboard. Fully self-hostable and GDPR compliant. Alternative to Google Analytics, MixPanel, Plausible,.
   - Updated on `Wed 03 December 2025 at 04:11 PM` | 1728⭐ | `Apache License 2.0`
- [FoxxMD/multi-scrobbler](https://github.com/FoxxMD/multi-scrobbler) - Scrobble plays from multiple sources to multiple clients.
   - Updated on `Tue 09 June 2026 at 08:35 PM` | 1090⭐ | `MIT License`
- [daytonaio/daytona](https://github.com/daytonaio/daytona) - Daytona is a Secure and Elastic Infrastructure for Running AI-Generated Code.
   - Updated on `Tue 09 June 2026 at 06:21 PM` | 72445⭐ | `GNU Affero General Public License v3.0`
- [TheNickOfTime/handbrake-web](https://github.com/TheNickOfTime/handbrake-web) - A self-hosted platform to use HandBrake on your headless devices via a bespoke web interface. Harness the processing power of multiple devices to work on a single queue.
   - Updated on `Fri 27 March 2026 at 02:31 AM` | 776⭐ | `GNU Affero General Public License v3.0`
- [zyachel/libremdb](https://github.com/zyachel/libremdb) - A free & open source IMDb front-end.
   - Updated on `Sun 19 April 2026 at 03:27 PM` | 424⭐ | `GNU Affero General Public License v3.0`
- [RSSNext/Folo](https://github.com/RSSNext/Folo) - 🧡 Folo is the AI RSS Reader.
   - Updated on `Fri 05 June 2026 at 11:44 AM` | 38434⭐ | `GNU Affero General Public License v3.0`
- [jaypyles/Scraperr](https://github.com/jaypyles/Scraperr) - Self-hosted webscraper.
   - Updated on `Sun 12 October 2025 at 04:55 PM` | 4895⭐ | `MIT License`
- [popcorntime/popcorntime](https://github.com/popcorntime/popcorntime) - Popcorn Time™ puts everything in one place. Your favorite platforms, your shows, your movies-ready when you are.
   - Updated on `Tue 09 June 2026 at 04:33 AM` | 10604⭐ | `MIT License`
- [umijs/umi](https://github.com/umijs/umi) - A framework in react community ✨.
   - Updated on `Mon 08 June 2026 at 03:22 PM` | 16030⭐ | `MIT License`
- [openstatusHQ/openstatus](https://github.com/openstatusHQ/openstatus) - 🫖 Status page with uptime monitoring & API monitoring as code   🫖.
   - Updated on `Tue 09 June 2026 at 08:27 PM` | 8741⭐ | `GNU Affero General Public License v3.0`
- [tailwindlabs/tailwindcss](https://github.com/tailwindlabs/tailwindcss) - A utility-first CSS framework for rapid UI development.
   - Updated on `Tue 09 June 2026 at 10:44 AM` | 95462⭐ | `MIT License`
- [trivule/trivule](https://github.com/trivule/trivule) - Trivule is a powerful, user-friendly JavaScript library designed to streamline form validation for developers.
   - Updated on `Tue 28 May 2024 at 03:07 PM` | 55⭐ | `MIT License`
- [realworld-apps/realworld](https://github.com/realworld-apps/realworld) - "The mother of all demo apps" — Exemplary fullstack Medium.com clone powered by React, Angular, Node, Django, and many more.
   - Updated on `Thu 21 May 2026 at 06:23 AM` | 83531⭐ | `Other`
- [nestrilabs/nestri](https://github.com/nestrilabs/nestri) - [Experimental] Deploy and stream games/apps in the cloud. Use our GPUs or bring your own.
   - Updated on `Thu 02 April 2026 at 11:23 PM` | 1709⭐ | `Other`
- [BetterDiscord/BetterDiscord](https://github.com/BetterDiscord/BetterDiscord) - Better Discord enhances Discord desktop app with new features.
   - Updated on `Mon 08 June 2026 at 12:03 AM` | 9093⭐ | `Apache License 2.0`
- [daltonmenezes/aura-theme](https://github.com/daltonmenezes/aura-theme) - ✨ A beautiful dark theme for your favorite apps.
   - Updated on `Sat 20 December 2025 at 12:24 PM` | 3795⭐ | `MIT License`
- [remix-run/remix](https://github.com/remix-run/remix) - Build Better Websites. Create modern, resilient user experiences with web fundamentals.
   - Updated on `Tue 09 June 2026 at 03:49 PM` | 33031⭐ | `MIT License`
- [mantinedev/mantine](https://github.com/mantinedev/mantine) - A fully featured React components library.
   - Updated on `Tue 09 June 2026 at 09:42 AM` | 31231⭐ | `MIT License`
- [Dokploy/dokploy](https://github.com/Dokploy/dokploy) - Open Source Alternative to Vercel, Netlify and Heroku.
   - Updated on `Tue 09 June 2026 at 10:41 PM` | 34708⭐ | `Other`
- [jitsi/jitsi-meet](https://github.com/jitsi/jitsi-meet) - Jitsi Meet - Secure, Simple and Scalable Video Conferences that you use as a standalone app or embed in your web application.
   - Updated on `Wed 10 June 2026 at 12:31 AM` | 29395⭐ | `Apache License 2.0`
- [firecrawl/firecrawl](https://github.com/firecrawl/firecrawl) - The API to search, scrape, and interact with the web at scale. 🔥.
   - Updated on `Tue 09 June 2026 at 11:27 PM` | 130744⭐ | `GNU Affero General Public License v3.0`
- [jina-ai/reader](https://github.com/jina-ai/reader) - Convert any URL to an LLM-friendly input with a simple prefix https://r.jina.ai/.
   - Updated on `Fri 22 May 2026 at 02:56 AM` | 11139⭐ | `Apache License 2.0`
- [hydralauncher/hydra](https://github.com/hydralauncher/hydra) - Hydra Launcher is an open-source gaming platform created to be the single tool that you need.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 15886⭐ | `MIT License`
- [atlassian/pragmatic-drag-and-drop](https://github.com/atlassian/pragmatic-drag-and-drop) - Fast drag and drop for any experience on any tech stack.
   - Updated on `Sat 06 June 2026 at 11:39 AM` | 12660⭐ | `Other`
- [apache/superset](https://github.com/apache/superset) - Apache Superset is a Data Visualization and Data Exploration Platform.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 73222⭐ | `Apache License 2.0`
- [semanser/codel](https://github.com/semanser/codel) - ✨ Fully autonomous AI Agent that can perform complicated tasks and projects using terminal, browser, and editor.
   - Updated on `Mon 29 April 2024 at 05:53 PM` | 2454⭐ | `GNU Affero General Public License v3.0`
- [heyform/heyform](https://github.com/heyform/heyform) - Open-Source Form Builder.
   - Updated on `Wed 03 June 2026 at 02:25 AM` | 8817⭐ | `GNU Affero General Public License v3.0`
- [Expensify/App](https://github.com/Expensify/App) - Welcome to New Expensify: a complete re-imagination of financial collaboration, centered around chat. Help us build the next generation of Expensify by sharing feedback and contributing to the code.
   - Updated on `Wed 10 June 2026 at 12:31 AM` | 4897⭐ | `MIT License`
- [FrigadeHQ/remote-storage](https://github.com/FrigadeHQ/remote-storage) - remoteStorage is a simple library that combines the localStorage API with a remote server to persist data across sessions, devices, and browsers. It works as a simple key value database store and back.
   - Updated on `Wed 27 March 2024 at 12:36 AM` | 1398⭐ | `MIT License`
- [ZoeyVid/NPMplus](https://github.com/ZoeyVid/NPMplus) - a fork of nginx-proxy-manager.
   - Updated on `Tue 09 June 2026 at 11:30 PM` | 2130⭐ | `MIT License`
- [withfig/autocomplete](https://github.com/withfig/autocomplete) - IDE-style autocomplete for your existing terminal & shell.
   - Updated on `Mon 05 May 2025 at 05:15 PM` | 25187⭐ | `MIT License`
- [HeyPuter/puter](https://github.com/HeyPuter/puter) - 🌐 The Internet Computer! Free, Open-Source, and Self-Hostable.
   - Updated on `Wed 10 June 2026 at 12:12 AM` | 42202⭐ | `GNU Affero General Public License v3.0`
- [bootstrap-vue-next/bootstrap-vue-next](https://github.com/bootstrap-vue-next/bootstrap-vue-next) - Seamless integration of Vue 3, Bootstrap 5, and TypeScript for modern, type-safe UI development.
   - Updated on `Fri 05 June 2026 at 02:54 PM` | 1285⭐ | `MIT License`
- [angular/components](https://github.com/angular/components) - Component infrastructure and Material Design components for Angular.
   - Updated on `Tue 09 June 2026 at 07:58 AM` | 25028⭐ | `MIT License`
- [ChatGPTNextWeb/NextChat](https://github.com/ChatGPTNextWeb/NextChat) - ✨ Light and Fast AI Assistant. Support: Web | iOS | MacOS | Android |  Linux | Windows.
   - Updated on `Fri 15 May 2026 at 01:19 AM` | 88207⭐ | `MIT License`
- [hoppscotch/hoppscotch](https://github.com/hoppscotch/hoppscotch) - Open-Source API Development Ecosystem • https://hoppscotch.io • Offline, On-Prem & Cloud • Web, Desktop & CLI • Open-Source Alternative to Postman, Insomnia.
   - Updated on `Tue 09 June 2026 at 05:09 PM` | 79449⭐ | `MIT License`
- [williamtroup/Heat.js](https://github.com/williamtroup/Heat.js) - 🌞 A highly customizable JavaScript library for generating interactive heatmaps. It transforms data into smooth, visually intuitive heat layers, making patterns and intensity easy to spot at a glance.
   - Updated on `Sat 30 May 2026 at 06:52 PM` | 898⭐ | `MIT License`
- [swup/swup](https://github.com/swup/swup) - Versatile and extensible page transition library for server-rendered websites 🎉.
   - Updated on `Tue 09 June 2026 at 10:35 PM` | 5180⭐ | `MIT License`
- [goenning/google-indexing-script](https://github.com/goenning/google-indexing-script) - Script to get your site indexed on Google in less than 48 hours.
   - Updated on `Thu 10 October 2024 at 06:39 PM` | 7654⭐ | `MIT License`
- [danny-avila/LibreChat](https://github.com/danny-avila/LibreChat) - Enhanced ChatGPT Clone: Features Agents, MCP, Skills, DeepSeek, Anthropic, AWS, OpenAI, Responses API, Azure, Groq, o1, GPT-5, Mistral, OpenRouter, Vertex AI, Gemini, Artifacts, AI model switching, me.
   - Updated on `Wed 10 June 2026 at 12:16 AM` | 38770⭐ | `MIT License`
- [Portkey-AI/gateway](https://github.com/Portkey-AI/gateway) - A blazing fast AI Gateway with integrated guardrails. Route to 1,600+ LLMs, 50+ AI Guardrails with 1 fast & friendly API.
   - Updated on `Mon 25 May 2026 at 01:54 PM` | 12019⭐ | `MIT License`
- [linkwarden/linkwarden](https://github.com/linkwarden/linkwarden) - ⚡️⚡️⚡️ Self-hosted collaborative bookmark manager to collect, read, annotate, and fully preserve what matters, all in one place.
   - Updated on `Tue 09 June 2026 at 05:04 PM` | 18563⭐ | `GNU Affero General Public License v3.0`
- [renovatebot/renovate](https://github.com/renovatebot/renovate) - Home of the Renovate CLI: Cross-platform Dependency Automation by Mend.io.
   - Updated on `Tue 09 June 2026 at 05:42 PM` | 21745⭐ | `GNU Affero General Public License v3.0`
- [puppeteer/puppeteer](https://github.com/puppeteer/puppeteer) - JavaScript API for Chrome and Firefox.
   - Updated on `Tue 09 June 2026 at 03:54 PM` | 94504⭐ | `Apache License 2.0`
- [janhq/jan](https://github.com/janhq/jan) - Jan is an open source alternative to ChatGPT that runs 100% offline on your computer.
   - Updated on `Tue 02 June 2026 at 03:00 AM` | 42956⭐ | `Other`
- [typehero/typehero](https://github.com/typehero/typehero) - Connect, collaborate, and grow with a community of TypeScript developers.
   - Updated on `Fri 23 January 2026 at 01:49 AM` | 6628⭐ | `GNU Affero General Public License v3.0`
- [makeplane/plane](https://github.com/makeplane/plane) - 🔥🔥🔥 Open-source Jira, Linear, Monday, and ClickUp alternative. Plane is a modern project management platform to manage tasks, sprints, docs, and triage.
   - Updated on `Tue 09 June 2026 at 09:46 PM` | 50595⭐ | `GNU Affero General Public License v3.0`
- [blakeblackshear/frigate](https://github.com/blakeblackshear/frigate) - NVR with realtime local object detection for IP cameras.
   - Updated on `Tue 09 June 2026 at 09:53 PM` | 33642⭐ | `MIT License`
- [abi/screenshot-to-code](https://github.com/abi/screenshot-to-code) - Drop in a screenshot and convert it to clean code (HTML/Tailwind/React/Vue).
   - Updated on `Fri 05 June 2026 at 07:02 PM` | 72845⭐ | `MIT License`
- [BuilderIO/gpt-crawler](https://github.com/BuilderIO/gpt-crawler) - Crawl a site to generate knowledge files to create your own custom GPT from a URL.
   - Updated on `Mon 07 July 2025 at 03:35 PM` | 22242⭐ | `ISC License`
- [mattermost-community/focalboard](https://github.com/mattermost-community/focalboard) - Focalboard is an open source, self-hosted alternative to Trello, Notion, and Asana.
   - Updated on `Mon 18 May 2026 at 04:05 PM` | 26223⭐ | `Other`
- [tldraw/tldraw](https://github.com/tldraw/tldraw) - very good whiteboard infinite canvas SDK.
   - Updated on `Tue 09 June 2026 at 11:06 PM` | 47661⭐ | `Other`
- [louislam/dockge](https://github.com/louislam/dockge) - A fancy, easy-to-use and reactive self-hosted docker compose.yaml stack-oriented manager.
   - Updated on `Sat 25 April 2026 at 02:28 AM` | 23449⭐ | `MIT License`
- [lobehub/lobehub](https://github.com/lobehub/lobehub) - 🤯 LobeHub is your Chief Agent Operator, organizing your agents into 7×24 operations by hiring, scheduling, and reporting on your entire AI team.
   - Updated on `Wed 10 June 2026 at 12:06 AM` | 78424⭐ | `Other`
- [fabricjs/fabric.js](https://github.com/fabricjs/fabric.js) - Javascript Canvas Library, SVG-to-Canvas (& canvas-to-SVG) Parser.
   - Updated on `Tue 09 June 2026 at 06:15 PM` | 31222⭐ | `MIT License`
- [continuedev/continue](https://github.com/continuedev/continue) - ⏩ Source-controlled AI checks, enforceable in CI. Powered by the open-source Continue CLI.
   - Updated on `Tue 09 June 2026 at 07:11 AM` | 33630⭐ | `Apache License 2.0`
- [ionic-team/ionic-framework](https://github.com/ionic-team/ionic-framework) - A powerful cross-platform UI toolkit for building native-quality iOS, Android, and Progressive Web Apps with HTML, CSS, and JavaScript.
   - Updated on `Tue 09 June 2026 at 11:21 PM` | 52501⭐ | `MIT License`
- [gitroomhq/postiz-app](https://github.com/gitroomhq/postiz-app) - 📨 The ultimate agentic social media scheduling tool 🤖.
   - Updated on `Fri 05 June 2026 at 06:07 AM` | 31648⭐ | `GNU Affero General Public License v3.0`
- [supabase/supabase](https://github.com/supabase/supabase) - The Postgres development platform. Supabase gives you a dedicated Postgres database to build your web, mobile, and AI applications.
   - Updated on `Tue 09 June 2026 at 08:38 PM` | 103921⭐ | `Apache License 2.0`
- [Maintainerr/Maintainerr](https://github.com/Maintainerr/Maintainerr) - Looks and smells like Seerr, does the opposite. A library maintenance tool for Plex and Jellyfin.
   - Updated on `Tue 09 June 2026 at 04:55 PM` | 2028⭐ | `MIT License`
- [aleksilassila/reiverr](https://github.com/aleksilassila/reiverr) - A clean combined interface for Jellyfin, TMDB, Radarr and Sonarr, as well as a replacement to Overseerr.
   - Updated on `Thu 19 February 2026 at 01:29 PM` | 2219⭐ | `GNU Affero General Public License v3.0`
- [TempoWorks/txtdot](https://github.com/TempoWorks/txtdot) - An HTTP proxy that parses only text, links and pictures from pages reducing internet bandwidth usage, removing ads and heavy scripts.
   - Updated on `Tue 09 June 2026 at 06:39 PM` | 208⭐ | `MIT License`
- [actualbudget/actual](https://github.com/actualbudget/actual) - A local-first personal finance app.
   - Updated on `Tue 09 June 2026 at 09:54 PM` | 26937⭐ | `MIT License`
- [RecipeUI/RecipeUI](https://github.com/RecipeUI/RecipeUI) - Discover, test, and share APIs in seconds.
   - Updated on `Fri 05 January 2024 at 04:59 AM` | 1335⭐ | `GNU Affero General Public License v3.0`
- [thepeacockproject/Peacock](https://github.com/thepeacockproject/Peacock) - The Peacock Project is a HITMAN™ World of Assassination trilogy server replacement.
   - Updated on `Tue 09 June 2026 at 06:30 PM` | 651⭐ | `GNU Affero General Public License v3.0`
- [documenso/documenso](https://github.com/documenso/documenso) - The Open Source DocuSign Alternative.
   - Updated on `Tue 09 June 2026 at 02:02 PM` | 13248⭐ | `GNU Affero General Public License v3.0`
- [smol-ai/GodMode](https://github.com/smol-ai/GodMode) - AI Chat Browser: Fast, Full webapp access to ChatGPT / Claude / Bard / Bing / Llama2! I use this 20 times a day.
   - Updated on `Mon 29 July 2024 at 12:31 AM` | 5496⭐ | `MIT License`
- [getumbrel/llama-gpt](https://github.com/getumbrel/llama-gpt) - A self-hosted, offline, ChatGPT-like chatbot. Powered by Llama 2. 100% private, with no data leaving your device. New: Code Llama support!.
   - Updated on `Tue 23 April 2024 at 06:56 PM` | 10950⭐ | `MIT License`
- [profclems/compozify](https://github.com/profclems/compozify) - Convert "docker run" commands to docker compose files.
   - Updated on `Sun 20 August 2023 at 12:35 PM` | 60⭐ | `MIT License`
- [mattwebbio/orbital-sync](https://github.com/mattwebbio/orbital-sync) - Synchronize multiple Pi-hole instances.
   - Updated on `Mon 31 March 2025 at 02:45 PM` | 973⭐ | `MIT License`
- [wei/socialify](https://github.com/wei/socialify) - 💞 Socialify your project. 🌐 Share with the world!.
   - Updated on `Tue 12 May 2026 at 05:52 AM` | 2201⭐ | `MIT License`
- [a16z-infra/ai-getting-started](https://github.com/a16z-infra/ai-getting-started) - A Javascript AI getting started stack for weekend projects, including image/text models, vector stores, auth, and deployment configs.
   - Updated on `Wed 21 August 2024 at 12:35 PM` | 4141⭐ | `MIT License`
- [angular/angular](https://github.com/angular/angular) - Deliver web apps with confidence 🚀.
   - Updated on `Wed 10 June 2026 at 12:12 AM` | 100301⭐ | `MIT License`
- [pavi2410/PlayBadges](https://github.com/pavi2410/PlayBadges) - Show off your Play Store™ app's downloads and ratings in your repo.
   - Updated on `Sat 24 January 2026 at 06:16 PM` | 105⭐ | `❌`
- [latentcat/qrbtf](https://github.com/latentcat/qrbtf) - AI & parametric QR code generator. AI & 参数化二维码生成器。https://qrbtf.com.
   - Updated on `Thu 17 April 2025 at 08:22 AM` | 6940⭐ | `GNU General Public License v3.0`
- [WAppAI/assistant](https://github.com/WAppAI/assistant) - A WhatsApp chatbot that leverages Bing AI's and others LLMs conversational capabilities.
   - Updated on `Mon 04 November 2024 at 05:17 AM` | 125⭐ | `MIT License`
- [IgnisDa/ryot](https://github.com/IgnisDa/ryot) - Roll your own tracker!.
   - Updated on `Sun 07 June 2026 at 02:56 PM` | 3331⭐ | `GNU General Public License v3.0`
- [MarconLP/loggl](https://github.com/MarconLP/loggl) - Send push notifications to your phone or desktop using HTTP requests.
   - Updated on `Sat 06 April 2024 at 09:25 PM` | 121⭐ | `Other`
- [Stirling-Tools/Stirling-PDF](https://github.com/Stirling-Tools/Stirling-PDF) - #1 PDF Application on GitHub that lets you edit PDFs on any device anywhere.
   - Updated on `Wed 10 June 2026 at 12:24 AM` | 80520⭐ | `Other`
- [HemmeligOrg/Hemmelig.app](https://github.com/HemmeligOrg/Hemmelig.app) - Keep your sensitive information out of chat logs, emails, and more with encrypted secrets.
   - Updated on `Sat 06 June 2026 at 12:05 AM` | 1212⭐ | `Other`
- [amruthpillai/reactive-resume](https://github.com/amruthpillai/reactive-resume) - A one-of-a-kind resume builder that keeps your privacy in mind. Completely secure, customizable, portable, open-source and free forever. Try it out today!.
   - Updated on `Sat 06 June 2026 at 07:33 AM` | 38340⭐ | `MIT License`
- [mckaywrigley/ai-code-translator](https://github.com/mckaywrigley/ai-code-translator) - Use AI to translate code from one language to another.
   - Updated on `Thu 21 December 2023 at 12:19 AM` | 4164⭐ | `❌`
- [homarr-labs/dashboard-icons](https://github.com/homarr-labs/dashboard-icons) - Your definitive source for dashboard icons.
   - Updated on `Tue 09 June 2026 at 01:06 PM` | 8264⭐ | `Apache License 2.0`
- [tbxark/ChatGPT-Telegram-Workers](https://github.com/tbxark/ChatGPT-Telegram-Workers) - Easily deploy your Telegram ChatGPT bot on Cloudflare Workers (or Vercel, Docker...).
   - Updated on `Mon 06 April 2026 at 07:48 PM` | 3795⭐ | `MIT License`
- [se1exin/Cleanarr](https://github.com/se1exin/Cleanarr) - A simple UI to help find and delete duplicate and sample files from your Plex server.
   - Updated on `Tue 23 July 2024 at 09:55 PM` | 274⭐ | `MIT License`
- [OneUptime/oneuptime](https://github.com/OneUptime/oneuptime) - Complete open-source monitoring and observability platform.
   - Updated on `Tue 09 June 2026 at 06:14 PM` | 7136⭐ | `Apache License 2.0`
- [fiduswriter/simple-datatables](https://github.com/fiduswriter/simple-datatables) - DataTables but in TypeScript transpiled to Vanilla JS.
   - Updated on `Sat 04 October 2025 at 11:42 AM` | 1604⭐ | `Other`
- [floating-ui/floating-ui](https://github.com/floating-ui/floating-ui) - A JavaScript library to position floating elements and create interactions for them.
   - Updated on `Tue 03 March 2026 at 03:02 AM` | 32616⭐ | `MIT License`
- [EvitanRelta/htmlarkdown](https://github.com/EvitanRelta/htmlarkdown) - HTML-to-Markdown converter that adaptively preserves HTML when needed (eg. when center-aligning, or resizing images).
   - Updated on `Sun 19 February 2023 at 12:04 PM` | 68⭐ | `MIT License`
- [upscayl/upscayl](https://github.com/upscayl/upscayl) - 🆙 Upscayl - #1 Free and Open Source AI Image Upscaler for Linux, MacOS and Windows.
   - Updated on `Fri 15 May 2026 at 10:35 PM` | 45924⭐ | `GNU Affero General Public License v3.0`
- [fallenbagel/jellywatch](https://github.com/fallenbagel/jellywatch) - A Javascript based web application for monitoring, analytics and notifications.
   - Updated on `Sat 29 June 2024 at 10:35 PM` | 54⭐ | `MIT License`
- [kubero-dev/kubero](https://github.com/kubero-dev/kubero) - A free and self-hosted PaaS alternative to Heroku / Netlify / Coolify / Vercel / Dokku / Portainer running on Kubernetes.
   - Updated on `Wed 15 April 2026 at 07:43 AM` | 4303⭐ | `GNU General Public License v3.0`
- [nativefier/nativefier](https://github.com/nativefier/nativefier) - Make any web page a desktop application.
   - Updated on `Fri 29 September 2023 at 01:45 PM` | 35310⭐ | `MIT License`
- [georgegebbett/recipe-buddy](https://github.com/georgegebbett/recipe-buddy) - Recipe Buddy is a tool that enables the easy adding of recipes to Grocy through web scraping.
   - Updated on `Fri 30 January 2026 at 03:20 PM` | 266⭐ | `GNU General Public License v3.0`
- [webosbrew/dev-manager-desktop](https://github.com/webosbrew/dev-manager-desktop) - Device/DevMode Manager for webOS TV.
   - Updated on `Mon 01 June 2026 at 10:24 AM` | 2325⭐ | `Apache License 2.0`
- [novuhq/novu](https://github.com/novuhq/novu) - The open-source communication infrastructure for agents and products.
   - Updated on `Tue 09 June 2026 at 07:08 PM` | 39085⭐ | `Other`
- [mifi/lossless-cut](https://github.com/mifi/lossless-cut) - The swiss army knife of lossless video/audio editing.
   - Updated on `Fri 05 June 2026 at 06:40 AM` | 41158⭐ | `GNU General Public License v2.0`
- [osmoscraft/osmosmemo](https://github.com/osmoscraft/osmosmemo) - Turn GitHub into a bookmark manager.
   - Updated on `Fri 01 August 2025 at 05:30 AM` | 492⭐ | `MIT License`
- [dubinc/dub](https://github.com/dubinc/dub) - The modern link attribution platform. Loved by world-class marketing teams like Framer, Perplexity, Superhuman, Twilio, Buffer and more.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 23662⭐ | `Other`
- [TechSquidTV/Shoutify](https://github.com/TechSquidTV/Shoutify) - An open source, self-hosted, and entirely free solution to social media management.
   - Updated on `Sat 28 January 2023 at 11:38 AM` | 410⭐ | `Apache License 2.0`
- [recurser/string-is](https://github.com/recurser/string-is) - An open-source, privacy-friendly online string toolkit for developers.
   - Updated on `Wed 03 June 2026 at 07:35 AM` | 301⭐ | `GNU Affero General Public License v3.0`
- [emuell/restic-browser](https://github.com/emuell/restic-browser) - A GUI to browse and restore restic backup repositories.
   - Updated on `Mon 13 April 2026 at 12:07 PM` | 1241⭐ | `MIT License`
- [Bit-Developer/text-compare-angular](https://github.com/Bit-Developer/text-compare-angular) - A tool used to compare text and show difference, built with Angular and Monaco Editor.
   - Updated on `Thu 02 April 2026 at 08:15 AM` | 29⭐ | `MIT License`
- [massCodeIO/massCode](https://github.com/massCodeIO/massCode) - A free, open-source developer workspace. Snippets, notes, HTTP requests, calculations, and dev tools in one local-first app.
   - Updated on `Tue 09 June 2026 at 03:05 PM` | 6813⭐ | `GNU Affero General Public License v3.0`
- [nobrayner/discord-webhook](https://github.com/nobrayner/discord-webhook) - A GitHub action to report workflow and job status to a Discord Channel via WebHook.
   - Updated on `Wed 12 July 2023 at 06:07 AM` | 11⭐ | `MIT License`
- [getwud/wud](https://github.com/getwud/wud) - Keep your containers up-to-date!.
   - Updated on `Wed 20 May 2026 at 11:29 AM` | 3596⭐ | `MIT License`
- [joelwmale/webhook-action](https://github.com/joelwmale/webhook-action) - Posts data to an endpoint on any event.
   - Updated on `Tue 02 June 2026 at 06:33 AM` | 164⭐ | `MIT License`
- [imLinguin/jelly-desktop](https://github.com/imLinguin/jelly-desktop) - Simple community made desktop client for Jellyfin.
   - Updated on `Thu 02 June 2022 at 12:10 PM` | 8⭐ | `GNU General Public License v3.0`
- [seerr-team/seerr](https://github.com/seerr-team/seerr) - Open-source media request and discovery manager for Jellyfin, Plex, and Emby.
   - Updated on `Wed 10 June 2026 at 12:33 AM` | 11527⭐ | `MIT License`
- [runtipi/runtipi](https://github.com/runtipi/runtipi) - Runtipi is a homeserver for everyone! One command setup, one click installs for your favorites self-hosted apps. ✨.
   - Updated on `Tue 02 June 2026 at 06:44 PM` | 9463⭐ | `GNU General Public License v3.0`
- [victrme/Bonjourr](https://github.com/victrme/Bonjourr) - Minimalist & lightweight startpage inspired by iOS.
   - Updated on `Tue 09 June 2026 at 09:00 AM` | 1953⭐ | `GNU General Public License v3.0`
- [ajnart/homarr](https://github.com/ajnart/homarr) - Customizable browser's home page to interact with your homeserver's Docker containers (e.g. Sonarr/Radarr).
   - Updated on `Mon 01 June 2026 at 11:11 PM` | 7208⭐ | `MIT License`
- [jeffvli/sonixd](https://github.com/jeffvli/sonixd) - A full-featured Subsonic/Jellyfin compatible desktop music player.
   - Updated on `Sat 17 February 2024 at 04:02 AM` | 1933⭐ | `GNU General Public License v3.0`
- [MauriceNino/dashdot](https://github.com/MauriceNino/dashdot) - A simple, modern server dashboard, primarily used by smaller private servers.
   - Updated on `Sat 06 June 2026 at 12:35 AM` | 3458⭐ | `MIT License`
- [xbrowsersync/api](https://github.com/xbrowsersync/api) - Server-side REST API that serves requests from xBrowserSync client apps.
   - Updated on `Fri 27 March 2026 at 10:06 PM` | 855⭐ | `GNU General Public License v3.0`
- [n8n-io/n8n](https://github.com/n8n-io/n8n) - Fair-code workflow automation platform with native AI capabilities. Combine visual building with custom code, self-host or cloud, 400+ integrations.
   - Updated on `Tue 09 June 2026 at 11:03 PM` | 191801⭐ | `Other`
- [photoview/photoview](https://github.com/photoview/photoview) - Photo gallery for self-hosted personal servers.
   - Updated on `Tue 09 June 2026 at 02:37 PM` | 6446⭐ | `GNU Affero General Public License v3.0`
- [leon-ai/leon](https://github.com/leon-ai/leon) - 🧠 Leon is your open-source personal assistant.
   - Updated on `Sun 07 June 2026 at 04:33 PM` | 17296⭐ | `MIT License`
- [Yooooomi/your_spotify](https://github.com/Yooooomi/your_spotify) - Self hosted Spotify tracking dashboard.
   - Updated on `Fri 29 May 2026 at 06:53 PM` | 4452⭐ | `GNU General Public License v3.0`
- [immich-app/immich](https://github.com/immich-app/immich) - High performance self-hosted photo and video management solution.
   - Updated on `Tue 09 June 2026 at 11:57 PM` | 102936⭐ | `GNU Affero General Public License v3.0`
- [Chocobozzz/PeerTube](https://github.com/Chocobozzz/PeerTube) - ActivityPub-federated video streaming platform using P2P directly in your web browser.
   - Updated on `Tue 09 June 2026 at 01:53 PM` | 14757⭐ | `GNU Affero General Public License v3.0`
- [ViewTube/viewtube](https://github.com/ViewTube/viewtube) - ▶️ ViewTube: The open source, privacy-conscious way to enjoy your favorite YouTube content. Docs: https://viewtube.wiki.
   - Updated on `Tue 09 June 2026 at 07:33 PM` | 1465⭐ | `GNU Affero General Public License v3.0`
- [ntoporcov/iQbit](https://github.com/ntoporcov/iQbit) - .
   - Updated on `Fri 01 May 2026 at 03:54 PM` | 768⭐ | `GNU General Public License v3.0`
- [umami-software/umami](https://github.com/umami-software/umami) - Umami is a modern, privacy-focused analytics platform. An open-source alternative to Google Analytics, Mixpanel and Amplitude.
   - Updated on `Tue 09 June 2026 at 02:17 AM` | 37091⭐ | `MIT License`
- [SigNoz/signoz](https://github.com/SigNoz/signoz) - SigNoz is an open-source observability platform native to OpenTelemetry with logs, traces and metrics in a single application. An open-source alternative to DataDog, NewRelic, etc. 🔥 🖥.   👉  Open sour.
   - Updated on `Tue 09 June 2026 at 10:49 PM` | 27268⭐ | `Other`
- [Egglord-Discord-Bot/auto-poster-and-notifications](https://github.com/Egglord-Discord-Bot/auto-poster-and-notifications) - This will send social media notifications to discord servers.
   - Updated on `Thu 16 March 2023 at 01:59 AM` | 54⭐ | `Apache License 2.0`
- [marktext/marktext](https://github.com/marktext/marktext) - 📝A simple and elegant markdown editor, available for Linux, macOS and Windows.
   - Updated on `Wed 10 June 2026 at 12:30 AM` | 57114⭐ | `MIT License`
- [jpmonette/feed](https://github.com/jpmonette/feed) - A RSS, Atom and JSON Feed generator for Node.js, making content syndication simple and intuitive! 🚀.
   - Updated on `Sun 19 April 2026 at 09:45 PM` | 1378⭐ | `MIT License`
- [bonukai/MediaTracker](https://github.com/bonukai/MediaTracker) - Self hosted media tracker for movies, tv shows, video games, books and audiobooks.
   - Updated on `Thu 20 February 2025 at 06:54 AM` | 907⭐ | `MIT License`
- [TriliumNext/Trilium](https://github.com/TriliumNext/Trilium) - Build your personal knowledge base with Trilium Notes.
   - Updated on `Tue 09 June 2026 at 07:48 PM` | 36385⭐ | `GNU Affero General Public License v3.0`
- [Tzahi12345/YoutubeDL-Material](https://github.com/Tzahi12345/YoutubeDL-Material) - Self-hosted YouTube downloader built on Material Design.
   - Updated on `Sun 08 March 2026 at 03:33 PM` | 3178⭐ | `MIT License`
- [laurent22/joplin](https://github.com/laurent22/joplin) - Joplin - the privacy-focused note taking app with sync capabilities for Windows, macOS, Linux, Android and iOS.
   - Updated on `Tue 09 June 2026 at 10:37 PM` | 55143⭐ | `Other`
- [burhan-syed/troddit](https://github.com/burhan-syed/troddit) - A web client for Reddit with authenticated logins and a variety of browsing options.
   - Updated on `Mon 28 July 2025 at 12:18 AM` | 624⭐ | `GNU Affero General Public License v3.0`
- [wg-easy/wg-easy](https://github.com/wg-easy/wg-easy) - The easiest way to run WireGuard VPN + Web-based Admin UI.
   - Updated on `Tue 09 June 2026 at 09:02 AM` | 26000⭐ | `GNU Affero General Public License v3.0`
- [medusajs/medusa](https://github.com/medusajs/medusa) - The world's most flexible commerce platform.
   - Updated on `Tue 09 June 2026 at 07:42 PM` | 34243⭐ | `MIT License`
- [Devrama/wonlog](https://github.com/Devrama/wonlog) - Stream your local logs to browsers.
   - Updated on `Thu 06 July 2023 at 03:14 PM` | 5⭐ | `❌`
- [pawelmalak/snippet-box](https://github.com/pawelmalak/snippet-box) - Snippet Box is a simple self-hosted app for organizing your code snippets. It allows you to easily create, edit, browse and manage your snippets in various languages.
   - Updated on `Wed 05 April 2023 at 05:41 PM` | 1084⭐ | `MIT License`
- [coder/code-server](https://github.com/coder/code-server) - VS Code in the browser.
   - Updated on `Thu 04 June 2026 at 06:50 PM` | 77886⭐ | `MIT License`
- [DavidIlie/personal-dashboard](https://github.com/DavidIlie/personal-dashboard) - Personal Dashboard for my services.
   - Updated on `Wed 24 April 2024 at 11:33 AM` | 118⭐ | `❌`
- [facebook/docusaurus](https://github.com/facebook/docusaurus) - Easy to maintain open source documentation websites.
   - Updated on `Fri 05 June 2026 at 08:45 PM` | 65148⭐ | `MIT License`
- [jesec/flood](https://github.com/jesec/flood) - A modern web UI for various torrent clients with a Node.js backend and React frontend.
   - Updated on `Tue 09 June 2026 at 07:58 PM` | 2781⭐ | `GNU General Public License v3.0`
- [pomber/stargazer](https://github.com/pomber/stargazer) - Your repo reached a stars milestone? Celebrate with a video of your stargazers!.
   - Updated on `Fri 20 February 2026 at 03:49 PM` | 1391⭐ | `❌`
- [pawelmalak/flame](https://github.com/pawelmalak/flame) - Flame is self-hosted startpage for your server. Easily manage your apps and bookmarks with built-in editors.
   - Updated on `Wed 03 June 2026 at 10:40 AM` | 6404⭐ | `MIT License`
- [portainer/portainer](https://github.com/portainer/portainer) - Making Docker and Kubernetes management easy.
   - Updated on `Tue 09 June 2026 at 01:49 PM` | 37687⭐ | `zlib License`
- [cross-seed/cross-seed](https://github.com/cross-seed/cross-seed) - Fully-automatic cross-seeding with Torznab.
   - Updated on `Thu 04 June 2026 at 07:48 PM` | 1472⭐ | `Apache License 2.0`
- [DIYgod/RSSHub](https://github.com/DIYgod/RSSHub) - 🧡 Everything is RSSible.
   - Updated on `Tue 09 June 2026 at 02:37 PM` | 44614⭐ | `GNU Affero General Public License v3.0`
- [bill-ahmed/qbit-matUI](https://github.com/bill-ahmed/qbit-matUI) - A material WebUI for qBittorrent, written in Angular.
   - Updated on `Fri 03 March 2023 at 02:30 PM` | 348⭐ | `MIT License`
- [grafana/grafana](https://github.com/grafana/grafana) - The open and composable observability and data visualization platform. Visualize metrics, logs, and traces from multiple sources like Prometheus, Loki, Elasticsearch, InfluxDB, Postgres and many more.
   - Updated on `Wed 10 June 2026 at 12:01 AM` | 74321⭐ | `GNU Affero General Public License v3.0`
- [watsonbox/exportify](https://github.com/watsonbox/exportify) - Export/Backup Spotify playlists using the Web API.
   - Updated on `Mon 13 April 2026 at 07:17 PM` | 4111⭐ | `MIT License`
- [sct/overseerr](https://github.com/sct/overseerr) - Request management and media discovery tool for the Plex ecosystem.
   - Updated on `Sun 15 February 2026 at 08:34 PM` | 5005⭐ | `MIT License`
- [iam4x/bobarr](https://github.com/iam4x/bobarr) - 🍿 The all-in-one alternative for Sonarr, Radarr, Jackett... with a VPN and running in docker.
   - Updated on `Sat 27 January 2024 at 11:51 PM` | 1690⭐ | `MIT License`
- [iptv-org/iptv](https://github.com/iptv-org/iptv) - Collection of publicly available IPTV channels from all over the world.
   - Updated on `Tue 09 June 2026 at 10:53 PM` | 117498⭐ | `The Unlicense`
- [out386/aria-telegram-mirror-bot](https://github.com/out386/aria-telegram-mirror-bot) - A Telegram bot to download files via HTTP(S)/BitTorrent and upload them to Google Drive.
   - Updated on `Sun 03 January 2021 at 11:58 AM` | 522⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## VBScript 

- [pyenv-win/pyenv-win](https://github.com/pyenv-win/pyenv-win) - pyenv for Windows. pyenv is a simple python version management tool. It lets you easily switch between multiple versions of Python. It's simple, unobtrusive, and follows the UNIX tradition of single-p.
   - Updated on `Fri 05 June 2026 at 04:41 AM` | 7291⭐ | `MIT License`
- [abbodi1406/vcredist](https://github.com/abbodi1406/vcredist) - AIO Repack for latest Microsoft Visual C++ Redistributable Runtimes.
   - Updated on `Mon 08 June 2026 at 04:42 PM` | 7748⭐ | `The Unlicense`
 
**[`^        back to top        ^`](#)**

## Visual Basic .NET 

- [AAndyProgram/SCrawler](https://github.com/AAndyProgram/SCrawler) - 🏳️‍🌈 Media downloader from any sites, including Twitter, Reddit, Instagram, BlueSky, TikTok, Threads, Facebook, OnlyFans, YouTube, Pinterest, PornHub, XHamster, XVIDEOS, ThisVid etc.
   - Updated on `Mon 01 June 2026 at 08:12 AM` | 2067⭐ | `GNU General Public License v3.0`
- [ChrisAnd1998/TaskbarX](https://github.com/ChrisAnd1998/TaskbarX) - Center Windows taskbar icons with a variety of animations and options.
   - Updated on `Sun 21 January 2024 at 07:45 PM` | 8019⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**

## Vue 

- [primefaces/primevue](https://github.com/primefaces/primevue) - Next Generation Vue UI Component Library.
   - Updated on `Mon 11 May 2026 at 05:42 PM` | 14438⭐ | `MIT License`
- [vbenjs/vue-vben-admin](https://github.com/vbenjs/vue-vben-admin) - A modern vue admin panel built with Vue3, Shadcn UI, Vite, TypeScript, and Monorepo. It's fast!.
   - Updated on `Tue 09 June 2026 at 07:17 PM` | 32586⭐ | `MIT License`
- [tiny-craft/tiny-rdm](https://github.com/tiny-craft/tiny-rdm) - Tiny RDM (Tiny Redis Desktop Manager) - A modern, colorful, super lightweight Redis GUI client for Mac, Windows, and Linux. It also provides a web version that can be deployed via Docker.
   - Updated on `Mon 18 May 2026 at 08:23 AM` | 12848⭐ | `GNU General Public License v3.0`
- [tranxuanthang/lrcget](https://github.com/tranxuanthang/lrcget) - Utility for mass-downloading LRC synced lyrics for your offline music library.
   - Updated on `Fri 05 June 2026 at 06:52 AM` | 2755⭐ | `MIT License`
- [VueTorrent/VueTorrent](https://github.com/VueTorrent/VueTorrent) - The sleekest looking WEBUI for qBittorrent made with Vuejs!.
   - Updated on `Mon 08 June 2026 at 08:44 AM` | 6829⭐ | `GNU General Public License v3.0`
- [zyronon/douyin](https://github.com/zyronon/douyin) - Vue3 + Pinia 仿抖音，Vue 在移动端的最佳实践 .  Imitate TikTok ，Vue Best practices on Mobile.
   - Updated on `Fri 05 June 2026 at 06:57 PM` | 11468⭐ | `GNU General Public License v3.0`
- [jellyfin/jellyfin-vue](https://github.com/jellyfin/jellyfin-vue) - A modern web client for Jellyfin based on Vue.
   - Updated on `Tue 09 June 2026 at 09:42 PM` | 1673⭐ | `GNU General Public License v3.0`
- [unovue/reka-ui](https://github.com/unovue/reka-ui) - An open-source UI component library for building high-quality, accessible design systems and web apps for Vue. Previously Radix Vue.
   - Updated on `Mon 08 June 2026 at 11:34 PM` | 6543⭐ | `MIT License`
- [buefy/buefy](https://github.com/buefy/buefy) - Lightweight UI components for Vue.js based on Bulma.
   - Updated on `Tue 09 June 2026 at 09:31 PM` | 9532⭐ | `MIT License`
- [FreeTubeApp/FreeTube](https://github.com/FreeTubeApp/FreeTube) - An Open Source YouTube app for privacy.
   - Updated on `Tue 09 June 2026 at 11:01 AM` | 21186⭐ | `GNU Affero General Public License v3.0`
- [pure-admin/vue-pure-admin](https://github.com/pure-admin/vue-pure-admin) - 全面ESM+Vue3+Vite+Element-Plus+TypeScript编写的一款后台管理系统（兼容移动端）.
   - Updated on `Fri 05 June 2026 at 03:02 AM` | 20285⭐ | `MIT License`
- [kaangiray26/forte](https://github.com/kaangiray26/forte) - Self-hosted, music streaming platform.
   - Updated on `Sat 27 September 2025 at 11:18 PM` | 356⭐ | `GNU General Public License v3.0`
- [CorentinTh/it-tools](https://github.com/CorentinTh/it-tools) - Collection of handy online tools for developers, with great UX.
   - Updated on `Mon 01 June 2026 at 10:39 PM` | 39224⭐ | `GNU General Public License v3.0`
- [Chanzhaoyu/chatgpt-web](https://github.com/Chanzhaoyu/chatgpt-web) - 用 Express 和  Vue3 搭建的 ChatGPT 演示网页.
   - Updated on `Fri 16 August 2024 at 03:26 PM` | 31620⭐ | `MIT License`
- [dullage/flatnotes](https://github.com/dullage/flatnotes) - A self-hosted, database-less note taking web app that utilises a flat folder of markdown files for storage.
   - Updated on `Tue 17 February 2026 at 12:43 PM` | 3103⭐ | `MIT License`
- [Splode/optimus](https://github.com/Splode/optimus) - Image conversion and optimization desktop app.
   - Updated on `Mon 23 March 2026 at 02:48 AM` | 550⭐ | `MIT License`
- [antonreshetov/mysigmail](https://github.com/antonreshetov/mysigmail) - A free and open-source email signature generator for Gmail, Outlook, Apple Mail, etc.
   - Updated on `Wed 03 June 2026 at 03:01 AM` | 1353⭐ | `GNU Affero General Public License v3.0`
- [interaapps/pastefy](https://github.com/interaapps/pastefy) - Pastefy is an feature-rich Open Source self-hostable Pastebin.
   - Updated on `Sat 06 June 2026 at 05:57 PM` | 433⭐ | `MIT License`
- [requarks/wiki](https://github.com/requarks/wiki) - Wiki.js | A modern and powerful wiki app built on Node.js.
   - Updated on `Fri 01 May 2026 at 10:34 AM` | 28423⭐ | `GNU Affero General Public License v3.0`
- [VueTubeApp/VueTube](https://github.com/VueTubeApp/VueTube) - A simple FOSS video streaming client aimed to recreate ALL features from their respective apps (and more).
   - Updated on `Fri 16 February 2024 at 10:24 AM` | 3191⭐ | `GNU General Public License v3.0`
- [bastienwirtz/homer](https://github.com/bastienwirtz/homer) - A very simple static homepage for your server.
   - Updated on `Sat 18 April 2026 at 02:35 PM` | 11387⭐ | `Apache License 2.0`
- [CzBiX/qb-web](https://github.com/CzBiX/qb-web) - A qBittorrent Web UI, write in TypeScript+Vue.
   - Updated on `Mon 17 June 2024 at 03:33 PM` | 1771⭐ | `GNU General Public License v3.0`
- [SushyDev/vapor-store](https://github.com/SushyDev/vapor-store) - Vapor Store.
   - Updated on `Wed 02 March 2022 at 02:36 AM` | 189⭐ | `❌`
 
**[`^        back to top        ^`](#)**

## YAML 

- [stashapp/CommunityScrapers](https://github.com/stashapp/CommunityScrapers) - This is a public repository containing scrapers created by the Stash Community.
   - Updated on `Mon 01 June 2026 at 03:46 PM` | 817⭐ | `GNU Affero General Public License v3.0`
 
**[`^        back to top        ^`](#)**

## Zig 

- [lightpanda-io/browser](https://github.com/lightpanda-io/browser) - Lightpanda: the headless browser designed for AI and automation.
   - Updated on `Tue 09 June 2026 at 11:44 PM` | 30874⭐ | `GNU Affero General Public License v3.0`
- [ghostty-org/ghostty](https://github.com/ghostty-org/ghostty) - 👻 Ghostty is a fast, feature-rich, and cross-platform terminal emulator that uses platform-native UI and GPU acceleration.
   - Updated on `Mon 08 June 2026 at 01:59 PM` | 56201⭐ | `MIT License`
 
**[`^        back to top        ^`](#)**


## License

[![CC0](http://mirrors.creativecommons.org/presskit/buttons/88x31/svg/cc-zero.svg)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, [NiNiyas](https://github.com/NiNiyas) has waived all copyright and related or neighboring rights to this work.

